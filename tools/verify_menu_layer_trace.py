...
source = json.loads(subprocess.check_output(["node", str(root / "tools/original_menu_layer_trace_harness.js"), str(fixture)], text=True))
port = json.loads(subprocess.check_output([str(root / "gradlew"), "-q", f"-DmenuFixture={fixture}", ":core:menuLayerTrace"], cwd=root, text=True))
...
if any(identifier.startswith("switch-") for identifier in ids):
    ...
    if source != port: raise SystemExit("MENU_LAYER_TRACE_MISMATCH\nsource="+json.dumps(source)+"\nport="+json.dumps(port))
