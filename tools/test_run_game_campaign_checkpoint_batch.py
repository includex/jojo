...
SCRIPT = Path(__file__).with_name("run_game_campaign_checkpoint_batch.py")
...
def route_trace(stop="R_01", actual=None, completion="checkpoint"):
    ...
    stages = [0, 1]
    route += [
        f"ScenarioScreen:{actual}",
        f"ScenarioScreen:{actual}:scene0",
        f"ScenarioScreen:{actual}:scene1",
    ]
