#!/usr/bin/env python3
"""Compare the live source Hall node tree with the port renderer frame log."""
...
if len(sys.argv) != 3:
    raise SystemExit("usage: verify_hall_composition.py SOURCE_FIXTURE.json PORT_TRACE.json")

source = json.load(open(sys.argv[1], encoding="utf-8"))
port = json.load(open(sys.argv[2], encoding="utf-8"))
snapshot = source["snapshot"]
scale = port["viewport"][1] / snapshot["visibleSize"]["height"]
close(port["viewport"], [snapshot["visibleSize"]["width"] * scale, snapshot["visibleSize"]["height"] * scale], "viewport")
...
port_heads = sorted(port["heads"], key=lambda head: head["rect"][0])
expected_heads = []
...
if len(port_heads) != len(expected_heads):
    raise AssertionError(f"head membership differs: port={len(port_heads)}, source={len(expected_heads)}")
for index, (candidate, expected) in enumerate(zip(port_heads, sorted(expected_heads))):
...
if port["backgroundId"] != expected_background_id or sorted(port_units) != sorted(unit[0] for unit in fixture["units"]):
...
print(f"HALL_COMPOSITION_OK scale={scale:.2f} units={len(port_units)} heads={len(port_heads)} panel=exact face=exact")
