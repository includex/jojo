...
# 이 파일을 "중복 블록"으로 보고 정리하지 말 것.
#
# `summarize`가 세 번 거듭 정의되고 뒤의 정의가 앞을 덮는 것은 맞다. 그래서 출력에 닿는
# `summarize`는 마지막 것뿐이다. 하지만 **마지막 판정부는 앞 블록이 만든 이름을 쓴다** —
# 예를 들어 `WALL_ONLY`는 첫 블록에서 정의되고 마지막 `timingChecks` 계산이 그것을 읽는다.
# 앞 블록을 지우면 `NameError`로 멈춘다(2026-09-20에 실제로 그렇게 깨뜨렸다가 되돌렸다).
#
# 정리하려면 세 블록을 한 벌로 합치고, 합치기 전후의 출력이 **바이트 단위로 같은지**
# 같은 trace 쌍으로 확인해야 한다. 그 확인 없이 줄을 지우지 말 것.
def summarize(path):
