...
# 허용치 = 고정 폭(TOL) + 양쪽 창의 정체 폭. QUANTIZED/DIALOGUE_TERMINATED는 정체와 무관한
# 이유로 이미 판정하지 않으므로 그대로 둔다.
skip=QUANTIZED|DIALOGUE_TERMINATED
...
r['timingChecks']={k:(True if k in skip else abs(v)<=r['tolerances'][k]) for k,v in r['timingDeltas'].items()}
