...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("usage: verify_battle_menu_render.py <source-menu.png> <port-menu.png>")
    source, port = (Image.open(Path(path)).convert("RGBA") for path in sys.argv[1:])
    if source.size != port.size:
        raise AssertionError(f"menu capture dimensions differ: {source.size} vs {port.size}")
    ...
    for index, x in enumerate(starts):
        ...
        if ImageChops.difference(source.crop(box), port.crop(box)).getbbox():
