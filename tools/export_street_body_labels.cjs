...
const outputRoot = path.resolve(process.argv[4] || process.env.JOJO_BODY_LABEL_OUT || path.join(process.cwd(), 'core/build/generated/street-body-labels'));
...
const contract = JSON.parse(fs.readFileSync(path.join(__dirname, 'street_body_label_contract.json')));
...
function pythonInput(sourceFile) {
  const script = `import ast,json,sys\np=sys.argv[1]\nt=ast.parse(open(p,encoding='utf8').read())\nf=next(n for n in t.body if isinstance(n,ast.FunctionDef) and n.name=='scene1')\nc=[]\nfor n in ast.walk(f):\n if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and n.func.attr=='say' and n.args and isinstance(n.args[0],ast.Constant) and isinstance(n.args[0].value,str): c.append((n.lineno,n.args[0].value))\nc.sort(); print(json.dumps([v for _,v in c[:2]],ensure_ascii=False))`;
...
function astJsonInput(value) {
  ...
  function walk(node, inScene1=false) {
    ...
    if (scene && node.type==='Call' && ((fields.func?.type==='Attribute' && fields.func.fields?.attr==='say') || (fields.func?.type==='Name' && fields.func.fields?.id==='say'))) {
      const arg=fields.args?.[0];
      if (arg?.type==='Constant' && typeof arg.fields?.value==='string') calls.push([node.location?.line||0,arg.fields.value]);
    }
    for (const child of Object.values(fields)) walk(child,scene);
  }
  walk(value.ast||value); calls.sort((a,b)=>a[0]-b[0]);
  if (calls.length<2) throw Error('R_00 AST JSON has fewer than two scene1 say calls');
  return pageInput(calls.slice(0,2).map(x=>x[1]));
}
function pageInput(calls) {
  ...
  for (const call of calls) {
    ...
    for (const line of call.split('\n')) {
      if (/^&\d+$/.test(line)) { if (current!==null) pages.push(current); current=''; }
      else if (current!==null) current+=(current?'\n':'')+line;
    }
    if (current!==null) pages.push(current);
  }
  if (pages.length!==3 || pages.some(text=>!text)) throw Error(`expected three initial R_00 pages, got ${JSON.stringify(pages)}`);
  return {width:728,pages};
}
function loadInput() {
  let value;
  if (catalogPath.endsWith('.py')) value=pythonInput(catalogPath);
    ...
    value=(Array.isArray(json)||Array.isArray(json.pages))?json:astJsonInput(json);
  }
  const width=Number(value.width??728), pages=Array.isArray(value)?value:value.pages;
  if (!Number.isFinite(width)||width<=0||!Array.isArray(pages)||pages.some(x=>typeof x!=='string'||!x)) throw Error('input must be R_00 AST JSON/Python or {width,pages:[nonempty strings]}');
  ...
  return {width,pages,prefixes:pages.flatMap((page,pageIndex)=>Array.from(page).map((_,i)=>({pageIndex,text:Array.from(page).slice(0,i+1).join('')})))};
}
