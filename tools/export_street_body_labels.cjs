...
function loadInput() {
  ...
  return {width,pages,prefixes:pages.flatMap((page,pageIndex)=>[...(labelStyle==='info'?[{pageIndex,text:''}]:[]),...Array.from(page).map((_,i)=>({pageIndex,text:Array.from(page).slice(0,i+1).join('')}))])};
}
