...
MODULE_PATH = Path(__file__).with_name("run_port_full_battle_batch.py")
SPEC = importlib.util.spec_from_file_location("port_batch", MODULE_PATH)
port_batch = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(port_batch)


class PortFullBattleBatchTest(unittest.TestCase):
    def trace(self, scenario="S_01", evidence=True, frames=None):
        frames = frames or [{}]
        summary = {"scenario": scenario, "end": True, "frameCount": len(frames)}
        if evidence:
            summary["portScenario"] = {
    ...
    def inspect(self, value, expected="S_01"):
        with tempfile.TemporaryDirectory() as directory:
            ...
            return port_batch.inspect_trace(path, expected)

    def test_default_and_selected_scenarios(self):
        self.assertEqual(58, len(port_batch.selected_scenarios(None)))
        self.assertEqual(["S_01", "S_57"], port_batch.selected_scenarios(["s_01,S_57", "S_01"]))

    def test_parallel_job_count_is_explicit_and_positive(self):
        options = port_batch.parse_args(["--scenario", "S_01,S_02", "--jobs", "3"])
        self.assertEqual(3, options.jobs)
        self.assertEqual(["S_01", "S_02"], options.scenarios)
        with contextlib.redirect_stderr(io.StringIO()):
            with self.assertRaises(SystemExit):
                port_batch.parse_args(["--jobs", "0"])

    def test_gzip_trace_path_is_selected_before_runner_launch(self):
        manifest = Path("build/manifest.json")
        self.assertEqual("S_18.json", port_batch.trace_path(manifest, "S_18").name)
        self.assertEqual("S_18.json.gz", port_batch.trace_path(manifest, "S_18", compressed=True).name)
    ...
    def test_inspects_gzipped_trace_without_losing_schema_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = port_batch.inspect_trace(path, "S_01")
            self.assertTrue(result["terminal"])
            self.assertIsNone(result["error"])
    ...
    def test_s52_requires_exact_canonical_initial_object_payload_and_order(self):
        canonical = list(port_batch.AUTHORED_EXACT_OBSERVATION_CONTRACTS["S_52"]["objectsPrefix"])
    ...
    def test_s57_contract_uses_exact_common_sequence_and_either_authored_branch(self):
        exact = port_batch.AUTHORED_EXACT_OBSERVATION_CONTRACTS["S_57"]
