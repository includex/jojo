...
FORMAT = "jojo-game-campaign-checkpoint-batch-manifest/v1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    for checkpoint in options.checkpoints:
        ...
        if (
            ...
            print(f"[game-campaign] {checkpoint}: resumed")
        else:
            pending.append(checkpoint)
            print(f"[game-campaign] {checkpoint}: running", flush=True)
    ...
    print(
        f"[game-campaign] complete: {len(selected) - len(failures)}/{len(selected)} reached; "
