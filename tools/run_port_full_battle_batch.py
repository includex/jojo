...
FORMAT = "jojo-port-full-battle-batch-manifest/v1"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("build/port-full-battle-batch/manifest.json"))
...
def inspect_trace(path: Path, expected: str) -> dict[str, object]:
    ...
    evidence = summary.get("portScenario") if isinstance(summary, dict) else None
    expected_script = f"Game/data/RS/{expected}"
    if not isinstance(evidence, dict):
        errors.append("portScenario evidence missing")
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for scenario in options.scenarios:
        ...
        if options.resume and prior and prior.get("passed") is True and prior_path.is_file() and inspect_trace(prior_path, scenario)["error"] is None:
            print(f"[port-full-battle] {scenario}: resumed")
            continue
        pending.append(scenario)
    for scenario in pending:
        print(f"[port-full-battle] {scenario}: running", flush=True)
    with ThreadPoolExecutor(max_workers=options.jobs) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(f"[port-full-battle] {scenario}: {'passed' if row['passed'] else 'failed'} reason={row['reason']} frames={row['frames']}", flush=True)
    manifest["results"] = [previous[key] for key in sorted(previous)]
    atomic_json(options.output, manifest)
    ...
    print(f"[port-full-battle] complete: {len(selected) - len(failures)}/{len(selected)} passed; manifest={options.output}")
