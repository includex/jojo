#!/usr/bin/env python3
"""Verify StartBattleScreen source nodes against the game's submitted quads."""
...
if len(sys.argv) != 3:
    raise SystemExit("usage: verify_start_battle_composition.py SOURCE.json GAME.json")
...
close(game["outerRect"], rect([canvas[0] * scale, canvas[1] * scale], [bg["size"][0] * scale, bg["size"][1] * scale]), "outer")

roster = node("Canvas/Layer/bg/scrollview")
roster_center = [(canvas[0] + roster["position"][0]) * scale, (canvas[1] + roster["position"][1]) * scale]
close(game["rosterClipRect"], rect(roster_center, [roster["size"][0] * scale, roster["size"][1] * scale]), "roster clip")

selected = node("Canvas/Layer/bg/bg")
selected_center = [(canvas[0] + selected["position"][0]) * scale, (canvas[1] + selected["position"][1]) * scale]
close(game["selectedPanelRect"], rect(selected_center, [selected["size"][0] * scale, selected["size"][1] * scale]), "selected panel")

info = node("Canvas/Layer/bg/box1")
info_center = [(canvas[0] + info["position"][0]) * scale, (canvas[1] + info["position"][1]) * scale]
close(game["infoPanelRect"], rect(info_center, [info["size"][0] * scale, info["size"][1] * scale]), "info panel")
...
close(game["infoTitleRect"], rect(title_center, [title["size"][0] * scale, title["size"][1] * scale]), "info title")
...
for key, path in [("confirmRect", "Canvas/Layer/bg/button0"), ("cancelRect", "Canvas/Layer/bg/button1")]:
    ...
    close(game[key], rect(center, [button["size"][0] * scale, button["size"][1] * scale]), key)
...
expected_frames = []
for row_index, (start, row) in enumerate(rows[:len(game["slots"])]):
...
for slot, expected in zip(game["slots"], expected_frames):
    close(slot["frameRect"], expected, f"slot {slot['index']}")

if len(game["roster"]) != 4 or game["selectedUnitId"] != 0 or game["backgroundId"] != 71:
    raise AssertionError("fixture membership, cursor, or background differs")
print(f"START_BATTLE_COMPOSITION_OK scale={scale:.2f} roster={len(game['roster'])} slots={len(game['slots'])} geometry=exact")
