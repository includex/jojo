#!/usr/bin/env node
/** Verifies the source Control._process selection overlay against the game capture. */
...
const gameCapture = "/tmp/jojo-yingchuan-selection.png";
...
assert.deepEqual(game.slice(1, 4).map(Number), [210, 10, 17], "game selected a different source unit");
assert.equal(Number(game[4]), sourceMove, "move-range tile count differs from source");
assert.equal(game[5], "green", "fixture must use the source non-Control Mine range frame");
assert.equal(Number(game[6]), sourceAttack, "attack RED_BOX tile count differs from source");
assert.equal(game[7], "true", "source Mark_14 cursor was not rendered");
assert.ok(existsSync(gameCapture), "game selection framebuffer was not written");
