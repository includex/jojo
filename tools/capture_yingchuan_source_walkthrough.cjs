...
const maxAllowedWallMs = ['round3-player-action', 'round3-first-combat'].includes(semanticMode) ? 210000 : ['single-player-action', 'round2-followup', 'round2-first-combat'].includes(semanticMode) ? 180000 : semanticMode === 'round2-handoff' ? 150000 : semanticMode === 'first-round-end' ? 90000 : 60000;
...
if (semanticMode && !['235-hit-hold', 'first-normal-combat', 'next-normal-actions', 'enemy-first-combat', 'enemy-arrival-only', '477-settlement-order', 'first-round-end', 'round2-handoff', 'single-player-action', 'round2-followup', 'round2-first-combat', 'round3-player-action', 'round3-first-combat'].includes(semanticMode)) throw new Error(`unknown semantic mode ${semanticMode}`);
...
(async () => {
  ...
  try {
    ...
    if(['single-player-action','round2-followup','round2-first-combat','round3-player-action','round3-first-combat'].includes(semanticMode)){
      const followup=semanticMode!=='single-player-action',round3Mode=['round3-player-action','round3-first-combat'].includes(semanticMode),round3FirstCombat=semanticMode==='round3-first-combat',continueFirstDialogue=semanticMode==='round2-first-combat'||round3Mode;
      const observations=[], inputs=[], wanted=['driver-disabled','unit0-selected','destination-reachable','unit0-moved','attack-command','enemy484-selected','attack-resolved','post-action-terminal',...(followup?['end-round-confirmed','next-camp-observed','followup-terminal']:[]),...(round3Mode?['round3-player-ready','round3-unit0-selected','round3-destination-reachable','round3-unit0-moved','round3-attack-command','round3-enemy483-selected','round3-attack-resolved','round3-post-action-terminal']:[]),...(round3FirstCombat?['round3-critical-dialogue-complete','round3-critical-dialogue-closed']:[])];
      ...
      try{
        ...
        if(followup){
          ...
          while(Date.now()-started<maxWallMs-1000&&captures.length<12){
            ...
            if(round3Mode&&state.round===3&&state.camp===0&&detail.ctrlWaiting&&!state.dialogue&&!detail.msgBox&&!detail.menu){terminal={kind:'round3-player-ready',frame:state.frame};seen.add('round3-player-ready');await snapshot('round3-player-ready');break;}
            if(state.dialogue){
          ...
          if(round3Mode){
            ...
            if(!s.round3Reachable)throw Error(`round3 destination 10,5 is not reachable: ${JSON.stringify(s.reachableKeys)}`);seen.add('round3-destination-reachable');await snapshot('round3-unit0-selected-reachable');s=await waitStablePoint('round3-destination-layout',x=>x.round3Destination,x=>x.selectedUnit===0&&x.ctrlWaiting&&x.round3Reachable);
            await click('round3-select-destination-10-5',s.round3Destination.point,s);s=await waitState('round3-unit0-moved-command-open',x=>x.unit0?.x===10&&x.unit0?.y===5&&x.command&&x.commandAttackInteractable,12000);seen.add('round3-unit0-moved');seen.add('round3-attack-command');await snapshot('round3-unit0-moved-command');s=await waitStablePoint('round3-attack-command-layout',x=>x.commandGeometry.attackButton,x=>x.command&&x.commandAttackInteractable&&x.unit0?.x===10&&x.unit0?.y===5);
            ...
            const beforeHp=s.enemy483.hp;await click('round3-select-enemy-483',s.enemy483.geometry.cssCenter,s);seen.add('round3-enemy483-selected');await snapshot('round3-enemy483-selected');
