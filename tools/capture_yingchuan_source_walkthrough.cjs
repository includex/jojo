...
const maxAllowedWallMs = semanticMode === 'round3-counterattack' ? 240000 : ['round3-player-action', 'round3-first-combat', 'round3-followup'].includes(semanticMode) ? 210000 : ['single-player-action', 'round2-followup', 'round2-first-combat'].includes(semanticMode) ? 180000 : semanticMode === 'round2-handoff' ? 150000 : semanticMode === 'first-round-end' ? 90000 : 60000;
...
if (semanticMode && !['235-hit-hold', 'first-normal-combat', 'next-normal-actions', 'enemy-first-combat', 'enemy-arrival-only', '477-settlement-order', 'first-round-end', 'round2-handoff', 'single-player-action', 'round2-followup', 'round2-first-combat', 'round3-player-action', 'round3-first-combat', 'round3-followup', 'round3-counterattack'].includes(semanticMode)) throw new Error(`unknown semantic mode ${semanticMode}`);
...
(async () => {
  ...
  try {
    ...
    if(['single-player-action','round2-followup','round2-first-combat','round3-player-action','round3-first-combat','round3-followup','round3-counterattack'].includes(semanticMode)){
      const followup=semanticMode!=='single-player-action',round3Mode=['round3-player-action','round3-first-combat','round3-followup','round3-counterattack'].includes(semanticMode),round3Counterattack=semanticMode==='round3-counterattack',round3Followup=semanticMode==='round3-followup'||round3Counterattack,round3FirstCombat=semanticMode==='round3-first-combat'||round3Followup,continueFirstDialogue=semanticMode==='round2-first-combat'||round3Mode;
      const observations=[], inputs=[], wanted=['driver-disabled','unit0-selected','destination-reachable','unit0-moved','attack-command','enemy484-selected','attack-resolved','post-action-terminal',...(followup?['end-round-confirmed','next-camp-observed','followup-terminal']:[]),...(round3Mode?['round3-player-ready','round3-unit0-selected','round3-destination-reachable','round3-unit0-moved','round3-attack-command','round3-enemy483-selected','round3-attack-resolved','round3-post-action-terminal']:[]),...(round3FirstCombat?['round3-critical-dialogue-complete','round3-critical-dialogue-closed']:[]),...(round3Followup?['round3-followup-confirmed','round3-followup-terminal']:[]),...(round3Counterattack?['round3-counter-dialogue-complete','round3-counter-dialogue-closed']:[])];
        ...
        if(followup){
          ...
          if(round3Mode){
            ...
            if(round3Followup){
              ...
              let followupMovement=false,followupAttack=false,followupHit=false,followupSettlement=false,counterMovement=false,counterAttack=false,counterHit=false,counterSettlement=false,counterDialogueAdvanced=false,counterAiStartOffset=null,counterActorStart=null,round3FollowupTerminal=null;
              while(Date.now()-started<maxWallMs-1000&&captures.length<12){
                ...
                if(!followupMovement&&moving.length){followupMovement=true;if(captures.length<11)await snapshot('round3-followup-movement');}
                if(!followupAttack&&attacking.length){followupAttack=true;if(captures.length<11)await snapshot('round3-followup-attack');}
                if(!followupHit&&hit.length){followupHit=true;if(captures.length<11)await snapshot('round3-followup-hit');}
                if(!followupSettlement&&settlement){followupSettlement=true;if(captures.length<11)await snapshot('round3-followup-settlement');}
                if(counterDialogueAdvanced){
                  if(!counterMovement&&moving.length){counterMovement=true;if(captures.length<11)await snapshot('round3-counter-movement');}
                  if(!counterAttack&&attacking.length){counterAttack=true;if(captures.length<11)await snapshot('round3-counter-attack');}
                  if(!counterHit&&hit.length){counterHit=true;if(captures.length<11)await snapshot('round3-counter-hit');}
                  if(!counterSettlement&&settlement){counterSettlement=true;if(captures.length<11)await snapshot('round3-counter-settlement');}
                }
                if(detail.dialogue){
                  if(round3Counterattack&&!counterDialogueAdvanced){
                    ...
                    if(detail.dialogueText!==expected||detail.dialogueRemaining!==''||detail.dialogueTypingActive||!detail.dialoguePanel){await delay(16);continue;}
                    counterAiStartOffset=detail.aiActorStarts.length;counterActorStart=detail.aiActorStarts[counterAiStartOffset-1]||null;if(!counterActorStart)throw Error(`round3 counter dialogue has no observed current AI actor: ${JSON.stringify(detail.aiActorStarts)}`);seen.add('round3-counter-dialogue-complete');if(captures.length<11)await snapshot('round3-counter-dialogue-complete');const fresh=await waitStablePoint('round3-counter-dialogue-layout',x=>x.dialoguePanel,x=>x.dialogue&&x.dialogueSpeakerId==='146'&&x.dialogueText===expected&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialoguePanel);await click('close-round3-counter-dialogue',fresh.dialoguePanel.cssCenter,fresh);counterDialogueAdvanced=true;seen.add('round3-counter-dialogue-closed');await waitState('round3-counter-dialogue-closed',x=>!x.dialogue,5000);continue;
                  }
                  const completed=await waitState('round3-followup-blocking-dialogue-complete',x=>x.dialogue&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialogueText,10000);round3FollowupTerminal={kind:'blocking-dialogue',frame:completed.frame,camp:completed.camp,speakerId:completed.dialogueSpeakerId,text:completed.dialogueText,dialogueComplete:true,aiActorStartOffset:round3Counterattack?counterAiStartOffset:round3AiStartOffset,currentActorStart:counterActorStart,newAiActorStarts:completed.aiActorStarts.slice(round3Counterattack?counterAiStartOffset:round3AiStartOffset)};if(captures.length<12)await snapshot('round3-followup-blocking-dialogue-complete');break;
                }
                const completionOffset=round3Counterattack?counterAiStartOffset:round3AiStartOffset,completionStarts=completionOffset==null?[]:detail.aiActorStarts.slice(completionOffset),completedActor=round3Counterattack?counterActorStart:newAiActorStarts[0],nextActorStart=round3Counterattack?completionStarts[0]:completionStarts[1];
                if((!round3Counterattack||counterDialogueAdvanced)&&nextActorStart){round3FollowupTerminal={kind:'next-ai-actor-started',frame:detail.frame,camp:detail.camp,completedActorId:completedActor&&completedActor.actorId,nextActorId:nextActorStart.actorId,aiActorStartOffset:completionOffset,currentActorStart:counterActorStart,newAiActorStarts:completionStarts};if(captures.length<12)await snapshot('round3-followup-first-action-complete');break;}
                if((!round3Counterattack||counterDialogueAdvanced)&&completedActor&&detail.camp!==completedActor.camp){round3FollowupTerminal={kind:'camp-transition-after-first-ai-action',frame:detail.frame,camp:detail.camp,completedActorId:completedActor.actorId,aiActorStartOffset:completionOffset,currentActorStart:counterActorStart,newAiActorStarts:completionStarts};if(captures.length<12)await snapshot('round3-followup-first-action-complete');break;}
                await delay(16);
              }
              ...
              round3FollowupResult={...round3FollowupTerminal,terminalReached:true,actionCompleted:round3FollowupTerminal.kind!=='blocking-dialogue',movementCaptured:followupMovement,attackCaptured:followupAttack,hitCaptured:followupHit,settlementCaptured:followupSettlement,counterDialogueAdvanced,counterActorStart,counterMovementCaptured:counterMovement,counterAttackCaptured:counterAttack,counterHitCaptured:counterHit,counterSettlementCaptured:counterSettlement};
