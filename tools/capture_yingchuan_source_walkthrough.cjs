...
const maxAllowedWallMs = ['round3-player-action', 'round3-first-combat', 'round3-followup'].includes(semanticMode) ? 210000 : ['single-player-action', 'round2-followup', 'round2-first-combat'].includes(semanticMode) ? 180000 : semanticMode === 'round2-handoff' ? 150000 : semanticMode === 'first-round-end' ? 90000 : 60000;
...
if (semanticMode && !['235-hit-hold', 'first-normal-combat', 'next-normal-actions', 'enemy-first-combat', 'enemy-arrival-only', '477-settlement-order', 'first-round-end', 'round2-handoff', 'single-player-action', 'round2-followup', 'round2-first-combat', 'round3-player-action', 'round3-first-combat', 'round3-followup'].includes(semanticMode)) throw new Error(`unknown semantic mode ${semanticMode}`);
        ...
        if(followup){
          ...
          if(round3Mode){
            ...
            while(Date.now()-started<maxWallMs-1000&&captures.length<12){
              ...
              if(!round3AttackCaptured&&round3Attacking.length){round3AttackCaptured=true;await snapshot('round3-critical-attack');}
              if(!damageObserved&&(!detail.enemy483?.exists||detail.enemy483.hp!==beforeHp)){damageObserved=true;await snapshot('round3-attack-damage-observed');}
              if(settlement&&!round3SettlementCaptured){round3SettlementCaptured=true;await snapshot('round3-settlement');}
              if(detail.dialogue){
                ...
                if(detail.dialogueText!==expected||detail.dialogueRemaining!==''||detail.dialogueTypingActive||!detail.dialoguePanel){await delay(16);continue;}
                seen.add('round3-critical-dialogue-complete');await snapshot('round3-critical-dialogue-complete');const fresh=await waitStablePoint('round3-critical-dialogue-layout',x=>x.dialoguePanel,x=>x.dialogue&&x.dialogueSpeakerId==='0'&&x.dialogueText===expected&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialoguePanel);await click('close-round3-critical-dialogue',fresh.dialoguePanel.cssCenter,fresh);round3CriticalAdvanced=true;seen.add('round3-critical-dialogue-closed');await waitState('round3-critical-dialogue-closed',x=>!x.dialogue,5000);continue;
              }
              if((!round3FirstCombat||round3CriticalAdvanced)&&detail.unit0?.acted===true&&!detail.fight&&!settlement&&(detail.ctrlWaiting||detail.msgBox)){if(detail.msgBox&&(detail.msgBoxText!=='모든 부대의 명령을 종료하시겠습니까?'||!detail.msgBoxConfirm))throw Error(`unexpected round3 post-action MsgBox: ${JSON.stringify(detail)}`);seen.add('round3-attack-resolved');round3Terminal={kind:detail.msgBox?'end-round-confirm':'player-input',frame:detail.frame,unit0Acted:detail.unit0.acted,damageObserved,settlementCaptured:round3SettlementCaptured};await snapshot(detail.msgBox?'round3-end-round-confirm-terminal':'round3-player-input-terminal');break;}
              await delay(16);
            }
            ...
            if(round3Followup){
              ...
              let followupMovement=false,followupAttack=false,followupHit=false,followupSettlement=false,round3FollowupTerminal=null;
              while(Date.now()-started<maxWallMs-1000&&captures.length<12){
                ...
                if(detail.dialogue){const completed=await waitState('round3-followup-blocking-dialogue-complete',x=>x.dialogue&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialogueText,10000);round3FollowupTerminal={kind:'blocking-dialogue',frame:completed.frame,camp:completed.camp,speakerId:completed.dialogueSpeakerId,text:completed.dialogueText,dialogueComplete:true,aiActorStartOffset:round3AiStartOffset,newAiActorStarts:completed.aiActorStarts.slice(round3AiStartOffset)};if(captures.length<12)await snapshot('round3-followup-blocking-dialogue-complete');break;}
                if(newAiActorStarts.length>=2){round3FollowupTerminal={kind:'next-ai-actor-started',frame:detail.frame,camp:detail.camp,completedActorId:newAiActorStarts[0].actorId,nextActorId:newAiActorStarts[1].actorId,aiActorStartOffset:round3AiStartOffset,newAiActorStarts};if(captures.length<12)await snapshot('round3-followup-first-action-complete');break;}
                if(newAiActorStarts.length&&detail.camp!==newAiActorStarts[0].camp){round3FollowupTerminal={kind:'camp-transition-after-first-ai-action',frame:detail.frame,camp:detail.camp,completedActorId:newAiActorStarts[0].actorId,aiActorStartOffset:round3AiStartOffset,newAiActorStarts};if(captures.length<12)await snapshot('round3-followup-first-action-complete');break;}
                await delay(16);
              }
              ...
              round3FollowupResult={...round3FollowupTerminal,terminalReached:true,actionCompleted:round3FollowupTerminal.kind!=='blocking-dialogue',movementCaptured:followupMovement,attackCaptured:followupAttack,hitCaptured:followupHit,settlementCaptured:followupSettlement};
