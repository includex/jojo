...
const maxAllowedWallMs = semanticMode === 'round3-210' ? 300000 : semanticMode === 'round3-counterattack' ? 240000 : ['round3-player-action', 'round3-first-combat', 'round3-followup'].includes(semanticMode) ? 210000 : ['single-player-action', 'round2-followup', 'round2-first-combat'].includes(semanticMode) ? 180000 : ['round2-handoff', 'menu-first-control'].includes(semanticMode) ? 150000 : semanticMode === 'first-round-end' ? 90000 : 60000;
...
if (semanticMode && !['235-hit-hold', 'first-normal-combat', 'next-normal-actions', 'enemy-first-combat', 'enemy-arrival-only', '477-settlement-order', 'first-round-end', 'round2-handoff', 'menu-first-control', 'single-player-action', 'round2-followup', 'round2-first-combat', 'round3-player-action', 'round3-first-combat', 'round3-followup', 'round3-counterattack', 'round3-210'].includes(semanticMode)) throw new Error(`unknown semantic mode ${semanticMode}`);
...
const stateExpression = `(() => {
  ...
  (function visit(node){if(!node)return; for(const component of node._components||[]){const name=cc.js.getClassName(component); if(name==='BattleLayer')battle=component;if(name==='SayLayer'&&node.activeInHierarchy)say=component; if(/Layer$/.test(name)&&node.activeInHierarchy)layers.push(name);if((name==='OtherUnitInfoLayer'||name==='MineUnitInfoLayer')&&node.activeInHierarchy){const labels=[];(function labelsOf(n,p){for(const c of n._components||[]){if(c instanceof cc.Label)labels.push({path:p,text:c.string});}n.children.forEach(ch=>labelsOf(ch,p+'/'+ch.name));})(node,node.name);infoPanels.push({kind:name,identity:node.uuid||node._id||null,labels,bars:Array.isArray(component._bars)?component._bars.map(b=>b&&b.progress):[],value:component._value??null,pendingValues:Array.isArray(component._arys)?component._arys.slice():[]});}} node.children.forEach(visit)})(scene);
  ...
  return {ready:!!battle,frame:cc.director.getTotalFrames?cc.director.getTotalFrames():null,scene:scene.name,round:battle&&battle.round?battle.round():null,camp:battle&&battle.curCamp?battle.curCamp():null,camera:camera?[camera.x,camera.y]:null,dialogue,dialogueSpeakerId,dialogueText:say&&say._curLab?say._curLab.string:null,dialogueRemaining:say&&say._nextString!=null?say._nextString:null,layers:[...new Set(layers)],gateKeys,infoPanels,battleTargetsEvidenceKind:'last-assigned-source-fields-not-current-action-authority',battleTargets:{source:targetInfo(battle&&battle._srcTarget),destination:targetInfo(battle&&battle._dscTarget)},units:units.filter(u=>[0,3,32,33,146,147,157,210,211,234,235,258,259,334,474,475,476,477,478,479,480,481,482,483,484,485].includes(u.id))};
})()`;
...
(async () => {
  ...
  // Chromium converts capturePage() into the attached display's colour profile; on a P3
  // display that shifts every source frame, so colour comparisons against the port's plain
  // sRGB framebuffer are meaningless without this flag.
  const child = spawn('npm', ['exec', '--', 'electron', '.', '--force-color-profile=srgb', '--battle', '--scenario=S_00', `--full-battle-trace=${trace}`, '--full-battle-time-scale=1', `--full-battle-max-wall-ms=${maxWallMs}`, '--full-battle-seed=1000', '--full-battle-math-seed=305419896', `--remote-debugging-port=${port}`], { cwd: sourceRoot, stdio: ['ignore', logFd, logFd], detached: true });
  ...
  try {
    ...
    if (semanticMode === 'menu-first-control') {
      const wanted = ['driver-disabled', 'player-input-ready', 'menu-open', 'terrain-open', 'terrain-closed'];
      const seen = new Set(), observations = [], inputs = [];
      let failure = null;
      ...
      const menuState = `(() => {
        const scene=cc.director.getScene();let battle=null,menu=null,terrain=null,say=null,win=null;
        (function visit(n){if(!n)return;for(const c of n._components||[]){const k=cc.js.getClassName(c);if(k==='BattleLayer')battle=c;else if(k==='MenuLayer'&&n.activeInHierarchy)menu=c;else if(k==='TerrainLayer'&&n.activeInHierarchy)terrain=c;else if(k==='SayLayer'&&n.activeInHierarchy)say=c;else if((k==='WinConditionsLayer'||k==='WinConBoxLayer')&&n.activeInHierarchy)win=c;}n.children.forEach(visit)})(scene);
        if(!battle)return {ready:false};
        const canvas=cc.game.canvas,rect=canvas.getBoundingClientRect(),vp=cc.view._viewportRect||{x:0,y:0};
        const toCss=w=>({x:rect.left+(vp.x+w.x*cc.view._scaleX)*rect.width/canvas.width,y:rect.top+(canvas.height-(vp.y+w.y*cc.view._scaleY))*rect.height/canvas.height});
        const geometry=n=>{if(!n||!n.activeInHierarchy)return null;const center=n.convertToWorldSpaceAR(cc.v2((.5-n.anchorX)*n.width,(.5-n.anchorY)*n.height));return {name:n.name,size:[n.width,n.height],center:[center.x,center.y],cssCenter:toCss(center),active:n.activeInHierarchy};};
        const button=(layer,name)=>{const b=layer&&layer.seekCompByName&&layer.seekCompByName(cc.Button,name);return b&&b.node?{...geometry(b.node),interactable:b.interactable!==false}:null;};
        const weatherBox=menu&&menu.seekNodeByName&&menu.seekNodeByName('bg/box2');
        const weatherReady=!!(weatherBox&&weatherBox.children&&weatherBox.children.some(n=>/^node\\d+$/.test(n.name)));
        const unit0=Object.values(battle._unitSet||{}).find(u=>u&&u.unit&&u.unit().id()===0);
        return {ready:true,frame:cc.director.getTotalFrames(),round:battle.round(),camp:battle.curCamp(),ctrlWaiting:!!battle._ctrlHelper,driverDisabled:!!globalThis.__jojoManualPlayerAction,driverSuppressed:globalThis.__jojoManualDriverSuppressed||0,menu:!!menu,terrain:!!terrain,dialogue:!!say,win:!!win,selectedUnit:battle.g_data&&battle.g_data.unit&&battle.g_data.unit.unit?battle.g_data.unit.unit().id():null,unit0:unit0&&{x:unit0.x(),y:unit0.y(),acted:unit0.isAction()},weatherReady,menuButton:button(battle,'menu_button'),terrainButton:button(menu,'bg/contain/button6'),menuCancel:button(menu,'Panel_cancel'),terrainClose:button(terrain,'bg/button2'),runtimeViewport:{inner:[innerWidth,innerHeight],canvasPixels:[canvas.width,canvas.height],canvasRect:[rect.left,rect.top,rect.width,rect.height],visible:[cc.view.getVisibleSize().width,cc.view.getVisibleSize().height],viewport:[vp.x,vp.y,vp.width,vp.height],scale:[cc.view._scaleX,cc.view._scaleY]}};
      })()`;
      async function current(label) { const state = await evaluate(menuState); observations.push({ label, wallSeconds: (Date.now() - started) / 1000, ...state }); return state; }
      async function waitState(label, predicate, limit) { return poll(async () => { const state = await current(label); return predicate(state) ? state : null; }, label, Date.now() + limit); }
      async function stableButton(label, field, predicate, limit = 5000) {
        let prior = null;
        return poll(async () => { const state = await current(label); if (!predicate(state)) return null; const button = state[field]; if (!button || !button.active || !button.interactable || !button.cssCenter) return null; const signature = JSON.stringify(button); const ready = prior && prior.frame !== state.frame && prior.signature === signature; prior = { frame: state.frame, signature }; return ready ? state : null; }, `${label} stable button`, Date.now() + limit);
      }
      async function snapshot(label) { const state = await current(`capture-${label}`); const image = await capturePng(client); const file = `source-${String(captures.length + 1).padStart(2, '0')}-${label}.png`; fs.writeFileSync(path.join(outputRoot, file), Buffer.from(image.data, 'base64')); captures.push({ label, file, wallSeconds: (Date.now() - started) / 1000, ...state }); return state; }
      ...
      try {
        await waitState('round2-script-before-player-handoff', s => s.round === 2 && s.camp === 3 && s.unit0?.x === 10 && s.unit0?.y === 5, 145000);
        const installed = await evaluate(`(() => {let battle=null;(function v(n){if(!n)return;for(const c of n._components||[])if(cc.js.getClassName(c)==='BattleLayer')battle=c;n.children.forEach(v)})(cc.director.getScene());if(!battle||!battle._menu_button||!battle._menu_button.node)return {ok:false};globalThis.__jojoManualPlayerAction=true;const suppress=(type,event)=>globalThis.__jojoManualPlayerAction&&battle.round()===2&&battle.curCamp()===0&&type===cc.Node.EventType.TOUCH_END&&(!event||typeof event.getLocation!=='function');const node=battle._menu_button.node;if(!node.__jojoManualOriginalEmit){node.__jojoManualOriginalEmit=node.emit;node.emit=function(type,event,...rest){if(suppress(type,event)){globalThis.__jojoManualDriverSuppressed=(globalThis.__jojoManualDriverSuppressed||0)+1;return;}return node.__jojoManualOriginalEmit.call(this,type,event,...rest);};}if(!globalThis.__jojoManualAllSyntheticTouchSuppressed){const original=cc.Node.prototype.emit;cc.Node.prototype.emit=function(type,event,...rest){if(suppress(type,event)){globalThis.__jojoManualDriverSuppressed=(globalThis.__jojoManualDriverSuppressed||0)+1;return;}return original.call(this,type,event,...rest);};globalThis.__jojoManualAllSyntheticTouchSuppressed=true;}return {ok:true,frame:cc.director.getTotalFrames()};})()`);
        if (!installed.ok) throw new Error('could not install harness-only automatic-menu suppression');
        seen.add('driver-disabled');
        let state = await waitState('player-input-ready', s => s.round === 2 && s.camp === 0 && s.ctrlWaiting && !s.win && !s.dialogue && !s.menu && !s.terrain && s.unit0?.x === 10 && s.unit0?.y === 5 && !s.unit0.acted, 20000);
        seen.add('player-input-ready'); await snapshot('player-input-ready');
        state = await stableButton('menu-button-layout', 'menuButton', s => s.round === 2 && s.camp === 0 && s.ctrlWaiting && !s.menu && !s.terrain);
        await click('open-battle-menu', state.menuButton.cssCenter, state);
        state = await waitState('menu-open', s => s.menu && !s.terrain && s.weatherReady && s.terrainButton?.interactable && s.terrainButton.active, 5000);
        seen.add('menu-open'); await snapshot('menu-open');
        state = await stableButton('terrain-button-layout', 'terrainButton', s => s.menu && s.weatherReady && !s.terrain);
        await click('open-terrain', state.terrainButton.cssCenter, state);
        state = await waitState('terrain-open', s => s.terrain && !s.menu && s.terrainClose?.interactable && s.terrainClose.active, 5000);
        seen.add('terrain-open'); await snapshot('terrain-open');
        state = await stableButton('terrain-close-layout', 'terrainClose', s => s.terrain && !s.menu);
        await click('close-terrain', state.terrainClose.cssCenter, state);
        state = await waitState('terrain-closed', s => !s.terrain && !s.menu && s.round === 2 && s.camp === 0 && s.ctrlWaiting && !s.dialogue && !s.win && s.selectedUnit == null && s.unit0?.x === 10 && s.unit0?.y === 5 && !s.unit0.acted, 5000);
        seen.add('terrain-closed'); await snapshot('terrain-closed');
      } catch (error) { failure = error.message; }
      const missingCaptures = wanted.filter(label => !seen.has(label));
      const finalState = await current('final-state').catch(() => null);
      fs.writeFileSync(path.join(outputRoot, 'screens.json'), JSON.stringify({ contract: 'source-yingchuan-normal-clock-menu-first-control-v1', evidenceKind: 'actual-source-renderer-direct-battle-bootstrap-with-cdp-pointer-input', sourceRoot, scenario: 'S_00', timeScale: 1, maxWallMs, semanticMode, complete: !failure && missingCaptures.length === 0, failure, missingCaptures, requestedAction: { round: 2, camp: 0, sequence: ['open-battle-menu', 'open-terrain', 'close-terrain'] }, automaticDriverHandoff: { mechanism: 'harness-only rejection of synthetic TOUCH_END during round-two player input; CDP touch events remain routed through Cocos input', installed: seen.has('driver-disabled'), suppressedCalls: finalState?.driverSuppressed }, inputs, observations, finalState, bootstrap: { route: 'HallLayer.jumpScene(0)', seededBattleUnits: [0], normalDialogueInput: 'SayLayer Panel_cancel TOUCH_END', fixture: false, fullCampaignEntry: false }, captures }, null, 2) + '\n');
      await childExit;
      if (!fs.existsSync(trace)) throw new Error('source menu-first-control trace was not flushed');
      const complete = !failure && missingCaptures.length === 0;
      console.log(`SOURCE_YINGCHUAN_MENU_FIRST_CONTROL_${complete ? 'OK' : 'PARTIAL'} ${captures.length}${failure ? ` failure=${failure}` : ''}`);
    ...
    if(['single-player-action','round2-followup','round2-first-combat','round3-player-action','round3-first-combat','round3-followup','round3-counterattack','round3-210'].includes(semanticMode)){
      const followup=semanticMode!=='single-player-action',round3Mode=['round3-player-action','round3-first-combat','round3-followup','round3-counterattack','round3-210'].includes(semanticMode),round3Speaker210=semanticMode==='round3-210',round3Counterattack=semanticMode==='round3-counterattack'||round3Speaker210,round3Followup=semanticMode==='round3-followup'||round3Counterattack,round3FirstCombat=semanticMode==='round3-first-combat'||round3Followup,continueFirstDialogue=semanticMode==='round2-first-combat'||round3Mode;
      const observations=[], inputs=[], wanted=['driver-disabled','unit0-selected','destination-reachable','unit0-moved','attack-command','enemy484-selected','attack-resolved','post-action-terminal',...(followup?['end-round-confirmed','next-camp-observed','followup-terminal']:[]),...(round3Mode?['round3-player-ready','round3-unit0-selected','round3-destination-reachable','round3-unit0-moved','round3-attack-command','round3-enemy483-selected','round3-attack-resolved','round3-post-action-terminal']:[]),...(round3FirstCombat?['round3-critical-dialogue-complete','round3-critical-dialogue-closed']:[]),...(round3Followup?['round3-followup-confirmed','round3-followup-terminal']:[]),...(round3Counterattack?['round3-counter-dialogue-complete','round3-counter-dialogue-closed']:[]),...(round3Speaker210?['round3-210-weather','round3-210-dialogue-complete','round3-210-dialogue-closed']:[])];
      const seen=new Set(); let failure=null,round3FollowupResult=null;
      ...
      const manualState=`(() => {
        ...
        if(!battle)return {ready:false}; const unit=id=>Object.values(battle._unitSet||{}).find(u=>u&&u.unit&&u.unit().id()===id),u0=unit(0),u483=unit(483),u484=unit(484);
        ...
        const tp=battle.turnPos(11,5),tileWorld=battle.map.node.convertToWorldSpaceAR(tp),reachable=!!(battle.g_data&&battle.g_data.psHash&&Object.prototype.hasOwnProperty.call(battle.g_data.psHash,(11<<8)|5)),tp3=battle.turnPos(10,5),tileWorld3=battle.map.node.convertToWorldSpaceAR(tp3),round3Reachable=!!(battle.g_data&&battle.g_data.psHash&&Object.prototype.hasOwnProperty.call(battle.g_data.psHash,(10<<8)|5));
        ...
        return {ready:true,frame:cc.director.getTotalFrames(),round:battle.round(),camp:battle.curCamp(),driverDisabled:!!globalThis.__jojoManualPlayerAction,driverSuppressed:globalThis.__jojoManualDriverSuppressed||0,ctrlWaiting:!!battle._ctrlHelper,menuPending:!!globalThis.__jojoManualMenuEmitWrapped,menu:!!menu,dialogue:!!say,dialogueSpeakerId,dialogueText:say&&say._curLab&&say._curLab.string,dialogueRemaining:say&&say._nextString,dialogueTypingActive:!!(say&&say._handle),dialoguePanel,win:!!win,fight:!!fight,settlementLayers,msgBox:!!msgBox,msgBoxText:msgText,msgBoxCancel:msgCancel,msgBoxConfirm:msgConfirm,aiActorStarts:(globalThis.__jojoManualAiActorStarts||[]).slice(),command:!!command,commandAttackInteractable:!!commandButton,commandGeometry:{root:command&&nodeGeometry(command.node),background:nodeGeometry(commandBg),attackButton:nodeGeometry(commandButton)},runtimeViewport:{inner:[innerWidth,innerHeight],devicePixelRatio,canvasPixels:[canvas.width,canvas.height],canvasRect:[rect.left,rect.top,rect.width,rect.height],visible:[cc.view.getVisibleSize().width,cc.view.getVisibleSize().height],viewport:[vp.x,vp.y,vp.width,vp.height],scale:[cc.view._scaleX,cc.view._scaleY]},reachable,round3Reachable,reachableKeys:battle.g_data&&battle.g_data.psHash?Object.keys(battle.g_data.psHash).map(Number):[],selectedUnit:battle.g_data&&battle.g_data.unit&&battle.g_data.unit.unit?battle.g_data.unit.unit().id():null,unit0:u0&&{x:u0.x(),y:u0.y(),hp:u0.hp_cur(),action:u0.unit().action(),acted:u0.isAction(),geometry:nodeGeometry(u0.node),point:unitPoint(u0)},enemy483:u483&&{x:u483.x(),y:u483.y(),hp:u483.hp_cur(),exists:u483.isExist(),visible:u483.visible(),geometry:nodeGeometry(u483.node),point:unitPoint(u483)},enemy484:u484&&{x:u484.x(),y:u484.y(),hp:u484.hp_cur(),exists:u484.isExist(),visible:u484.visible(),geometry:nodeGeometry(u484.node),point:unitPoint(u484)},destination:{x:11,y:5,local:[tp.x,tp.y],world:[tileWorld.x,tileWorld.y],point:toCss(tileWorld)},round3Destination:{x:10,y:5,local:[tp3.x,tp3.y],world:[tileWorld3.x,tileWorld3.y],point:toCss(tileWorld3)},commandPoint:buttonPoint,inputEvents:(globalThis.__jojoManualInputEvents||[]).slice()};
      })()`;
      let weatherCaptured=false;
      // 라운드 롤오버의 날씨 안내(MenuLayer switch_weather)는 어느 대기 루프에서 열릴지 모르므로
      // 공용 상태 조회 지점에서 한 번만 기회 포착한다. 입력과 기존 판정은 바꾸지 않는다.
      async function current(){const s=await evaluate(manualState);if(round3Speaker210&&!weatherCaptured&&s.menu&&captures.length<11){weatherCaptured=true;seen.add('round3-210-weather');await snapshot('round3-210-weather');}return s;}
      ...
      async function waitStablePoint(label,geometryOf,valid,limit=5000){let prior=null;return poll(async()=>{const s=await current();observations.push({label,wallSeconds:(Date.now()-started)/1000,...s});if(!valid(s))return null;const geometry=geometryOf(s),point=geometry&&(geometry.cssCenter||geometry.point);if(!geometry||!point)return null;const signature=JSON.stringify({...geometry,stablePoint:point});const stable=prior&&prior.frame!==s.frame&&prior.signature===signature;prior={frame:s.frame,signature};return stable?s:null;},`${label} stable geometry`,Date.now()+limit);}
      async function click(label,point,before){inputs.push({label,requestedFrame:before.frame,wallSeconds:(Date.now()-started)/1000,point,before});await cdpClick(client,point);}
      try{
        ...
        let s=await waitState('manual-player-input-ready',x=>x.round===2&&x.camp===0&&x.ctrlWaiting&&!x.win&&!x.dialogue&&!x.menu&&x.unit0?.x===10&&x.unit0?.y===5&&x.enemy484?.x===10&&x.enemy484?.y===6,20000);if(!round3Mode)await snapshot('manual-input-ready');
        ...
        if(!s.reachable)throw Error(`declared destination 11,5 is not reachable: ${JSON.stringify(s.reachableKeys)}`);seen.add('destination-reachable');if(!round3Mode)await snapshot('unit0-selected-reachable');
        await click('select-destination-11-5',s.destination.point,s);s=await waitState('unit0-moved-command-open',x=>x.unit0?.x===11&&x.unit0?.y===5&&x.command&&x.commandAttackInteractable,12000);seen.add('unit0-moved');seen.add('attack-command');if(!round3Mode)await snapshot('unit0-moved-command');
        ...
        await click('select-enemy-484',s.enemy484.point,s);seen.add('enemy484-selected');if(!round3Mode)await snapshot('enemy484-selected');
        ...
        seen.add('post-action-terminal');if(!round3Mode)await snapshot(s.msgBox?'end-round-confirm-terminal':'player-input-terminal');
        if(followup){
          ...
          if(!round3Mode)await snapshot('next-camp-observed');let movementCaptured=false,attackCaptured=false,settlementCaptured=false,terminal=null,dialogueAdvanced=false;
          while(Date.now()-started<maxWallMs-1000&&captures.length<12){
            ...
            if(!movementCaptured&&moving.length){movementCaptured=true;if(!round3Mode)await snapshot('next-camp-movement');}
            if(!attackCaptured&&attacking.length){attackCaptured=true;if(!round3Mode)await snapshot('next-camp-attack');}
            if(!settlementCaptured&&settlement.length){settlementCaptured=true;if(!round3Mode)await snapshot('next-camp-settlement');}
            if(round3Mode&&state.round===3&&state.camp===0&&detail.ctrlWaiting&&!state.dialogue&&!detail.msgBox&&!detail.menu){terminal={kind:'round3-player-ready',frame:state.frame};seen.add('round3-player-ready');if(!round3Followup)await snapshot('round3-player-ready');break;}
            if(state.dialogue){
          ...
          if(round3Mode&&terminal.kind!=='round3-player-ready')throw Error(`round3 player input was blocked before readiness: ${JSON.stringify(terminal)}`);
          ...
          if(round3Mode){
            s=await current();if(s.unit0?.x!==11||s.unit0?.y!==5||s.enemy483?.x!==9||s.enemy483?.y!==6||s.enemy483.hp!==19)throw Error(`round3 action precondition changed: ${JSON.stringify(s)}`);
            s=await waitStablePoint('round3-unit0-layout',x=>x.unit0&&x.unit0.geometry,x=>x.round===3&&x.camp===0&&x.ctrlWaiting&&!x.dialogue&&!x.msgBox&&x.unit0?.x===11&&x.unit0?.y===5);await click('round3-select-unit-0',s.unit0.geometry.cssCenter,s);s=await waitState('round3-unit0-selected',x=>x.round===3&&x.selectedUnit===0&&x.ctrlWaiting,5000);seen.add('round3-unit0-selected');
            if(!s.round3Reachable)throw Error(`round3 destination 10,5 is not reachable: ${JSON.stringify(s.reachableKeys)}`);seen.add('round3-destination-reachable');if(!round3Followup)await snapshot('round3-unit0-selected-reachable');s=await waitStablePoint('round3-destination-layout',x=>x.round3Destination,x=>x.selectedUnit===0&&x.ctrlWaiting&&x.round3Reachable);
            await click('round3-select-destination-10-5',s.round3Destination.point,s);s=await waitState('round3-unit0-moved-command-open',x=>x.unit0?.x===10&&x.unit0?.y===5&&x.command&&x.commandAttackInteractable,12000);seen.add('round3-unit0-moved');seen.add('round3-attack-command');if(!round3Followup)await snapshot('round3-unit0-moved-command');s=await waitStablePoint('round3-attack-command-layout',x=>x.commandGeometry.attackButton,x=>x.command&&x.commandAttackInteractable&&x.unit0?.x===10&&x.unit0?.y===5);
            await click('round3-select-attack-command',s.commandGeometry.attackButton.cssCenter,s);s=await waitState('round3-attack-target-ready',x=>!x.command&&x.ctrlWaiting&&x.enemy483?.exists,5000);s=await waitStablePoint('round3-enemy483-layout',x=>x.enemy483&&x.enemy483.geometry,x=>!x.command&&x.ctrlWaiting&&x.enemy483?.exists&&x.enemy483?.x===9&&x.enemy483?.y===6);
            const beforeHp=s.enemy483.hp;await click('round3-select-enemy-483',s.enemy483.geometry.cssCenter,s);seen.add('round3-enemy483-selected');if(!round3Followup)await snapshot('round3-enemy483-selected');
            let damageObserved=false,round3SettlementCaptured=false,round3AttackCaptured=false,round3CriticalAdvanced=false,round3Terminal=null;
            while(Date.now()-started<maxWallMs-1000&&captures.length<12){
              const detail=await current(),full=await evaluate(stateExpression),settlement=detail.settlementLayers.length>0;
              observations.push({label:'round3-attack-followup',wallSeconds:(Date.now()-started)/1000,frame:detail.frame,dialogue:detail.dialogue,speakerId:detail.dialogueSpeakerId,text:detail.dialogueText,enemy483:detail.enemy483,settlementLayers:detail.settlementLayers,infoPanels:full.infoPanels});
              const round3Attacking=full.units.filter(u=>u.id===0&&u.animation&&/^anime(?:21|49)_/.test(u.animation));
              if(!round3AttackCaptured&&round3Attacking.length){round3AttackCaptured=true;if(!round3Counterattack)await snapshot('round3-critical-attack');}
              if(!damageObserved&&(!detail.enemy483?.exists||detail.enemy483.hp!==beforeHp)){damageObserved=true;if(!round3Counterattack)await snapshot('round3-attack-damage-observed');}
              if(settlement&&!round3SettlementCaptured){round3SettlementCaptured=true;if(!round3Counterattack)await snapshot('round3-settlement');}
              if(detail.dialogue){
                if(!round3FirstCombat||round3CriticalAdvanced){round3Terminal={kind:round3CriticalAdvanced?'blocking-subsequent-dialogue':'blocking-dialogue',frame:detail.frame,speakerId:detail.dialogueSpeakerId,text:detail.dialogueText,damageObserved,settlementCaptured:round3SettlementCaptured};await snapshot('round3-blocking-dialogue');break;}
                const expected='내 이 기술을 받아라! 이것이 바로 황천지검이다!';
                if(detail.dialogueSpeakerId!=='0')throw Error(`unexpected round3 critical dialogue speaker: ${JSON.stringify(detail)}`);
                if(detail.dialogueRemaining===''&&!detail.dialogueTypingActive&&detail.dialogueText!==expected)throw Error(`completed round3 critical dialogue text changed: ${JSON.stringify(detail)}`);
                if(detail.dialogueText!==expected||detail.dialogueRemaining!==''||detail.dialogueTypingActive||!detail.dialoguePanel){await delay(16);continue;}
                seen.add('round3-critical-dialogue-complete');if(!round3Counterattack)await snapshot('round3-critical-dialogue-complete');const fresh=await waitStablePoint('round3-critical-dialogue-layout',x=>x.dialoguePanel,x=>x.dialogue&&x.dialogueSpeakerId==='0'&&x.dialogueText===expected&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialoguePanel);await click('close-round3-critical-dialogue',fresh.dialoguePanel.cssCenter,fresh);round3CriticalAdvanced=true;seen.add('round3-critical-dialogue-closed');await waitState('round3-critical-dialogue-closed',x=>!x.dialogue,5000);continue;
              }
              if((!round3FirstCombat||round3CriticalAdvanced)&&detail.unit0?.acted===true&&!detail.fight&&!settlement&&(detail.ctrlWaiting||detail.msgBox)){if(detail.msgBox&&(detail.msgBoxText!=='모든 부대의 명령을 종료하시겠습니까?'||!detail.msgBoxConfirm))throw Error(`unexpected round3 post-action MsgBox: ${JSON.stringify(detail)}`);seen.add('round3-attack-resolved');round3Terminal={kind:detail.msgBox?'end-round-confirm':'player-input',frame:detail.frame,unit0Acted:detail.unit0.acted,damageObserved,settlementCaptured:round3SettlementCaptured};if(!round3Counterattack)await snapshot(detail.msgBox?'round3-end-round-confirm-terminal':'round3-player-input-terminal');break;}
              await delay(16);
            }
            if(!round3Terminal)throw Error(`round3 post-action terminal not observed: ${JSON.stringify({damageObserved,round3SettlementCaptured})}`);
            seen.add('round3-post-action-terminal');observations.push({label:'round3-post-action-terminal',...round3Terminal,criticalDialogueAdvanced:round3CriticalAdvanced,attackAnimationCaptured:round3AttackCaptured});
            if(round3Followup){
              if(round3Terminal.kind!=='end-round-confirm')throw Error(`round3 followup requires authored automatic end-round prompt: ${JSON.stringify(round3Terminal)}`);
              s=await waitStablePoint('round3-automatic-end-round-confirm-layout',x=>x.msgBoxConfirm,x=>x.msgBox&&x.msgBoxText==='모든 부대의 명령을 종료하시겠습니까?'&&!!x.msgBoxConfirm);const round3PlayerCamp=s.camp,round3AiStartOffset=s.aiActorStarts.length;await click('round3-confirm-automatic-end-round',s.msgBoxConfirm.cssCenter,s);seen.add('round3-followup-confirmed');
              s=await waitState('round3-followup-next-camp',x=>!x.msgBox&&x.camp!==round3PlayerCamp,10000);
              let followupMovement=false,followupAttack=false,followupHit=false,followupSettlement=false,counterMovement=false,counterAttack=false,counterHit=false,counterSettlement=false,counterDialogueAdvanced=false,counterAiStartOffset=null,counterActorStart=null,speaker210Movement=false,speaker210Attack=false,speaker210Hit=false,speaker210Settlement=false,speaker210DialogueAdvanced=false,speaker210AiStartOffset=null,speaker210ActorStart=null,round3FollowupTerminal=null;
              while(Date.now()-started<maxWallMs-1000&&captures.length<12){
                const detail=await current(),full=await evaluate(stateExpression),newAiActorStarts=detail.aiActorStarts.slice(round3AiStartOffset),moving=full.units.filter(u=>u.animation&&u.animation.startsWith('anime20')),attacking=full.units.filter(u=>u.animation&&/^anime(?:21|25|48|49)_/.test(u.animation)),hit=full.units.filter(u=>u.animation&&/^anime32_/.test(u.animation)),settlement=detail.settlementLayers.length>0;
                observations.push({label:'round3-followup',wallSeconds:(Date.now()-started)/1000,frame:detail.frame,camp:detail.camp,dialogue:detail.dialogue,speakerId:detail.dialogueSpeakerId,text:detail.dialogueText,dialogueRemaining:detail.dialogueRemaining,dialogueTypingActive:detail.dialogueTypingActive,movingActorIds:moving.map(u=>u.id),attackingActorIds:attacking.map(u=>u.id),hitActorIds:hit.map(u=>u.id),settlementLayers:detail.settlementLayers,aiActorStartOffset:round3AiStartOffset,newAiActorStarts,battleTargets:full.battleTargets,units:full.units});
                const activeOffset=round3Speaker210&&speaker210DialogueAdvanced?speaker210AiStartOffset:round3Counterattack?counterAiStartOffset:round3AiStartOffset,activeActorStart=round3Speaker210&&speaker210DialogueAdvanced?speaker210ActorStart:round3Counterattack?counterActorStart:null,advanceGate=round3Speaker210?speaker210DialogueAdvanced:round3Counterattack?counterDialogueAdvanced:true;
                if(!followupMovement&&moving.length){followupMovement=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-followup-movement');}
                if(!followupAttack&&attacking.length){followupAttack=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-followup-attack');}
                if(!followupHit&&hit.length){followupHit=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-followup-hit');}
                if(!followupSettlement&&settlement){followupSettlement=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-followup-settlement');}
                if(counterDialogueAdvanced){
                  if(!counterMovement&&moving.length){counterMovement=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-counter-movement');}
                  if(!counterAttack&&attacking.length){counterAttack=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-counter-attack');}
                  if(!counterHit&&hit.length){counterHit=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-counter-hit');}
                  if(!counterSettlement&&settlement){counterSettlement=true;if(!round3Speaker210&&captures.length<11)await snapshot('round3-counter-settlement');}
                }
                if(speaker210DialogueAdvanced){
                  if(!speaker210Movement&&moving.length){speaker210Movement=true;if(captures.length<11)await snapshot('round3-210-movement');}
                  if(!speaker210Attack&&attacking.length){speaker210Attack=true;if(captures.length<11)await snapshot('round3-210-attack');}
                  if(!speaker210Hit&&hit.length){speaker210Hit=true;if(captures.length<11)await snapshot('round3-210-hit');}
                  if(!speaker210Settlement&&settlement){speaker210Settlement=true;if(captures.length<11)await snapshot('round3-210-settlement');}
                }
                if(detail.dialogue){
                  if(round3Counterattack&&!counterDialogueAdvanced){
                    const expected='후우후……!';if(detail.dialogueSpeakerId!=='146')throw Error(`unexpected round3 counter dialogue speaker: ${JSON.stringify(detail)}`);
                    if(detail.dialogueRemaining===''&&!detail.dialogueTypingActive&&detail.dialogueText!==expected)throw Error(`completed round3 counter dialogue text changed: ${JSON.stringify(detail)}`);
                    if(detail.dialogueText!==expected||detail.dialogueRemaining!==''||detail.dialogueTypingActive||!detail.dialoguePanel){await delay(16);continue;}
                    counterAiStartOffset=detail.aiActorStarts.length;counterActorStart=detail.aiActorStarts[counterAiStartOffset-1]||null;if(!counterActorStart)throw Error(`round3 counter dialogue has no observed current AI actor: ${JSON.stringify(detail.aiActorStarts)}`);seen.add('round3-counter-dialogue-complete');if(!round3Speaker210&&captures.length<11)await snapshot('round3-counter-dialogue-complete');const fresh=await waitStablePoint('round3-counter-dialogue-layout',x=>x.dialoguePanel,x=>x.dialogue&&x.dialogueSpeakerId==='146'&&x.dialogueText===expected&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialoguePanel);await click('close-round3-counter-dialogue',fresh.dialoguePanel.cssCenter,fresh);counterDialogueAdvanced=true;seen.add('round3-counter-dialogue-closed');await waitState('round3-counter-dialogue-closed',x=>!x.dialogue,5000);continue;
                  }
                  if(round3Speaker210&&!speaker210DialogueAdvanced){
                    const expected210='하아……!';if(detail.dialogueSpeakerId!=='210')throw Error(`unexpected round3 speaker-210 dialogue speaker: ${JSON.stringify(detail)}`);
                    if(detail.dialogueRemaining===''&&!detail.dialogueTypingActive&&detail.dialogueText!==expected210)throw Error(`completed round3 speaker-210 dialogue text changed: ${JSON.stringify(detail)}`);
                    if(detail.dialogueText!==expected210||detail.dialogueRemaining!==''||detail.dialogueTypingActive||!detail.dialoguePanel){await delay(16);continue;}
                    speaker210AiStartOffset=detail.aiActorStarts.length;speaker210ActorStart=detail.aiActorStarts[speaker210AiStartOffset-1]||null;if(!speaker210ActorStart)throw Error(`round3 speaker-210 dialogue has no observed current AI actor: ${JSON.stringify(detail.aiActorStarts)}`);seen.add('round3-210-dialogue-complete');if(captures.length<11)await snapshot('round3-210-dialogue-complete');const fresh210=await waitStablePoint('round3-210-dialogue-layout',x=>x.dialoguePanel,x=>x.dialogue&&x.dialogueSpeakerId==='210'&&x.dialogueText===expected210&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialoguePanel);await click('close-round3-210-dialogue',fresh210.dialoguePanel.cssCenter,fresh210);speaker210DialogueAdvanced=true;seen.add('round3-210-dialogue-closed');await waitState('round3-210-dialogue-closed',x=>!x.dialogue,5000);continue;
                  }
                  const completed=await waitState('round3-followup-blocking-dialogue-complete',x=>x.dialogue&&x.dialogueRemaining===''&&!x.dialogueTypingActive&&!!x.dialogueText,10000);round3FollowupTerminal={kind:'blocking-dialogue',frame:completed.frame,camp:completed.camp,speakerId:completed.dialogueSpeakerId,text:completed.dialogueText,dialogueComplete:true,aiActorStartOffset:activeOffset,currentActorStart:activeActorStart,newAiActorStarts:completed.aiActorStarts.slice(activeOffset)};if(captures.length<12)await snapshot('round3-followup-blocking-dialogue-complete');break;
                }
                const completionOffset=activeOffset,completionStarts=completionOffset==null?[]:detail.aiActorStarts.slice(completionOffset),completedActor=round3Counterattack?activeActorStart:newAiActorStarts[0],nextActorStart=round3Counterattack?completionStarts[0]:completionStarts[1];
                if(advanceGate&&nextActorStart){round3FollowupTerminal={kind:'next-ai-actor-started',frame:detail.frame,camp:detail.camp,completedActorId:completedActor&&completedActor.actorId,nextActorId:nextActorStart.actorId,aiActorStartOffset:completionOffset,currentActorStart:activeActorStart,newAiActorStarts:completionStarts};if(captures.length<12)await snapshot('round3-followup-first-action-complete');break;}
                if(advanceGate&&completedActor&&detail.camp!==completedActor.camp){round3FollowupTerminal={kind:'camp-transition-after-first-ai-action',frame:detail.frame,camp:detail.camp,completedActorId:completedActor.actorId,aiActorStartOffset:completionOffset,currentActorStart:activeActorStart,newAiActorStarts:completionStarts};if(captures.length<12)await snapshot('round3-followup-first-action-complete');break;}
                await delay(16);
              }
              if(!round3FollowupTerminal)throw Error(`round3 followup terminal not observed: ${JSON.stringify({followupMovement,followupAttack,followupHit,followupSettlement})}`);
              round3FollowupResult={...round3FollowupTerminal,terminalReached:true,actionCompleted:round3FollowupTerminal.kind!=='blocking-dialogue',movementCaptured:followupMovement,attackCaptured:followupAttack,hitCaptured:followupHit,settlementCaptured:followupSettlement,counterDialogueAdvanced,counterActorStart,counterMovementCaptured:counterMovement,counterAttackCaptured:counterAttack,counterHitCaptured:counterHit,counterSettlementCaptured:counterSettlement,speaker210DialogueAdvanced,speaker210ActorStart,speaker210AiStartOffset,speaker210MovementCaptured:speaker210Movement,speaker210AttackCaptured:speaker210Attack,speaker210HitCaptured:speaker210Hit,speaker210SettlementCaptured:speaker210Settlement};
              seen.add('round3-followup-terminal');observations.push({label:'round3-followup-terminal',...round3FollowupTerminal,movementCaptured:followupMovement,attackCaptured:followupAttack,hitCaptured:followupHit,settlementCaptured:followupSettlement});
            }
          }
        }
      }catch(error){failure=error.message;}
      const missingCaptures=wanted.filter(name=>!seen.has(name));const finalState=await current().catch(()=>null);
      fs.writeFileSync(path.join(outputRoot,'screens.json'),JSON.stringify({contract:round3Speaker210?'source-yingchuan-normal-clock-round3-210-v1':round3Counterattack?'source-yingchuan-normal-clock-round3-counterattack-v1':round3Followup?'source-yingchuan-normal-clock-round3-followup-v1':round3FirstCombat?'source-yingchuan-normal-clock-round3-first-combat-v1':round3Mode?'source-yingchuan-normal-clock-round3-player-action-v1':continueFirstDialogue?'source-yingchuan-normal-clock-round2-first-combat-v1':followup?'source-yingchuan-normal-clock-round2-followup-v1':'source-yingchuan-normal-clock-single-player-action-v1',evidenceKind:'actual-source-renderer-direct-battle-bootstrap-with-cdp-pointer-input',sourceRoot,scenario:'S_00',timeScale:1,maxWallMs,semanticMode,complete:!failure&&missingCaptures.length===0,failure,missingCaptures,requestedAction:{unitId:0,from:[10,5],destination:[11,5],command:'attack',targetId:484,target:[10,6]},round3RequestedAction:round3Mode?{unitId:0,from:[11,5],destination:[10,5],command:'attack',targetId:483,target:[9,6],damageOptional:true,completionEvidence:'unit0-isAction-and-no-fight-or-settlement-at-control-or-authored-prompt',criticalDialogueAdvance:round3FirstCombat?{speakerId:'0',text:'내 이 기술을 받아라! 이것이 바로 황천지검이다!',input:'actual-CDP-pointer-after-natural-completion',requiredForCompletion:true}:false}:null,round3Followup:round3Followup?{automaticEndRoundConfirm:'actual-CDP-pointer-button0-no-entrust',additionalDialogueInputs:round3Speaker210?2:round3Counterattack?1:0,counterDialogueAdvance:round3Counterattack?{speakerId:'146',text:'후우후……!',input:'actual-CDP-pointer-after-natural-completion',requiredForCompletion:true}:false,speaker210DialogueAdvance:round3Speaker210?{speakerId:'210',text:'하아……!',input:'actual-CDP-pointer-after-natural-completion',requiredForCompletion:true,completionEvidence:'actor-210-ai-action-settlement-then-next-actor-start-or-camp-transition'}:false,terminal:round3FollowupResult}:null,followup:{enabled:followup,automaticEndRoundConfirm:followup?'actual-CDP-pointer-button0-no-entrust':false,firstDialogueAdvance:continueFirstDialogue?{speakerId:'32',text:'이것은 만민의 분노입니다!',input:'actual-CDP-pointer-after-natural-completion'}:false,completionEvidence:round3Mode?'natural-round3-camp0-control-ready':'next-ai-actor-start-or-camp-transition',expectedDamage:null,expectedNextActor:null},automaticDriverHandoff:{mechanism:'harness-only rejection of full-trace synthetic menu_button node.emit; CDP touch events remain routed through Cocos input',installed:seen.has('driver-disabled'),suppressedCalls:finalState&&finalState.driverSuppressed},inputs,observations,finalState,bootstrap:{route:'HallLayer.jumpScene(0)',seededBattleUnits:[0],normalDialogueInput:'SayLayer Panel_cancel TOUCH_END',fixture:false,fullCampaignEntry:false},captures},null,2)+'\n');
