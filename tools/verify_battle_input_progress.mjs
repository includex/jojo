#!/usr/bin/env node
/** Smoke-test the production BattleLayer input paths using its real runtime.
 *
 * The shared production-input driver dispatches through the BattleLayer's
...
const battleLayer = resolve(root, "core/src/main/kotlin/com/jojo/port/BattleLayer.kt");
...
for (const contract of [
  ...
  "endTurn()",
  "touchDown",
]) assert.ok(source.includes(contract), `missing BattleLayer input contract: ${contract}`);
...
if (result.status !== 0) throw new Error(`port runner failed (${result.status})\n${result.stdout}\n${result.stderr}`);
assert.ok(existsSync(trace), "port did not write input progress trace");
