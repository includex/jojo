...
원본 Cocos 게임과 LibGDX 게임의 전투 표현 차이를 조사하면서 발견한
`상태를 먼저 확정한 뒤 애니메이션만 뒤늦게 재생하는 문제`를 기록하고,
같은 방식으로 게임 개발이 누락되거나 테스트가 실제 플레이 경로를 우회하는 사례가
...
- `BattleScreen`, `ScenarioScreen`, `ScenarioBatchVerificationScreen`
...
게임의 AI 처리에서는 `Battle.resolveAiTurn()`이 한 유닛의 이동, 공격, HP 변화,
사망 및 제거 결과를 먼저 모델에 반영한다. 그 후 `BattleScreen`가 반환된 결과를
...
실제 영천전투 게임 로그에서 캐릭터 210의 이동은 다음과 같이 관찰됐다.
...
원본은 `BattleScreen._ai2`와 `BattleUnit.move2`의 콜백을 통해 다음 단계로
진행한다.
...
게임는 결과 계산과 화면 표현을 분리하면서 이 콜백 경계를 보존하지 못했다.
단순히 단계 이름과 대기시간을 추가하는 방식으로는 해결되지 않으며, 모델
mutation 자체를 원본 콜백 지점으로 옮겨야 한다.

## 2. 같은 유형으로 확인된 다른 전투 게임 개발 누락
...
실제 `BattleScreen`의 전투 실행 경로에는 연결되지 않았다.
...
`BattleScreen`는 `BattleAttackSequence.selectAttackAction()`처럼 일부 정적 선택
함수만 재사용하고, 실제 공격/피격/완료 콜백 시퀀스는 별도로 다시 구현한다.
따라서 이 클래스들의 단위 테스트가 통과해도 실제 전투의 공격, 반격, 연속 공격,
마법, 퇴각 및 사망 순서는 검증되지 않는다.

해결 후에는 `BattleScreen`의 실제 AI 경로가 이동 완료, authored `hit`, 피격 완료,
...
공격 action 선택만 남겼다. `BattleSceneCoordinator`는 전술 런타임 구현이 아니라 원본
...
전투 control 호환 기반 모델, `ControlPathPlanner`, `ControlResumeTerrain`,
...
확인됨. 대상:
`core/src/main/kotlin/com/jojo/game/ScenarioBatchVerificationScreen.kt`
...
확인됨. 대상: `core/src/main/kotlin/com/jojo/game/JojoGame.kt`
...
`BattleScreen`에는 capture/fixture/verify 관련 참조가 약 280곳,
`ScenarioScreen`에는 약 105곳 있다. 주요 예는 다음과 같다.
...
`exgameScenarioChoiceReference`는 원본의 합성 RGBA 프레임을 게임 자산으로
...
- stage-layer 호환 모델 (삭제: 실제 `ScenarioStage` 구현과 중복)
- `WinConditionRouteFlow` (삭제: 실제 `BattleScreen` 경로와 중복)
...
일부 기능은 `BattleScreen` 또는 `ScenarioScreen` 내부에 별도로 다시 구현되어
있을 수 있다. 그러므로 위 목록은 모두 고장났다는 판정이 아니라, 해당 클래스의
테스트를 실게임 구현 완료의 증거로 사용할 수 없다는 판정이다.

네트워크 `registerCheck`와 hot update 자체는 게임 정책상 지원하지 않는다.
...
자체는 실제 Title/Battle Setting 인스턴스에 연결되어 있다. reset-layer 재구성은
...
`tools/verify_battle_presentation_paths.py`는 `BattleScreen.kt`에 단계 이름이나
...
5. 테스트 전용 게임 클래스와 실제 화면 구현의 중복

### P2 — 시각적 완료로 오인될 위험

1. 원본 합성 framebuffer를 게임 자산으로 사용
...
실게임과 연결되지 않은 `*Sequence`, `*Game`, UI 모델을 연결하거나 제거하고,
`BattleScreen` 내부의 중복 구현을 없앤다.
...
원본과 게임 양쪽에서 다음 정보를 동일 형식으로 기록한다.
...
2. 정상 플레이 전체 전환 trace를 reference/game 쌍으로 비교
...
다음 조건을 모두 만족하기 전에는 게임 개발 완료로 판정하지 않는다.
...
- 마지막 단계의 기준 구현/현재 게임 스크린샷 비교 통과
...
- `ProgressionLayerTraceHarness`는 고정 예상 스냅샷 출력 대신
  achievements, sign-in, raffle, reset, register layer 재구성을 fixture 이벤트로 직접 호출한다.
...
- `BattleBootstrapTraceHarness`는 상수와 레지스트리 출력을 제거하고 실제
  `BattleSceneCoordinator`의 resource/save/forces API를 호출한다. 상수 127개와 레이어
...
- `verifyRuntimeTestIntegrity`를 기본 `core:test`에 연결했다. 이 게이트는
  ...
  제거했다. 전투 흐름 정리 과정에서는 control 호환 기반 모델,
...
Head 3건과 `BattleSceneCoordinator` 동작 1건에서 통과했다. source inventory 감사는
...
일반 화면 진입과 분리된 `BattleSceneCoordinator` 1개만
`isolatedSourceContracts`로 남아 있고, 이는 게임 개발 완료 증거로 계산하지 않는다.

### 11.1 최종 교차 감사

과거 `NaturalCampaignTransitionTest`는 `ScenarioScreen`/`BattleScreen`를
...
`BattleScreen`를 끝까지 실행한다. 단, 자연스러운 Title→R_00 화면 진입이 아니라
결정적 roster bootstrap을 사용하는 S_00 직접 실행이므로 그 범위를 설명에
명시했다. 실행 직전 이전 trace를 삭제하여 새 trace가 생성되지 않았을 때 과거
결과로 통과할 수 없게 했고, verifier는 trace의 `engine=jojo-game`, 프레임,
...
- 설정의 자동 닫힘 비트를 실제 `ScenarioInterpreter` 업데이트에 전달한다.
...
- 원본 `SayLayer` snapshot의 node transform에 맞춰 패널 하단을 428, 초상화
  하단을 426 논리 좌표로 수정했다. 기존 게임 값은 각각 332/330으로 정확히
  96 논리 단위 아래에 있었다.
...
- `BattleSceneCoordinator` 1개만 원본 Cocos `UIScene` 컨테이너의 독립 reference contract로
  격리되어 있다. LibGDX 일반 실행에서는 `JojoGame`/`BattleScreen`가 그 엔진
  경계를 대체하므로 전술 runtime 완료 증거로 계산하지 않는다.
- 영천 전체 전투 검증은 `DesktopLauncher -> JojoGame -> BattleScreen`를 실제로
  실행하지만 Title부터 들어가는 E2E가 아니라 결정적 S_00 roster bootstrap이다.
  이전 trace 삭제, `engine=jojo-game`, 실제 콜백 단계 및 승패 결과 검증으로
  stale/강제승리 우회를 차단했다.
...
`DEVELOPMENT_HANDOFF_2026-09-04.md` section 6.0을 따른다.

검증 안정화를 위해 원본 Electron verify mode에만 `backgroundThrottling=false`를
적용했고 actor-state runner에 Electron/Gradle timeout을 추가했다. 게임 LWJGL 자동 실행은
macOS null primary monitor 경계를 피하도록 `(0,0)` 위치를 명시하되 일반 실행의 중앙
정렬은 유지한다.

## 14. 2026-09-05 S_22 6라운드 최초 산술 차이의 확정 원인

원본과 게임의 캠 종료 로그를 비교했을 때 처음 남은 전술 차이는 6라운드 적군 턴의
217→5 공격이었다. 원본은 유닛 5의 HP를 `34→24`, AI 값을 10으로 기록했지만 게임는
...
게임는 당시 유닛 217을 계속 레벨 1, ATT 42, CRI 32로 유지해 기초 피해 16·명중률 73·
최종 피해/AI 값 9를 만들었다. 원본은 `BattleScreen._attack2` 완료 시 일반 공격, 반격,
연속 공격의 공격자에게 진영과 무관하게 `count_exp` 결과를 누적하고 `_jiesuan`에서
`unitAddExp`와 파생 능력치 갱신을 수행한다. 게임는 적 격파 시 플레이어 승자에게만
...
- 캠 비교 기준: `build/reports/s22-source-v12-game-v29-camp-boundaries.json`
