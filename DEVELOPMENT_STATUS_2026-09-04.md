# 조조전 원본 충실 게임 개발 — 개발 에이전트 최신 인수인계

> **2026-09-05 이후에는 `DEVELOPMENT_STATUS.md`를 먼저 읽는다.** 이 문서는
...
아직 완료되지 않은 가장 중요한 항목은 `physicalPasses`를 `BattleScreen`의 실제 시간축에
...
이 게임 개발은 **아직 100% 완료되지 않았다.** 현재 상태를 완료라고 선언하거나 기존의
...
우회하지 말고 원본 `SayLayer._resetPos()`와 카메라 동작을 게임에 옮겨야 한다.
...
기존 문서 `DEVELOPMENT_HANDOFF_2026-09-04.md`에는 유용한 상세 기록이 많지만, 상단의 과거
green 상태는 현재 fresh gate 추가 전 결과다. **상태 판단은 이 문서를 우선한다.**

## 2. 사용자가 요구한 완료 기준

- 원본 JS/Python 실행이 유일한 동작 권위다. 게임 코드와 게임 테스트가 서로 일치해도
  원본과 다르면 둘 다 수정한다.
- 화면 캡처는 논리 전환을 모두 맞춘 마지막 단계에서 사용한다.
- 그 전에는 기준 구현/현재 게임 양쪽에서 좌표, 이미지/action ID, 프레임 또는 simulation timestamp,
...
- 전체 게임 완료를 선언하려면 주요 한 전투만이 아니라 전 시나리오 route를 production
...
- 원본을 수정해도 된다는 사용자 허가가 있다. 단, 원본 gameplay 의미를 게임에 맞게
...
- `core/src/main/kotlin/com/jojo/game/GamePreferences.kt` (신규)
- `core/src/main/kotlin/com/jojo/game/JojoGame.kt`
- `core/src/main/kotlin/com/jojo/game/CampaignStore.kt`
- `core/src/main/kotlin/com/jojo/game/TitleScreen.kt`
- `core/src/main/kotlin/com/jojo/game/ScenarioScreen.kt`
- `desktop/src/main/kotlin/com/jojo/game/desktop/DesktopLauncher.kt`
- `core/src/test/kotlin/com/jojo/game/GamePreferenceProviderTest.kt` (신규)
...
- `core/src/main/kotlin/com/jojo/game/ScenarioRuntime.kt`
- `core/src/main/kotlin/com/jojo/game/CampaignStore.kt`
- `core/src/main/kotlin/com/jojo/game/ScenarioScreen.kt`
- `core/src/test/kotlin/com/jojo/game/ScenarioRuntimeTest.kt`
...
- `core/src/main/kotlin/com/jojo/game/BattlePreparationScreen.kt`
- `desktop/src/main/kotlin/com/jojo/game/desktop/DesktopLauncher.kt`
...
- 영천 대화 step 1, 2, 3 reference/game PNG
...
./gradlew --no-daemon -Pkotlin.incremental=false \
  ...
  --tests com.jojo.game.GamePreferenceProviderTest \
...
구현한다. 허용 오차나 expected PNG를 게임에 맞춰 바꾸지 않는다.

수정 직후 빠른 검증:

```bash
./gradlew --no-daemon -Pkotlin.incremental=false \
  :core:test --tests com.jojo.game.BattleScreenTest \
  :desktop:compileKotlin
./gradlew --no-daemon -Pkotlin.incremental=false \
  :desktop:verifyFreshBattleRenderParity --rerun-tasks
```
...
각 에이전트는 서로 다른 파일을 맡기고 같은 `BattleScreen.kt` 동시 편집은 피한다.
...
각 단계의 reference/game artifact가 **현재 run-id에서 fresh 생성됐는지** 확인한다.
...
경계를 먼저 trace하고 게임도 같은 경계에서 상태를 바꿔야 한다.
...
- 전투 렌더/입력/카메라/animation: `core/src/main/kotlin/com/jojo/game/BattleScreen.kt`
- 전투 aggregate와 규칙 조립: `core/src/main/kotlin/com/jojo/game/Battle.kt`
- scenario runtime: `core/src/main/kotlin/com/jojo/game/ScenarioRuntime.kt`
- campaign route/store: `core/src/main/kotlin/com/jojo/game/CampaignStore.kt`
- 길거리/회관 scenario 화면: `core/src/main/kotlin/com/jojo/game/ScenarioScreen.kt`
- 출진 화면: `core/src/main/kotlin/com/jojo/game/BattlePreparationScreen.kt`
- production E2E driver/trace: `core/src/main/kotlin/com/jojo/game/CampaignE2eTrace.kt`
- full battle trace: `core/src/main/kotlin/com/jojo/game/FullBattleTrace.kt`
- 실행 인자: `desktop/src/main/kotlin/com/jojo/game/desktop/DesktopLauncher.kt`
...
- 원본 BattleScreen: `../jojo_mobile/sgccz-desktop/recovered-js/modules/battle/BattleLayer.js`
...
실행 전에 오래된 게임/Electron 프로세스가 없는지 확인한다. 사용자가 단순히 “실행”을
요청하면 background 검증 fixture가 아니라 조작 가능한 일반 게임 게임을 실행한다.
...
- [ ] 최신 `:core:test` 통과
- [ ] 최신 `:desktop:check` 통과
- [ ] 마지막 단계별 reference/game screenshot 비교 통과
- [ ] 일반 게임 실행에서 시작, 대화 자동 연결, 조조 조작, 적 턴, 다음 scenario 전환 수동 확인
