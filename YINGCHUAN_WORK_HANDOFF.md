...
- 2턴 첫 수동 조작의 `전투 메뉴 → 지형 정보 → 닫기` 경로를 원본 CDP 포인터와 포트의
  운영 InputProcessor로 각각 캡처했다. 원본 `screens.json`의 네 화면과 포트
  `yingchuan-walkthrough.json`의 세 화면에서 라운드·메뉴·지형·조조 위치 및 행동 상태가
  일치했다. `tools/yingchuan-comparisons/menu-first-control.py`가 이 상태를 검사한다.
- 원본 하네스의 자동 입력 차단 범위를 2턴 아군 조작 중으로 제한했다. 그 전에는 승리 조건
  안내의 닫기까지 막아 조작 구간에 도달하지 못했다. 새 캡처는 완료됐다.
- 지형창 실화면 대조에서 빠진 제목 띠·표 머리 칸 배경·가림막을 원본 자산으로 복구하고,
  머리 칸과 하단 단추의 글자 정렬 및 전체 라벨 기준선을 조정했다. 마지막 캡처의 첫 등급
  원형 표식은 원본 `(896..954, 372..430)`, 포트 `(891..947, 368..424)` 픽셀로 남은
  4~7px 차이가 있다. 비교 스크립트는 상태만 판정하므로 픽셀 대조는 별도 작업이다.
...
- 이전의 미완 작업 파일은 스테이징하지 않았다. `BattleScreen.kt`는 새 수정 네 hunk만
  따로 스테이징했다.
- 강화창 렌더 기록기의 라벨 색도 실제 그리기와 연결했다 (`d0c3483`). 색 비교를 켜자
  원본의 장착자 이름은 `#0000f0`, 포트는 검정이라는 결함이 드러나 수정했다.
  `item-upgrade` 원본 대조와 두 모듈 전체 테스트가 통과했다. 원본 강화창 캡처는 가끔
  다른 장면을 잡아 게이트 재시도가 있었으므로 실패 시 캡처 내용을 확인해야 한다.
- 보상창의 빠진 색 비교를 켰고, 원본의 전투 종료 회색 그림자, 보상금·별의 주황/노랑,
  전리품 제목 파랑 그림자, 아이템 이름 진한 파랑을 실제 렌더러에도 복구했다 (`3ccdde6`).
  보상 3경로와 전체 테스트가 통과했다.
- 전당 `battle-view` 16개 항목의 색을 모두 비교하도록 했다 (`ea484a6`). 선택 마커 인덱스를
  화면 상태에 추가해 렌더러와 기록기가 함께 사용한다. 배경 검정, 선택 칸 빨강, 라벨 검정,
  나머지 흰색이 원본 대조를 통과했고 `:core:test :verification:test`도 통과했다.
- 전장 편집창 4경로의 공통 23~42개 색을 실제 그리기 계약에 연결했다 (`1f5a712`). 제목 띠
  `#97fee7`, 라벨·흐림막 검정, 나머지 흰색이 모두 원본 대조를 통과했다.
- 패배 확인창의 질문 파랑, "비" 빨강, "예" 초록을 실제 화면에 복구하고 9개 항목의 색을
  비교하도록 했다 (`234aeec`). 공통 전투 제목의 검정 그림자·흰색 제목도 보상/강화 4경로에
  연결했다 (`904f338`). 전체 테스트가 통과했다.
- 현재 생성돼 있는 모든 원본/포트 렌더 로그 쌍에서 `원본 color != null && 포트 color == null`은
  **0개**다. 다음 우선순위는 남은 구조적 기록기 중복과 글꼴 치환 오차다.
