...
// Applied by verification/build.gradle.kts.  These are source/game oracle
// tasks, so their owning project is verification and the only project
// dependency is verification -> core.
// Applied Kotlin scripts do not receive Gradle's generated task accessors, so
// provide the few accessors this legacy task catalog uses explicitly.
val sourceSets = extensions.getByType<SourceSetContainer>()
val SourceSetContainer.main get() = named("main")
val SourceSetContainer.test get() = named("test")
val SourceSet.runtimeClasspath: FileCollection get() = getRuntimeClasspath()
val SourceSet.output: SourceSetOutput get() = getOutput()
val TaskContainer.classes get() = named("classes")
val TaskContainer.testClasses get() = named("testClasses")
fun TaskContainer.test(configure: Test.() -> Unit) = named<Test>("test").configure(configure)
fun TaskContainer.processResources(configure: ProcessResources.() -> Unit) =
    named<ProcessResources>("processResources").configure(configure)
...
// Resource packaging belongs to core.  Keep this legacy declaration disabled
// here while the trace catalog is hosted by verification.
if (false) {
