    ...
    fun main(args: Array<String>) {
        ...
        val observer = RuntimeScreenObserver { delta, probe ->
            if (!finished && probe is ScenarioRuntimeProbe) {
                ...
                if (probe.playback == PlaybackState.DIALOGUE && probe.dialogueRevision == 1L) {
                    check(probe.dialogueSpeakerId == "181")
                    ...
                    if (firstDialogueFrames.size < 3) {
                        ...
                        observation.addChild("portraitReady", JsonValue(portraitReady[181] == true))
                        observation.addChild("actor181", actor(game, probe, 181))
                    ...
                    if (prefixLength >= nextPrefixLength) {
                        check(prefixLength == nextPrefixLength) {
                            "first dialogue skipped prefix $nextPrefixLength and reached $prefixLength"
                        }
                        check(probe.dialogueVisibleText == fullText.take(prefixLength))
                        check(!probe.naturalStreetTextIsolation)
                        val expectedActors = mapOf(0 to (40 to 35), 157 to (54 to 65), 181 to (40 to 45), 182 to (40 to 25))
                        ...
                        capture.addChild("portraitReady", JsonValue(portraitReady[181] == true))
                        ...
                        if (nextPrefixLength > fullText.length) {
                            ...
                            report.addChild("contract", JsonValue("natural-first-dialogue-full-prefixes-rgba8"))
                            ...
                            File(directory, "game-full-prefixes.json").writeText(
                                report.prettyPrint(JsonWriter.OutputType.json, 120),
                            )
                            finished = true
                            Gdx.app.log("JojoGame", "OPENING_FULL_PREFIXES_COMPLETE frame=$frame")
                            Gdx.app.exit()
                        }
                    }
                }
        ...
        val window = Lwjgl3ApplicationConfiguration().apply {
            setTitle("Jojo full opening dialogue prefixes verification")
