...
object OpeningFinalGroupFramesDesktopLauncher {
    ...
    fun main(args: Array<String>) {
            ...
            if (!finished && probe is ScenarioRuntimeProbe) {
                ...
                if (groupStarted) {
                    ...
                    if (groupOrdinal in captureOrdinals) {
                        val fileName = "game-final-group-${groupOrdinal.toString().padStart(3, '0')}.rgba"
                    ...
                    if (groupOrdinal == captureOrdinals.max()) {
                        ...
                        report.addChild("contract", JsonValue("controlled-opening-final-group-rgba8"))
                        ...
                        File(directory, "game-final-group.json").writeText(
                            report.prettyPrint(JsonWriter.OutputType.json, 120),
                        )
                        finished = true
                        Gdx.app.log("JojoGame", "OPENING_FINAL_GROUP_FRAMES_COMPLETE frame=$frame")
                        Gdx.app.exit()
                    }
                }
            }
        ...
        val window = Lwjgl3ApplicationConfiguration().apply {
            setTitle("Jojo controlled opening final group verification")
