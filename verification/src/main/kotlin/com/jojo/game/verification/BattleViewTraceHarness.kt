...
import com.jojo.game.presentation.battle.fight.FightUnit
...
object BattleViewTraceHarness {
    private fun block(s: String, at: Int): String {
        ...
        var d = 0
        var q = false
        var esc = false; for (i in at until s.length) {
            val c = s[i]; if (q) {
                if (!esc && c == '"') q = false; esc = !esc && c == '\\'; continue
            }; if (c == '"') q = true else if (c == open) d++ else if (c == close && --d == 0) return s.substring(
                at,
                i + 1
            )
        }; error("unclosed")
    ...
    private fun objects(s: String): List<String> {
        ...
        var i = 0; while (i < s.length) {
            if (s[i] == '{') {
                val b = block(s, i); r += b; i += b.length
            } else i++
        }; return r
    }
    ...
    fun main(a: Array<String>) {
        ...
        val cases = objects(arr(raw, "cases"))
        val result = cases.joinToString(",", "{", "}") { c ->
            val n = str(c, "name"); q(n) + ":" + when (str(c, "kind")) {
            "view" -> view(c); "fight" -> fight(c); else -> dule(c)
        }
        }
    ...
    private fun view(c: String): String {
        ...
        val ps = Regex("\\[(-?\\d+),(-?\\d+)]").findAll(arr(c, "pos"))
            ...
            val m = x.markers().joinToString(
                ...
                "[",
                "]"
            ) { "{\"x\":${it.x},\"y\":${it.y},\"label\":${q(it.label)},\"red\":${it.red ?: "null"},\"opacity\":${it.opacity ?: "null"}}" }; return "{\"step\":${
                q(
                    step
                )
            },\"mapPath\":${q("frame")},\"markers\":$m,\"events\":[\"BATTLE_VIEW_INIT_OVER\"]}"
        }

        val trace = mutableListOf(snap("create")); strings(
            arr(
                c,
                "events"
            )
        ).forEach { e ->
            if (e.startsWith("unit:")) x.battleUnitN(
    ...
    private fun fight(c: String): String {
        ...
        val e = strings(arr(c, "events")); x.create(
            if ("sound:321" in e) 321 else -1,
            if ("sound:321" in e) "321" else "yidong",
            e.filter { it.startsWith("shader:") }.map { it.substringAfter(':').toInt() }); x.setActionDir(
            int(c, "action"),
            "finished" in e
        ); return "[{\"step\":\"create\",\"parent\":[${x.parentX},${x.parentY},${x.parentScaleX}],\"node\":[${x.nodeX},${x.nodeY},${x.nodeScaleX}],\"action\":${x.action},\"animation\":${
            q(
                x.animation
            )
    ...
    private fun dule(c: String): String = strings(arr(c, "events")).joinToString(
        ...
        "[",
        "]"
    ) { e -> "{\"step\":${q(e)},\"events\":[],\"attached\":true}" }
}
