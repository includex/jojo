# 조조전 원본 충실 포팅 작업 인수인계 (2026-09-04)

> **주의: 최신 상태와 재개 계획은 `PORTING_AGENT_HANDOFF_LATEST.md`를 먼저 읽는다.**
...
- `core/src/main/kotlin/com/jojo/port/JojoGame.kt`
  ...
  - `super.render()` 뒤 실제 `BattleLayer.campaignE2eState()`를 관찰해 다음 입력을 보낸다.
...
- `core/src/main/kotlin/com/jojo/port/BattleLayer.kt`
  ...
  - full trace가 개발자의 `jojo-auto-battle/TUOGUAN` preference를 읽거나 덮어쓰지 않게
    했다.
- `core/src/main/kotlin/com/jojo/port/FullBattleTrace.kt`
...
3. **논리 검증이 모두 green인 뒤에만 최종 render gate**
   ...
   ```
4. **문서 갱신 및 최신 포트 실행**
   - `PORTING_RUNTIME_AUDIT.md`, `RUNTIME_UI_ROUTE_CLASSIFICATION.md`에 최종 증거를 반영한다.
...
- 원본 JS/Python callback 순서가 유일한 권위다. 포트 코드와 기존 테스트가 일치해도
  원본과 다르면 둘 다 고친다.
...
- 원본/포트 GUI capture는 foreground 우연성에 의존하지 않는다. verify mode의 원본
...
“전체 포트 100%”를 문자 그대로 전체 게임으로 해석한다면, 아래 회귀를 통과한 뒤에도
전 시나리오 production E2E 확대가 별도 장기 작업으로 남는다.

## 2. 디렉터리와 권위 소스

- 포트: `/Users/ain/workspace/jojo`
...
- 원본 코드가 권위다. 포트의 기존 동작이나 기존 테스트 기대값을 권위로 삼지 않는다.
...
- production `TitleScreen`, `ScenarioPreviewScreen`, `BattleLayer` 사용
...
이번 변경에서 `PythonAstRuntime`은 BattleLayer가 external presentation을 명시적으로
...
- `PythonAstRuntime.kt`: external battle presentation gate
- `Battle.kt`: scripted hidden unit 보존
- `BattleLayer.kt`: typed hide 및 generic unitDeath state machine
...
- 실제 `PythonAstRuntime.currentDialogue`를 사용하는 외부 say4 gate
...
```bash
./gradlew core:test \
  --tests com.jojo.port.BattleDefeatPresentationTest \
  --tests 'com.jojo.port.ScenarioRuntimeTest.native battle say4 borrows the production dialogue state until input dismisses it'
```
...
- `PythonAstRuntime.kt`: invisible unit show의 external callback gate
- `Battle.kt`: presentation unit을 active roster로 복구하고 attr15 영속화
- `BattleLayer.kt`: typed show driver와 lose/combat busy gate
...
- 강조/사망 표현, HP bar, 대사 패널 뒤 투명도·블렌딩 경로를 실제 BattleLayer에
  연결했다.
...
line 1232를 기록하며 실패했다. 포트 로직 결함이 아니라 fixture가 `lab1542`로 직접
...
`BattleScenePort` 1개는 source contract로 격리되어 있고 runtime 완료 증거로 세면 안 된다.
...
직접 원인은 `core/src/main/kotlin/com/jojo/port/BattleLayer.kt`의
...
3. `CampaignE2eTraceDriver.driveBattle()`의 input helper를 재사용 가능한 public-input
   ...
   `BattleLayer.campaignE2eState()`를 관찰해 standalone trace도 같은 driver로 구동한다.
...
- `core/src/main/kotlin/com/jojo/port/BattleLayer.kt`: `driveFullBattleTrace()` 약 2262행
- `core/src/main/kotlin/com/jojo/port/CampaignE2eTrace.kt`: 실제 화면 입력 driver
- `core/src/main/kotlin/com/jojo/port/FullBattleTrace.kt`: config/trace 계약
- `desktop/src/main/kotlin/com/jojo/port/desktop/DesktopLauncher.kt`: `externalUiDriver`
...
standalone driver 수정이 `BattleLayer.kt`를 건드리므로 수정 후 다시 실행한다.
...
실패하면 테스트 기대값을 임의로 포트에 맞추지 말고 원본 JS/Python의 호출 순서와
callback을 다시 확인한다.

### B. 빠른 정적/집중 검증

다른 Gradle 프로세스가 없는지 먼저 확인한다. 인수인계 작성 도중 아래의 오래된 포트
...
- `PORTING_RUNTIME_AUDIT.md` section 12의 최종 실행 결과를 최신 전체 pass로 갱신한다.
...
- 마지막으로 stale 포트 프로세스를 종료하고 최신 포트를 실행한다.
...
```text
/Users/ain/workspace/jojo/PORTING_HANDOFF_2026-09-04.md를 처음부터 끝까지 읽고
