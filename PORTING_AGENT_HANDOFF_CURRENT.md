...
> `PORTING_AGENT_HANDOFF_LATEST.md`, `PORTING_HANDOFF_2026-09-04.md`에는 당시에는
> 유효했지만 현재 수정되었거나 아직 완료로 오해할 수 있는 기록이 섞여 있다.

## 1. 최종 목표와 완료 정의

목표는 원본 게임을 LibGDX 포트에서 처음부터 엔딩까지 플레이할 수 있게 만드는 것이며,
...
- S22 source v13 ↔ port v41 camp boundary 114개 exact, mismatch 0
  - `build/reports/s22-source-v13-port-v41-camp-boundaries.json`
- S52 authored prefix/objects/center/setUnitStatus 요구 통과
  - `build/port-full-battle-late-v12/manifest.json`
- S01 v2 round-1 정지 원인 수정
  ...
  - 수정 후 `build/port-full-battle-fixes-v21`에서 3,301 frames, natural battle-end 통과
...
- `build/port-full-battle-s57-v22/manifest.json`
...
- 관련 파일:
  - `core/src/main/kotlin/com/jojo/port/Battle.kt`
  - `core/src/main/kotlin/com/jojo/port/BattleLayer.kt`
  - `core/src/test/kotlin/com/jojo/port/BattleInteractiveInputTest.kt`
...
python3 tools/summarize_battle_trace.py \
  --trace build/port-full-battle-s57-v22/traces/S_57.json \
  --scenario S_57 \
  --leaders 162,169,165 \
...
- 관련 파일:
  ...
  - `core/src/main/kotlin/com/jojo/port/TerrainLayerChromeRenderContract.kt`
  - `core/src/main/kotlin/com/jojo/port/SettlementInfoRenderContract.kt`
...
### 4.1 포트 내부 일치와 원본 일치는 다르다

포트 모델과 포트 테스트가 서로 green이어도 원본 callback/asset/geometry와 다르면 실패다.
...
2. `BattleLayer.kt`, `tools/export_map_assets.py`처럼 공유 hot file은 동시에 편집하지 않는다.
...
- 여러 agent가 `BattleLayer.kt`나 exporter를 동시에 편집
...
./gradlew --no-daemon --console=plain \
  :core:test \
  --tests com.jojo.port.BattleInteractiveInputTest \
  --tests com.jojo.port.BattleCommandFlowTest \
  --tests com.jojo.port.BattleUiAssetsTest \
  --tests com.jojo.port.TerrainLayerTest \
  --tests com.jojo.port.MineUnitInfoLayerTest \
  --tests com.jojo.port.OtherUnitInfoLayerTest \
  :desktop:installDist

python3 tools/run_port_full_battle_batch.py \
  --scenario S_57 \
  --output build/port-full-battle-s57-v23/manifest.json \
...
```bash
python3 tools/summarize_battle_trace.py \
  --trace build/port-full-battle-s57-v23/traces/S_57.json \
  --scenario S_57 \
...
- `tools/run_port_campaign_checkpoint_batch.py`
- `tools/verify_campaign_route_e2e.py`
- `tools/run_port_full_battle_batch.py`
...
- "시각 검증됨": fresh source/port render 비교까지 통과했다.
