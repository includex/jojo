...
// Source runners are Exec tasks, and several legacy declarations only listed
...
/** Runs the Kotlin side of the shared MenuLayer source/port trace fixture. */
val menuLayerTrace = tasks.register<JavaExec>("menuLayerTrace") {
    dependsOn(tasks.classes)
    classpath = sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.MenuLayerTraceHarness")
    args(System.getProperty("menuFixture") ?: rootProject.file("tools/menu_layer_trace_cases.json").absolutePath)
}
val terrainLayerTrace = tasks.register<JavaExec>("terrainLayerTrace") { dependsOn(tasks.classes); classpath=sourceSets.main.get().runtimeClasspath; mainClass.set("com.jojo.port.TerrainLayerTraceHarness"); args(rootProject.file("tools/terrain_layer_trace_cases.json").absolutePath) }

/** Executes the recovered original BattleUnit.countRate through its Cocos-minimal Node harness. */
val verifyOriginalCountRate = tasks.register<Exec>("verifyOriginalCountRate") {
    inputs.file(rootProject.file("tools/original_count_rate_harness.js"))
    inputs.file(rootProject.file("../jojo_mobile/sgccz-desktop/recovered-js/modules/battle/BattleUnit.js"))
    commandLine("node", rootProject.file("tools/original_count_rate_harness.js").absolutePath)
}
...
val unitInfoPortOutput = layout.buildDirectory.file("unit-info/port-trace.json")
...
val unitInfoTrace = tasks.register<JavaExec>("unitInfoTrace") { dependsOn(tasks.classes); classpath=sourceSets.main.get().runtimeClasspath; mainClass.set("com.jojo.port.UnitInfoTraceHarness"); args(unitInfoFixture.absolutePath,unitInfoPortOutput.get().asFile.absolutePath); outputs.file(unitInfoPortOutput) }
val verifyUnitInfoPairwise = tasks.register<Exec>("verifyUnitInfoPairwise") { dependsOn(unitInfoTrace,verifyUnitInfoSourceTrace); commandLine("python3",rootProject.file("tools/verify_unit_info_pairwise.py").absolutePath,unitInfoSourceOutput.get().asFile.absolutePath,unitInfoPortOutput.get().asFile.absolutePath) }
...
val loadGamePortOutput=layout.buildDirectory.file("load-game/port-trace.json")
...
val loadGameTrace=tasks.register<JavaExec>("loadGameTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.LoadGameTraceHarness");args(loadGameFixture.absolutePath,loadGamePortOutput.get().asFile.absolutePath);outputs.file(loadGamePortOutput)}
val verifyLoadGamePairwise=tasks.register<Exec>("verifyLoadGamePairwise"){dependsOn(verifyLoadGameSourceTrace,loadGameTrace);commandLine("python3",rootProject.file("tools/verify_load_game_pairwise.py").absolutePath,loadGameSourceOutput.get().asFile.absolutePath,loadGamePortOutput.get().asFile.absolutePath)}
val battleLayerFixture=rootProject.file("tools/battle_layer_trace_cases.json");val battleLayerSourceOutput=layout.buildDirectory.file("battle-layer/source-trace.json");val battleLayerPortOutput=layout.buildDirectory.file("battle-layer/port-trace.json")
val verifyBattleLayerSourceTrace=tasks.register<Exec>("verifyBattleLayerSourceTrace"){commandLine("node",rootProject.file("tools/battle_layer_source_trace_harness.js").absolutePath,battleLayerFixture.absolutePath,battleLayerSourceOutput.get().asFile.absolutePath);outputs.file(battleLayerSourceOutput)}
val battleLayerTrace=tasks.register<JavaExec>("battleLayerTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.BattleLayerTraceHarness");args(battleLayerFixture.absolutePath,battleLayerPortOutput.get().asFile.absolutePath);outputs.file(battleLayerPortOutput)}
val verifyBattleLayerPairwise=tasks.register<Exec>("verifyBattleLayerPairwise"){dependsOn(verifyBattleLayerSourceTrace,battleLayerTrace);commandLine("python3",rootProject.file("tools/verify_battle_layer_pairwise.py").absolutePath,battleLayerSourceOutput.get().asFile.absolutePath,battleLayerPortOutput.get().asFile.absolutePath)}
val fightFixture=rootProject.file("tools/fight_presentation_trace_cases.json");val fightSource=layout.buildDirectory.file("fight/source.json");val fightPort=layout.buildDirectory.file("fight/port.json")
...
val fightTrace=tasks.register<JavaExec>("fightPresentationTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.FightPresentationTraceHarness");args(fightFixture.absolutePath,fightPort.get().asFile.absolutePath);outputs.file(fightPort)}
val verifyFightPairwise=tasks.register<Exec>("verifyFightPresentationPairwise"){dependsOn(verifyFightSource,fightTrace);commandLine("python3",rootProject.file("tools/verify_fight_presentation_pairwise.py").absolutePath,fightSource.get().asFile.absolutePath,fightPort.get().asFile.absolutePath)}

/** WinConBox/Lose/End/Skip recovered factories and direct Kotlin lifecycle ports. */
...
val endFlowPort=layout.buildDirectory.file("end-flow/port.json")
...
val endFlowTrace=tasks.register<JavaExec>("endFlowTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.EndFlowTraceHarness");args(endFlowFixture.absolutePath,endFlowPort.get().asFile.absolutePath);outputs.file(endFlowPort)}
val verifyEndFlowPairwise=tasks.register<Exec>("verifyEndFlowPairwise"){dependsOn(endFlowSourceTrace,endFlowTrace);commandLine("python3",rootProject.file("tools/verify_end_flow_pairwise.py").absolutePath,endFlowSource.get().asFile.absolutePath,endFlowPort.get().asFile.absolutePath)}

tasks.test {
    dependsOn(verifyOriginalCountRate)
    dependsOn(verifyUnitInfoSourceTrace)
    dependsOn(verifyUnitInfoPairwise)
    dependsOn(verifyLoadGamePairwise)
    dependsOn(verifyBattleLayerPairwise)
...
val verifyMenuLayerTrace = tasks.register<Exec>("verifyMenuLayerTrace") {
    ...
    inputs.file(rootProject.file("tools/original_menu_layer_trace_harness.js"))
...
val verifyTerrainLayerPairwise = tasks.register<Exec>("verifyTerrainLayerPairwise") {
    ...
    inputs.file(rootProject.file("tools/original_terrain_layer_trace_harness.js"))
...
/** Recovered ui/TreasureLayer.js factory and the direct Kotlin port. */
...
val treasureLayerPort=layout.buildDirectory.file("treasure-layer/port.json")
...
val treasureLayerTrace=tasks.register<JavaExec>("treasureLayerTrace") { dependsOn(tasks.classes); classpath=sourceSets.main.get().runtimeClasspath; mainClass.set("com.jojo.port.TreasureLayerTraceHarness"); args(treasureLayerFixture.absolutePath,treasureLayerPort.get().asFile.absolutePath); outputs.file(treasureLayerPort) }
...
/** Recovered ui/PropertyLayer.js and the Kotlin data/lifecycle port share this trace fixture. */
...
val propertyLayerPortOutput = layout.buildDirectory.file("property-layer/port-trace.json")
...
val propertyLayerTrace = tasks.register<JavaExec>("propertyLayerTrace") {
    dependsOn(tasks.classes)
    classpath = sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.PropertyLayerTraceHarness")
    args(propertyLayerFixture.absolutePath, propertyLayerPortOutput.get().asFile.absolutePath)
    outputs.file(propertyLayerPortOutput)
}
val verifyPropertyLayerPairwise = tasks.register<Exec>("verifyPropertyLayerPairwise") {
    ...
    inputs.file(propertyLayerPortOutput)
    commandLine("python3", rootProject.file("tools/verify_property_layer_pairwise.py").absolutePath, propertyLayerSourceOutput.get().asFile.absolutePath, propertyLayerPortOutput.get().asFile.absolutePath)
}
...
val saveLayerPortOutput = layout.buildDirectory.file("save-layer/port-trace.json")
...
val saveLayerTrace = tasks.register<JavaExec>("saveLayerTrace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.SaveLayerTraceHarness");args(saveLayerFixture.absolutePath,saveLayerPortOutput.get().asFile.absolutePath);outputs.file(saveLayerPortOutput) }
val verifySaveLayerPairwise = tasks.register<Exec>("verifySaveLayerPairwise") { dependsOn(saveLayerSourceTrace,saveLayerTrace);commandLine("python3",rootProject.file("tools/verify_save_layer_pairwise.py").absolutePath,saveLayerSourceOutput.get().asFile.absolutePath,saveLayerPortOutput.get().asFile.absolutePath) }
val loadLayerFixture=rootProject.file("tools/load_layer_trace_cases.json");val loadLayerSource=layout.buildDirectory.file("load-layer/source.json");val loadLayerPort=layout.buildDirectory.file("load-layer/port.json")
...
val loadLayerTrace=tasks.register<JavaExec>("loadLayerTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.LoadLayerTraceHarness");args(loadLayerFixture.absolutePath,loadLayerPort.get().asFile.absolutePath);outputs.file(loadLayerPort)}
val verifyLoadLayerPairwise=tasks.register<Exec>("verifyLoadLayerPairwise"){dependsOn(loadLayerSourceTrace,loadLayerTrace);commandLine("python3",rootProject.file("tools/verify_load_layer_pairwise.py").absolutePath,loadLayerSource.get().asFile.absolutePath,loadLayerPort.get().asFile.absolutePath)}
...
val settingLayerPortOutput = layout.buildDirectory.file("setting-layer/port-trace.json")
...
val settingLayerTrace = tasks.register<JavaExec>("settingLayerTrace") {
    dependsOn(tasks.classes)
    classpath = sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.SettingLayerTraceHarness")
    args(settingLayerFixture.absolutePath, settingLayerPortOutput.get().asFile.absolutePath)
    outputs.file(settingLayerPortOutput)
}
val verifySettingLayerPairwise = tasks.register<Exec>("verifySettingLayerPairwise") {
    ...
    inputs.file(settingLayerPortOutput)
    commandLine("python3", rootProject.file("tools/verify_setting_layer_pairwise.py").absolutePath, settingLayerSourceOutput.get().asFile.absolutePath, settingLayerPortOutput.get().asFile.absolutePath)
}
...
val helperLayerPortOutput = layout.buildDirectory.file("helper-layer/port-trace.json")
...
val helperLayerTrace = tasks.register<JavaExec>("helperLayerTrace") {
    dependsOn(tasks.classes)
    classpath = sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.HelperLayerTraceHarness")
    args(helperLayerFixture.absolutePath, helperLayerPortOutput.get().asFile.absolutePath)
    outputs.file(helperLayerPortOutput)
}
val verifyHelperLayerPairwise = tasks.register<Exec>("verifyHelperLayerPairwise") {
    ...
    inputs.file(helperLayerPortOutput)
    commandLine("python3", rootProject.file("tools/verify_helper_layer_pairwise.py").absolutePath, helperLayerSourceOutput.get().asFile.absolutePath, helperLayerPortOutput.get().asFile.absolutePath)
}
...
val roundLayerPortOutput = layout.buildDirectory.file("round-layer/port-trace.json")
...
val roundLayerTrace = tasks.register<JavaExec>("roundLayerTrace") {
    ...
    mainClass.set("com.jojo.port.RoundLayerTraceHarness")
    args(roundLayerFixture.absolutePath, roundLayerPortOutput.get().asFile.absolutePath); outputs.file(roundLayerPortOutput)
}
val verifyRoundLayerPairwise = tasks.register<Exec>("verifyRoundLayerPairwise") {
    dependsOn(roundLayerSourceTrace, roundLayerTrace)
    inputs.file(rootProject.file("tools/verify_round_layer_pairwise.py")); inputs.file(roundLayerSourceOutput); inputs.file(roundLayerPortOutput)
    commandLine("python3", rootProject.file("tools/verify_round_layer_pairwise.py").absolutePath, roundLayerSourceOutput.get().asFile.absolutePath, roundLayerPortOutput.get().asFile.absolutePath)
}
tasks.test { dependsOn(verifyRoundLayerPairwise) }

val mapInfoFixture=rootProject.file("tools/map_info_layer_trace_cases.json")
val mapInfoSource=layout.buildDirectory.file("map-info/source.json");val mapInfoPort=layout.buildDirectory.file("map-info/port.json")
...
val mapInfoTrace=tasks.register<JavaExec>("mapInfoLayerTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.MapInfoLayerTraceHarness");args(mapInfoFixture.absolutePath,mapInfoPort.get().asFile.absolutePath);outputs.file(mapInfoPort)}
val verifyMapInfoLayerPairwise=tasks.register<Exec>("verifyMapInfoLayerPairwise"){dependsOn(mapInfoSourceTrace,mapInfoTrace);commandLine("python3",rootProject.file("tools/verify_map_info_layer_pairwise.py").absolutePath,mapInfoSource.get().asFile.absolutePath,mapInfoPort.get().asFile.absolutePath)}
tasks.test { dependsOn(verifyMapInfoLayerPairwise) }

val miniMapFixture=rootProject.file("tools/mini_map_layer_trace_cases.json");val miniMapSource=layout.buildDirectory.file("mini-map/source.json");val miniMapPort=layout.buildDirectory.file("mini-map/port.json")
...
val miniMapTrace=tasks.register<JavaExec>("miniMapLayerTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.MiniMapLayerTraceHarness");args(miniMapFixture.absolutePath,miniMapPort.get().asFile.absolutePath);outputs.file(miniMapPort)}
val verifyMiniMapLayerPairwise=tasks.register<Exec>("verifyMiniMapLayerPairwise"){dependsOn(miniMapSourceTrace,miniMapTrace);commandLine("python3",rootProject.file("tools/verify_mini_map_layer_pairwise.py").absolutePath,miniMapSource.get().asFile.absolutePath,miniMapPort.get().asFile.absolutePath)}
...
val enemyTurnPort=layout.buildDirectory.file("enemy-turn/port.json")
...
val enemyTurnTrace=tasks.register<JavaExec>("enemyTurnTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.EnemyTurnTraceHarness")
    args(enemyTurnFixture.absolutePath,enemyTurnPort.get().asFile.absolutePath);outputs.file(enemyTurnPort)
}
val verifyEnemyTurnPairwise=tasks.register<Exec>("verifyEnemyTurnPairwise") {
    dependsOn(enemyTurnSourceTrace,enemyTurnTrace)
    inputs.file(rootProject.file("tools/verify_enemy_turn_pairwise.py"));inputs.file(enemyTurnSource);inputs.file(enemyTurnPort)
    commandLine("python3",rootProject.file("tools/verify_enemy_turn_pairwise.py").absolutePath,enemyTurnSource.get().asFile.absolutePath,enemyTurnPort.get().asFile.absolutePath)
}
tasks.test { dependsOn(verifyEnemyTurnPairwise) }

/** ForcesListLayer: original recovered factory and Kotlin port consume one input stream. */
...
val forcesListPort=layout.buildDirectory.file("forces-list/port-trace.json")
...
val forcesListTrace=tasks.register<JavaExec>("forcesListLayerTrace") {
    dependsOn(tasks.classes); classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.ForcesListLayerTraceHarness")
    args(forcesListFixture.absolutePath,forcesListPort.get().asFile.absolutePath); outputs.file(forcesListPort)
}
val verifyForcesListLayerPairwise=tasks.register<Exec>("verifyForcesListLayerPairwise") {
    ...
    commandLine("python3",rootProject.file("tools/verify_forces_list_layer_pairwise.py").absolutePath,forcesListSource.get().asFile.absolutePath,forcesListPort.get().asFile.absolutePath)
}
tasks.test { dependsOn(verifyForcesListLayerPairwise) }

val sectionFixture=rootProject.file("tools/section_layer_trace_cases.json"); val sectionSource=layout.buildDirectory.file("section-layer/source.json"); val sectionPort=layout.buildDirectory.file("section-layer/port.json")
...
val sectionTrace=tasks.register<JavaExec>("sectionLayerTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.SectionLayerTraceHarness");args(sectionFixture.absolutePath,sectionPort.get().asFile.absolutePath);outputs.file(sectionPort)}
val verifySectionLayerPairwise=tasks.register<Exec>("verifySectionLayerPairwise"){dependsOn(sectionSourceTrace,sectionTrace);commandLine("python3",rootProject.file("tools/verify_section_layer_pairwise.py").absolutePath,sectionSource.get().asFile.absolutePath,sectionPort.get().asFile.absolutePath)}
...
val magicSource=layout.buildDirectory.file("magic/source.json");val magicPort=layout.buildDirectory.file("magic/port.json")
...
val magicTrace=tasks.register<JavaExec>("magicTrace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.MagicTraceHarness");args(magicFixture.absolutePath,magicPort.get().asFile.absolutePath);outputs.file(magicPort) }
val verifyMagicPairwise=tasks.register<Exec>("verifyMagicPairwise") { dependsOn(magicSourceTrace,magicTrace);commandLine("python3",rootProject.file("tools/verify_magic_pairwise.py").absolutePath,magicSource.get().asFile.absolutePath,magicPort.get().asFile.absolutePath) }
...
val choiceCommandPort=layout.buildDirectory.file("choice-command/port.json")
...
val choiceCommandTrace=tasks.register<JavaExec>("choiceCommandTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.ChoiceCommandTraceHarness")
    args(choiceCommandFixture.absolutePath,choiceCommandPort.get().asFile.absolutePath);outputs.file(choiceCommandPort)
}
val verifyChoiceCommandPairwise=tasks.register<Exec>("verifyChoiceCommandPairwise") {
    ...
    commandLine("python3",rootProject.file("tools/verify_choice_command_pairwise.py").absolutePath,choiceCommandSource.get().asFile.absolutePath,choiceCommandPort.get().asFile.absolutePath)
}
...
val itemEquipPort=layout.buildDirectory.file("item-equip/port.json")
...
val itemEquipTrace=tasks.register<JavaExec>("itemEquipTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.ItemEquipTraceHarness")
    args(itemEquipFixture.absolutePath,itemEquipPort.get().asFile.absolutePath);outputs.file(itemEquipPort)
}
val verifyItemEquipPairwise=tasks.register<Exec>("verifyItemEquipPairwise") {
    dependsOn(itemEquipSourceTrace,itemEquipTrace)
    commandLine("python3",rootProject.file("tools/verify_item_equip_pairwise.py").absolutePath,itemEquipSource.get().asFile.absolutePath,itemEquipPort.get().asFile.absolutePath)
}
...
val characterAbilityPort=layout.buildDirectory.file("character-ability/port.json")
...
val cmdLayerPort=layout.buildDirectory.file("cmd-layer/port.json")
...
val cmdLayerTrace=tasks.register<JavaExec>("cmdLayerTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.CmdLayerTraceHarness")
    args(cmdLayerFixture.absolutePath,cmdLayerPort.get().asFile.absolutePath);outputs.file(cmdLayerPort)
}
val verifyCmdLayerPairwise=tasks.register<Exec>("verifyCmdLayerPairwise") {
    dependsOn(cmdLayerSourceTrace,cmdLayerTrace)
    commandLine("python3",rootProject.file("tools/verify_cmd_layer_pairwise.py").absolutePath,cmdLayerSource.get().asFile.absolutePath,cmdLayerPort.get().asFile.absolutePath)
}
...
val battleViewPort=layout.buildDirectory.file("battle-view/port.json")
...
val battleViewTrace=tasks.register<JavaExec>("battleViewTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.BattleViewTraceHarness")
    args(battleViewFixture.absolutePath,battleViewPort.get().asFile.absolutePath);outputs.file(battleViewPort)
}
val verifyBattleViewPairwise=tasks.register<Exec>("verifyBattleViewPairwise") {
    dependsOn(battleViewSourceTrace,battleViewTrace)
    commandLine("python3",rootProject.file("tools/verify_battle_view_pairwise.py").absolutePath,battleViewSource.get().asFile.absolutePath,battleViewPort.get().asFile.absolutePath)
}
...
val hallUiPort=layout.buildDirectory.file("hall-ui/port.json")
...
val hallUiTrace=tasks.register<JavaExec>("hallUiTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.HallUiTraceHarness")
    args(hallUiFixture.absolutePath,hallUiPort.get().asFile.absolutePath);outputs.file(hallUiPort)
}
val verifyHallUiPairwise=tasks.register<Exec>("verifyHallUiPairwise") {
    dependsOn(hallUiSourceTrace,hallUiTrace)
    inputs.files(rootProject.file("tools/verify_hall_ui_pairwise.py"),hallUiSource,hallUiPort)
    commandLine("python3",rootProject.file("tools/verify_hall_ui_pairwise.py").absolutePath,hallUiSource.get().asFile.absolutePath,hallUiPort.get().asFile.absolutePath)
}
...
val hallPrepPort=layout.buildDirectory.file("hall-prep/port.json")
...
val hallPrepTrace=tasks.register<JavaExec>("hallPrepTrace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.HallPrepTraceHarness");args(hallPrepFixture.absolutePath,hallPrepPort.get().asFile.absolutePath);outputs.file(hallPrepPort) }
val verifyHallPrepPairwise=tasks.register<Exec>("verifyHallPrepPairwise") { dependsOn(hallPrepSourceTrace,hallPrepTrace);commandLine("python3",rootProject.file("tools/verify_hall_prep_pairwise.py").absolutePath,hallPrepSource.get().asFile.absolutePath,hallPrepPort.get().asFile.absolutePath) }
tasks.test { dependsOn(verifyHallPrepPairwise) }

/** EditLayer owner mutations: source factory closures and Kotlin direct state ports. */
...
val editMutationPort=layout.buildDirectory.file("edit-mutation/port.json")
...
val unitListInfoPort=layout.buildDirectory.file("unit-list-info/port.json")
...
val unitListInfoTrace=tasks.register<JavaExec>("unitListInfoTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.UnitListInfoLayerTraceHarness")
    args(unitListInfoFixture.absolutePath,unitListInfoPort.get().asFile.absolutePath);outputs.file(unitListInfoPort)
}
val verifyUnitListInfoPairwise=tasks.register<Exec>("verifyUnitListInfoPairwise") {
    ...
    commandLine("python3",rootProject.file("tools/verify_unit_list_info_pairwise.py").absolutePath,unitListInfoSource.get().asFile.absolutePath,unitListInfoPort.get().asFile.absolutePath)
}
// Isolated source/port oracle only. Runtime coverage is enforced separately.

// Candidate-only read-only source input.  The copied project lives below
// `.port-isolated`, so the central project's relative source path is no
// longer its sibling.
...
val sourceLoginFramebuffers = files(
    rootProject.file(".port-isolated/natural-battle-capture/captures/source-login.rgba"),
    rootProject.file(".port-isolated/natural-battle-capture/captures/source-login-1-blank.rgba"),
    rootProject.file(".port-isolated/natural-battle-capture/captures/source-login-2.rgba"),
)
...
val extractTitleLoadConfirmations = tasks.register<Exec>("extractTitleLoadConfirmations") {
    val sources = (0..7).map { rootProject.file(".port-isolated/natural-battle-capture/captures/source-login-1-blank-row$it.rgba") }
    ...
    commandLine(
        ...
        rootProject.file(".port-isolated/natural-battle-capture/captures").absolutePath,
        generatedTitleLoadConfirmationDirectory.get().asFile.absolutePath,
    )
}
val sourceReferenceFramebuffers = files(
    rootProject.file(".port-isolated/natural-battle-capture/captures/source-login-0.rgba"),
    rootProject.file(".port-isolated/raw-framebuffer-common-space/infolayer-subtree-observation/source-hall-infolayer-bg-frame.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-choice.rgba"),
    rootProject.file(".port-isolated/raw-framebuffer-common-space/dialogue1-live-raw/source-r00-dialogue-1.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-dialogue-2.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-menu.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-save.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-load.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-setting.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-helper.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-win-condition.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-terrain.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-property.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-treasure.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-forces.rgba"),
    rootProject.file(".port-isolated/asset-recovery-audit/captures/source-unit-info.rgba"),
    rootProject.file(".port-isolated/natural-battle-capture/captures/source-r00-win-result.rgba"),
)
...
/** Source-to-port conformance gate for every BattleUnit atlas and animeBR row. */
...
// owns the repository-wide source/port pairwise gates; keeping this focused
...
tasks.register<Test>("battleRewardFlowTest") {
    ...
    filter {
        includeTestsMatching("com.jojo.port.BattleRewardFlowTest")
        includeTestsMatching("com.jojo.port.ScenarioRuntimeTest")
    }
}

/** Recovered MsgBox/Toast/Progress/Loading factories against direct Kotlin ports. */
...
val systemUiPort = layout.buildDirectory.file("system-ui/port.json")
...
val systemUiTrace = tasks.register<JavaExec>("systemUiTrace") {
    dependsOn(tasks.classes)
    classpath = sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.SystemUiTraceHarness")
    args(systemUiFixture.absolutePath, systemUiPort.get().asFile.absolutePath)
    outputs.file(systemUiPort)
}
val verifySystemUiPairwise = tasks.register<Exec>("verifySystemUiPairwise") {
    ...
    commandLine("python3", rootProject.file("tools/verify_system_ui_pairwise.py").absolutePath, systemUiSource.get().asFile.absolutePath, systemUiPort.get().asFile.absolutePath)
}
...
val platformPort=layout.buildDirectory.file("platform/port.json")
...
val platformTrace=tasks.register<JavaExec>("platformTrace") {
    ...
    classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.PlatformTraceHarness")
    args(platformFixture.absolutePath,platformPort.get().asFile.absolutePath)
    outputs.file(platformPort)
}
val verifyPlatformPairwise=tasks.register<Exec>("verifyPlatformPairwise") { dependsOn(platformSourceTrace,platformTrace);commandLine("python3",rootProject.file("tools/verify_platform_pairwise.py").absolutePath,platformSource.get().asFile.absolutePath,platformPort.get().asFile.absolutePath) }
...
val foundationPort=layout.buildDirectory.file("foundation/port.json")
...
val foundationTrace=tasks.register<JavaExec>("foundationTrace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.FoundationTraceHarness");args(foundationFixture.absolutePath,foundationPort.get().asFile.absolutePath);outputs.file(foundationPort)}
val verifyFoundationPairwise=tasks.register<Exec>("verifyFoundationPairwise"){dependsOn(foundationSourceTrace,foundationTrace);commandLine("python3",rootProject.file("tools/verify_foundation_pairwise.py").absolutePath,foundationSource.get().asFile.absolutePath,foundationPort.get().asFile.absolutePath)}
...
val modelRandomPort=layout.buildDirectory.file("model-random/port.json")
...
val modelRandomTrace=tasks.register<JavaExec>("modelRandomTrace") {
    ...
    mainClass.set("com.jojo.port.ModelRandomTraceHarness"); args(modelRandomFixture.absolutePath,modelRandomPort.get().asFile.absolutePath); outputs.file(modelRandomPort)
}
val verifyModelRandomPairwise=tasks.register<Exec>("verifyModelRandomPairwise") { dependsOn(modelRandomSourceTrace,modelRandomTrace); commandLine("python3",rootProject.file("tools/verify_model_random_pairwise.py").absolutePath,modelRandomSource.get().asFile.absolutePath,modelRandomPort.get().asFile.absolutePath) }
tasks.test { dependsOn(verifyModelRandomPairwise) }
val frameworkServiceFixture=rootProject.file("tools/framework_trace_cases.json");val frameworkServiceSource=layout.buildDirectory.file("framework-service/source.json");val frameworkServicePort=layout.buildDirectory.file("framework-service/port.json")
...
val miscUiPort=layout.buildDirectory.file("misc-ui/port.json")
...
val miscUiTrace=tasks.register<JavaExec>("miscUiTrace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.MiscUiTraceHarness");args(miscUiFixture.absolutePath,miscUiPort.get().asFile.absolutePath);outputs.file(miscUiPort) }
val verifyMiscUiPairwise=tasks.register<Exec>("verifyMiscUiPairwise") { dependsOn(miscUiSourceTrace,miscUiTrace);commandLine("python3",rootProject.file("tools/verify_misc_ui_pairwise.py").absolutePath,miscUiSource.get().asFile.absolutePath,miscUiPort.get().asFile.absolutePath) }
...
val progressionLayerPort=layout.buildDirectory.file("progression-layer/port.json")
...
val shopRewardPort=layout.buildDirectory.file("shop-reward/port.json")
...
val shopRewardTrace=tasks.register<JavaExec>("shopRewardTrace") {
    dependsOn(tasks.classes); classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.ShopRewardTraceHarness")
    args(shopRewardFixture.absolutePath,shopRewardPort.get().asFile.absolutePath); outputs.file(shopRewardPort)
}
val verifyShopRewardPairwise=tasks.register<Exec>("verifyShopRewardPairwise") {
    dependsOn(shopRewardSourceTrace,shopRewardTrace)
    commandLine("python3",rootProject.file("tools/verify_shop_reward_pairwise.py").absolutePath,shopRewardSource.get().asFile.absolutePath,shopRewardPort.get().asFile.absolutePath)
}
...
val shopRewardFullPort=layout.buildDirectory.file("shop-reward-full/port.json")
...
val shopRewardFullTrace=tasks.register<JavaExec>("shopRewardFullTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.ShopRewardTraceHarness")
    args(shopRewardFullFixture.absolutePath,shopRewardFullPort.get().asFile.absolutePath);outputs.file(shopRewardFullPort)
}
val verifyShopRewardFullPairwise=tasks.register<Exec>("verifyShopRewardFullPairwise") {
    ...
    commandLine("python3",rootProject.file("tools/verify_shop_reward_pairwise.py").absolutePath,shopRewardFullSource.get().asFile.absolutePath,shopRewardFullPort.get().asFile.absolutePath)
}
...
val gameDataPort=layout.buildDirectory.file("game-data/port.json")
...
val gameDataTrace=tasks.register<JavaExec>("gameDataTrace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.GameDataTraceHarness");args(gameDataFixture.absolutePath,gameDataPort.get().asFile.absolutePath);outputs.file(gameDataPort) }
val verifyGameDataPairwise=tasks.register<Exec>("verifyGameDataPairwise") { dependsOn(gameDataSourceTrace,gameDataTrace);commandLine("python3",rootProject.file("tools/verify_game_data_pairwise.py").absolutePath,gameDataSource.get().asFile.absolutePath,gameDataPort.get().asFile.absolutePath) }
...
val upgradeSkillPort=layout.buildDirectory.file("upgrade-skill/port.json")
...
val upgradeSkillTrace=tasks.register<JavaExec>("upgradeSkillTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.UpgradeSkillTraceHarness")
    args(upgradeSkillFixture.absolutePath,upgradeSkillPort.get().asFile.absolutePath);outputs.file(upgradeSkillPort)
}
val verifyUpgradeSkillPairwise=tasks.register<Exec>("verifyUpgradeSkillPairwise") {
    ...
    commandLine("python3",rootProject.file("tools/verify_upgrade_skill_pairwise.py").absolutePath,upgradeSkillSource.get().asFile.absolutePath,upgradeSkillPort.get().asFile.absolutePath)
}
...
tasks.register<Test>("itemUpgradeFlowTest") {
    ...
    classpath = sourceSets.test.get().runtimeClasspath
    useJUnitPlatform()
    filter { includeTestsMatching("com.jojo.port.ItemUpgradeFlowTest") }
}
...
tasks.register<Test>("loseSceneFlowTest") {
    ...
    classpath = sourceSets.test.get().runtimeClasspath
    useJUnitPlatform()
    filter { includeTestsMatching("com.jojo.port.LoseSceneFlowTest") }
}
...
val autoBattlePort=layout.buildDirectory.file("auto-battle/port.json")
...
val autoBattleTrace=tasks.register<JavaExec>("autoBattleTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.AutoBattleTraceHarness")
    args(autoBattleFixture.absolutePath,autoBattlePort.get().asFile.absolutePath);outputs.file(autoBattlePort)
}
val verifyAutoBattlePairwise=tasks.register<Exec>("verifyAutoBattlePairwise") {
    dependsOn(autoBattleSourceTrace,autoBattleTrace)
    commandLine("python3",rootProject.file("tools/verify_auto_battle_pairwise.py").absolutePath,autoBattleSource.get().asFile.absolutePath,autoBattlePort.get().asFile.absolutePath)
}
...
tasks.register<Test>("autoBattleFlowTest") {
    ...
    classpath = sourceSets.test.get().runtimeClasspath
    useJUnitPlatform()
    filter { includeTestsMatching("com.jojo.port.AutoBattleFlowTest") }
}
...
tasks.register<Test>("battleCommandFlowTest") {
    ...
    classpath = sourceSets.test.get().runtimeClasspath
    useJUnitPlatform()
    filter { includeTestsMatching("com.jojo.port.BattleCommandFlowTest") }
}
...
tasks.register<Test>("editAdminFlowTest") {
    ...
    useJUnitPlatform()
    filter {
        includeTestsMatching("com.jojo.port.BattleEditLayer2Test")
        includeTestsMatching("com.jojo.port.EditAdminFlowsTest")
    }
}
...
val battleBootstrapPort=layout.buildDirectory.file("battle-bootstrap/port.json")
...
val battleBootstrapTrace=tasks.register<JavaExec>("battleBootstrapTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.BattleBootstrapTraceHarness")
    args(battleBootstrapFixture.absolutePath,battleBootstrapPort.get().asFile.absolutePath);outputs.file(battleBootstrapPort)
}
val verifyBattleScenePortBehavior=tasks.register<Exec>("verifyBattleScenePortBehavior") {
    group = "isolated oracle"
    description = "Compares Battle.js event behavior through production BattleScenePort; not a screen-entry test."
    dependsOn(battleBootstrapSourceTrace,battleBootstrapTrace)
    inputs.file(rootProject.file("tools/verify_battle_scene_port_behavior.py"))
    commandLine("python3",rootProject.file("tools/verify_battle_scene_port_behavior.py").absolutePath,battleBootstrapSource.get().asFile.absolutePath,battleBootstrapPort.get().asFile.absolutePath)
}
...
val scenarioRuntimePort=layout.buildDirectory.file("scenario-runtime/port.json")
...
val scenarioRuntimeTrace=tasks.register<JavaExec>("scenarioRuntimeTrace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.ScenarioRuntimeTraceHarness");args(scenarioRuntimeFixture.absolutePath,scenarioRuntimePort.get().asFile.absolutePath);outputs.file(scenarioRuntimePort) }
...
/** Complete enumerable core/Config.js inventory, source factory versus direct Kotlin port. */
...
val configFullPort=layout.buildDirectory.file("config-full/port.json")
...
val configFullTrace=tasks.register<JavaExec>("configFullTrace") {
    dependsOn(tasks.classes); classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.ConfigFullTraceHarness")
    args(configFullFixture.absolutePath,configFullPort.get().asFile.absolutePath); outputs.file(configFullPort)
}
val verifyConfigFullPairwise=tasks.register<Exec>("verifyConfigFullPairwise") {
    dependsOn(configFullSourceTrace,configFullTrace)
    commandLine("python3",rootProject.file("tools/verify_config_full_pairwise.py").absolutePath,configFullSource.get().asFile.absolutePath,configFullPort.get().asFile.absolutePath)
}
...
val welcomePort=layout.buildDirectory.file("welcome/port.json")
...
val welcomeTrace=tasks.register<JavaExec>("welcomeTrace") {
    dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath
    mainClass.set("com.jojo.port.WelcomeTraceHarness")
    args(welcomeFixture.absolutePath,welcomePort.get().asFile.absolutePath);outputs.file(welcomePort)
}
val verifyWelcomePairwise=tasks.register<Exec>("verifyWelcomePairwise") {
    dependsOn(welcomeSourceTrace,welcomeTrace)
    commandLine("python3",rootProject.file("tools/verify_welcome_pairwise.py").absolutePath,welcomeSource.get().asFile.absolutePath,welcomePort.get().asFile.absolutePath)
}
...
val sendGiftsPort=layout.buildDirectory.file("send-gifts/port.json")
...
val sendGiftsTrace=tasks.register<JavaExec>("sendGiftsTrace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.SendGiftsTraceHarness");args(sendGiftsFixture.absolutePath,sendGiftsPort.get().asFile.absolutePath);outputs.file(sendGiftsPort) }
val verifySendGiftsPairwise=tasks.register<Exec>("verifySendGiftsPairwise") { dependsOn(sendGiftsSourceTrace,sendGiftsTrace);commandLine("python3",rootProject.file("tools/verify_send_gifts_pairwise.py").absolutePath,sendGiftsSource.get().asFile.absolutePath,sendGiftsPort.get().asFile.absolutePath) }
tasks.test { dependsOn(verifySendGiftsPairwise) }
val progress2Fixture=rootProject.file("tools/progress2_trace_cases.json");val progress2Source=layout.buildDirectory.file("progress2/source.json");val progress2Port=layout.buildDirectory.file("progress2/port.json")
...
val progress2Trace=tasks.register<JavaExec>("progress2Trace"){dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.Progress2TraceHarness");args(progress2Fixture.absolutePath,progress2Port.get().asFile.absolutePath);outputs.file(progress2Port)}
val verifyProgress2Pairwise=tasks.register<Exec>("verifyProgress2Pairwise"){dependsOn(progress2SourceTrace,progress2Trace);commandLine("python3",rootProject.file("tools/verify_progress2_pairwise.py").absolutePath,progress2Source.get().asFile.absolutePath,progress2Port.get().asFile.absolutePath)}
tasks.test { dependsOn(verifyProgress2Pairwise) }

/** Model.js isolated source/port contracts: state, persistence and lifecycle. */
fun modelGate(group:String) {
    val fixture=rootProject.file("tools/model_${group}_trace_cases.json")
    val source=layout.buildDirectory.file("model-$group/source.json"); val port=layout.buildDirectory.file("model-$group/port.json")
    ...
    val portTask=tasks.register<JavaExec>("model${group.replaceFirstChar { it.uppercase() }}Trace") { dependsOn(tasks.classes);classpath=sourceSets.main.get().runtimeClasspath;mainClass.set("com.jojo.port.ModelTraceHarness");args(fixture.absolutePath,port.get().asFile.absolutePath,group);outputs.file(port) }
    val gate=tasks.register<Exec>("verifyModel${group.replaceFirstChar { it.uppercase() }}Pairwise") { dependsOn(sourceTask,portTask);commandLine("python3",rootProject.file("tools/verify_model_pairwise.py").absolutePath,source.get().asFile.absolutePath,port.get().asFile.absolutePath,group) }
    tasks.test { dependsOn(gate) }
}
modelGate("state");modelGate("persistence");modelGate("lifecycle")
val headFixture=rootProject.file("tools/head_trace_cases.json");val headSource=layout.buildDirectory.file("head/source.json");val headPort=layout.buildDirectory.file("head/port.json")
...
tasks.register<Test>("titleInteractionContractTest") {
    ...
    filter {
        ...
        includeTestsMatching("com.jojo.port.TitleInteractionTest")
        includeTestsMatching("com.jojo.port.SettingLayerTest")
        includeTestsMatching("com.jojo.port.ProgressLoadingLayerTest")
    }
}
