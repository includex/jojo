...
class BattleCameraTest {
    ...
    @Test fun `source authored slot offsets come from Config battle partitions`() {
        assertEquals(0, SourceBattleSlots.indexFor(BattleUnit("mine-0", "mine", Faction.PLAYER, 0, 0)))
        assertEquals(40, SourceBattleSlots.indexFor(BattleUnit("friend-0", "friend", Faction.FRIEND, 0, 0)))
        assertEquals(60, SourceBattleSlots.indexFor(BattleUnit("enemy-0", "enemy", Faction.ENEMY, 0, 0)))
        assertEquals(60, SourceBattleSlots.indexFor(BattleUnit("enemy-0", "reinforcement", Faction.REINFORCEMENTS, 0, 0)))
    }
    ...
    @Test fun `source force center uses tile position rather than unit node half tile`() {
        ...
        // BattleLayer.center(9,14) => (960-9*96, 14*96-960).
