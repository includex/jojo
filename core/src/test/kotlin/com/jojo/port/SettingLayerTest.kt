...
class SettingLayerTest {
 ...
 @Test fun `optional source buttons keep support and scene gates on normal setting state machine`() {
  val achievements=mapOf(1 to listOf<Any>("첫 전투",2,30,5))
  ...
  val hall=layer("Hall",8);hall.onCreate();assertEquals(SettingLayer.FeatureResult.Opened("AchievementsLayer"),hall.featureButton(7,2));assertIs<AchievementsLayerPort>(hall.activeFeature);assertEquals(SettingLayer.FeatureResult.Opened("RaffleLayer"),hall.featureButton(8,2));assertIs<RaffleLayerPort>(hall.activeFeature);assertEquals(SettingLayer.FeatureResult.Opened("SignInLayer"),hall.featureButton(9,2));assertIs<SignInLayerPort>(hall.activeFeature)
 }
}
