...
class GamePreferenceProviderTest {
    ...
    fun `automated stores are shared by name within one game and never delegate to disk`() {
        ...
        val campaign = provider.get("jojo-original-campaign")
        campaign.putString("state", "isolated").flush()

        assertSame(campaign, provider.get("jojo-original-campaign"))
        assertEquals("isolated", provider.get("jojo-original-campaign").getString("state"))
        assertNotSame(campaign, provider.get("jojo-original-settings"))
    ...
    fun `campaign writes stay inside the automated game store`() {
        ...
        val campaign = CampaignStore(provider.get("jojo-original-campaign"))
    ...
    fun `interactive games retain and share platform preferences`() {
        ...
        val settings = provider.get("jojo-original-settings")
        settings.putInteger("GAME_SETTING", 15).flush()

        assertSame(settings, provider.get("jojo-original-settings"))
        assertSame(disk.getValue("jojo-original-settings"), settings)
        assertEquals(15, provider.get("jojo-original-settings").getInteger("GAME_SETTING"))
    }
}
