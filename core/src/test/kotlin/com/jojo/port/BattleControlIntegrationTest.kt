...
class BattleControlIntegrationTest {
    ...
    fun `Control AI excludes same camp units from enemy strategy splash scoring`() {
        val moraleDown = OriginalGameData.MagicProfile(
            id = 25, name = "탈진", type = 10, target = 0,
            hitArea = OriginalGameData.HitAreaProfile(0, setOf(0 to 1)), effectAreaId = 0,
    ...
    fun `friend magic does not receive enemy-only minimum damage`() {
        val weakMagic = OriginalGameData.MagicProfile(
    ...
    fun `designated follow AI becomes passive when already within source distance`() {
        val battle = Battle(
            units = listOf(
                BattleUnit("leader", "주장", Faction.ENEMY, 1, 0, sourceCharacterId = 9, ai = 2),
    ...
    @Test
    fun `BattleLayer ai2 holds confused unit before Control process1`() {
        val battle = Battle(
            units = listOf(
                BattleUnit("player", "아군", Faction.PLAYER, 0, 0),
    ...
    fun `designated attack AI applies source GJZDWJ target bonus`() {
        val battle = Battle(
            units = listOf(
                BattleUnit("first", "첫 대상", Faction.PLAYER, 0, 0, hitPoints = 500, maxHitPoints = 500, sourceCharacterId = 1),
                BattleUnit("designated", "지정 대상", Faction.PLAYER, 2, 0, hitPoints = 500, maxHitPoints = 500, sourceCharacterId = 2),
        ...
        val battle = Battle(
            units = listOf(
                BattleUnit(
                    ...
                    sourceCharacterId = 117, terrainMovementCosts = mapOf(0 to 1),
                ),
                BattleUnit(
                    ...
                    sourceCharacterId = 236, terrainMovementCosts = mapOf(0 to 1),
                ),
            ),
            events = emptyList(),
            terrain = terrain,
        ...
        val battle = Battle(
            units = listOf(
                ...
                BattleUnit(
                    ...
                    sourceCharacterId = 211, terrainMovementCosts = mapOf(0 to 1),
                ),
            ),
            events = emptyList(),
            terrain = terrain,
