...
class BattleAttackSequenceTest {
    ...
    fun `ZYSH redirects landed harm after primary and before splash settlement`() {
        ...
        assertEquals(0, targets[0].sourceHarm)
        assertTrue(targets[1].sourceHarm > 0)
    ...
    @Test
    fun `BJBLJ critical changes BattleLayer attack loop from one pass to two`() {
        val battle = Battle(
            units = listOf(
    ...
    fun `critical speech follows source checkCrit alternating gate per attacker`() {
        val speech = OriginalGameData.CriticalSpeechProfile(listOf("필살 대사"), randomized = false)
        val battle = Battle(
            units = listOf(
                BattleUnit("attacker", "필살", Faction.PLAYER, 0, 0, morale = 1,
        ...
        val battle = Battle(
            units = listOf(
                BattleUnit("attacker", "필살", Faction.PLAYER, 0, 0,
                    ...
                    criticalSpeech = OriginalGameData.CriticalSpeechProfile(
    ...
    fun `physical and magic criticals share one source checkCrit counter`() {
        val magic = OriginalGameData.MagicProfile(
            10, "회오리", 3, 0, OriginalGameData.HitAreaProfile(0, setOf(1 to 0)),
        ...
        val battle = Battle(
            units = listOf(
                BattleUnit("attacker", "필살", Faction.PLAYER, 0, 0, spirit = 100, morale = 100,
                    ...
                    criticalSpeech = OriginalGameData.CriticalSpeechProfile(listOf("공용 필살"), randomized = false)),
    ...
    @Test
    fun `BattleLayer truncUnitData seeds all eight source rate gauges for initial and added units`() {
        ...
        battle.initializeAllSourceRates()
    ...
    fun `CLFJ performs configured strategy counter before physical counter without consuming defender action`() {
        val counterMagic = OriginalGameData.MagicProfile(
    ...
    fun `CLFJ remains queued after an active miss and retains its zero harm attack target`() {
        ...
        assertEquals(0, result.physicalPasses.single().targets.single().sourceHarm)
