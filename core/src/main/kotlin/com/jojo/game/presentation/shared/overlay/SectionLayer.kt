...
class SectionLayer(private val setting: Int) {
    ...
    fun onCreate(idx: Int, name: String, callback: () -> Unit): View {
        this.index = idx; this.name = name; fn = callback; label =
            if (idx == 0) SystemMessage.S_9CA0190DEC else chapter(idx); if (setting and AUTO_CLOSE != 0) scheduled += 3; return view()
    }
    ...
    private fun chapter(n: Int): String {
        val r = listOf(SystemMessage.S_BDA0208A5C, SystemMessage.S_06CF3E90DE, "2", SystemMessage.S_59463E84FB, SystemMessage.S_CA5EEA5B52, SystemMessage.S_FBBD1D1816, SystemMessage.S_959F80BB16, SystemMessage.S_4259DD769C, SystemMessage.S_C6D6E215B1, SystemMessage.S_E8B701A283)
        var i = n
        var v = SystemMessage.S_22EF795E23; while (i > 0) {
            v = r[i % 10] + v; i /= 10
        }; return "제$v"
    }
