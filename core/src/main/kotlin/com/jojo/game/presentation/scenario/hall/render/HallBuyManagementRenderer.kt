...
internal object HallBuyManagementRenderer {
    ...
    fun draw(assets: ScenarioSceneAssets, batch: SpriteBatch, view: HallBuyManagementRenderView) {
        ...
        val layout=GlyphLayout(assets.titleFont,GameText.S_D1379C024C); assets.titleFont.color=Color.BLACK; assets.titleFont.draw(batch,layout,x+(w-layout.width)/2f,y+h-5f)
        ...
        label(GameText.S_6102409B7F,x+22f,y+23f,240f); label(view.money,x+170f,y+23f,140f,true)
        listOf(Triple(GameText.S_CAFDC61BBF,530.78f,120.4f),Triple(GameText.S_B38CAAEF34,678.70f,146.2f),Triple(GameText.S_D4B491F164,838.66f,146.2f)).forEach { (text,bx,bw) -> patch("button",9)?.draw(batch,bx,36.98f,bw,43f); label(text,bx,67.98f,bw,true) }
