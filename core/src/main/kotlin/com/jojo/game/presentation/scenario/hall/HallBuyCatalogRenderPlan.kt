...
internal object HallBuyCatalogRenderPlan {
    ...
    fun commands(view: HallBuyCatalogView): List<HallBuyCatalogDrawCommand> = buildList {
        ...
        text(SystemMessage.S_EA6F893483, 193.72f, 595.19f, 240f, align = Align.left)
        button(SystemMessage.S_256C17D85E, 183.25f, 521.28f, 154.8f)
        button(SystemMessage.S_9F66DED326, 337.85f, 521.28f, 154.8f)
    ...
    private fun MutableList<HallBuyCatalogDrawCommand>.equipmentRows(rows: List<HallBuyCatalogRowView>) {
        rows.forEachIndexed { index, row ->
            ...
            text(SystemMessage.S_E10C69D0A1, 475.86f, y + 142f, 80f, align = Align.left)
            text("1", 562.72f, y + 142f, 42f, align = Align.left)
            text(SystemMessage.S_991BD0CC20, 283.10f, y + 97f, 90f, align = Align.left)
            text(row.typeName, 371.68f, y + 97f, 90f, align = Align.left)
            text(SystemMessage.S_EF2ACA14AE, 193.70f, y + 46f, 112f, align = Align.left)
            text(row.inventory.toString(), 280.56f, y + 46f, 44f, align = Align.left)
            text(SystemMessage.S_21E6F82F77, 338.62f, y + 46f, 80f, align = Align.left)
            text(row.total.toString(), 425.48f, y + 46f, 44f, align = Align.left)
            text(SystemMessage.S_C61005675A, 475.86f, y + 46f, 80f, align = Align.left)
    ...
    private fun MutableList<HallBuyCatalogDrawCommand>.propertyRows(rows: List<HallBuyCatalogRowView>) {
        rows.forEachIndexed { index, row ->
            ...
            text(SystemMessage.format(SystemMessage.Key.INVENTORY_COUNT, row.inventory), 445f, y + 73f, 165f, align = Align.left)
            text(SystemMessage.format(SystemMessage.Key.PRICE, row.price), 338f, y + 30f, 220f, align = Align.left)
        }
    }
