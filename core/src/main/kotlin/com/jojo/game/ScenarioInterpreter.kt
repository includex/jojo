...
/** Recovered BattleScreen._testUnitType's exact 0..6 selector table. */
 ...
 */
class ScenarioInterpreter private constructor(
    ...
    /** Source BattleScreen.loadBg pauses until its resource callback tail. */
    ...
    /** True from `loadBg`'s pause until BattleScreen reports its map/avatars ready. */
    val hasPendingBattleBackgroundLoad: Boolean get() = pendingBattleBackgroundLoadIndex != null
    /** Requested HM index, including BattleScreen.loadBg's JUMP_OFFSET adjustment. */
     ...
     * so this barrier is opted into by the production BattleScreen call site.
     */
    private var dialogueCallbackFramePending = false
    private var dialogueCallbackReturnState: PlaybackState? = null
    /** Native BattleScreen say4 temporarily borrows the ordinary SayLayer. */
    ...
    /** Exact BattleScreen.unit(t, flags) routing, exposed internally for contract tests. */
    ...
    /** Presents BattleScreen.say4 through the same dialogue renderer/input path as stage.say. */
    ...
    /** BattleScreen.getItem opens InfoLayer before its final Script.resume callback. */
    ...
    /**
     * Every FightLayer command calls BattleScreen.pause() and resumes from its
    ...
    private fun invokeCall(node: JsonValue, frame: Frame): Any? {
        ...
        when (path) {
            "stage.say" -> {
                ...
                // BattleScreen.say routes text without a leading speaker token
            ...
            "stage.talk" -> {
                // BattleScreen.talk(primary, fallback, text) chooses the first
            ...
            "stage.loadBg" -> {
                ...
                // BattleScreen._loadBg(t) interprets its sole argument as an
                ...
                if (moduleName.startsWith("S_")) {
                    // BattleScreen.loadBg consumes JUMP_OFFSET before its
            ...
            "stage.showWinCondition" -> {
                // Original BattleScreen.showWinCondition pauses the Script
            ...
            "stage.reward" -> {
                ...
                // BattleScreen.reward pauses the Script until RewardLayer's
            ...
            // BattleScreen.center mutates ScrollView.content.position and
            ...
            "stage.setFire" -> {
                ...
                // BattleScreen.setObject2 centers every newly drawn fire and
            ...
            "stage.playMagicMeff" -> {
                ...
                if (externalBattlePresentation) {
                    ...
                    // BattleScreen owns that FINISHED callback and resumes.
            ...
            "stage.unit" -> {
                ...
                // BattleScreen.unit(t, flags) switches from character-ID
    ...
    private fun totalRectangleUnits(args: List<Any?>): Int {
        ...
        // BattleScreen.totalRectUnit uses isExist(): hidden and HP-zero actors
    ...
    private fun positionsForFilterSelector(selector: Int): List<Pair<Int, Int>> = when (selector) {
        // BattleScreen._filterUnit selectors: all, mine side, enemy side, and
    ...
    companion object {
        ...
        /** Config.GLOBAL_VAR.JUMP_OFFSET, consumed by BattleScreen.loadBg. */
