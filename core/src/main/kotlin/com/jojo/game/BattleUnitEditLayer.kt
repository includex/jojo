...
class BattleUnitEditLayer(initialAttack: Int = 50, initialPosts: Int = 0) {
    ...
    fun editAttack(value: Int) { pendingAttack = value }
    fun openPosts() { postsPanelVisible = true }
    fun closePosts() { postsPanelVisible = false }
    fun selectPosts(value: Int) { pendingPosts = value }
...
object BattleUnitEditRenderEvents {
    ...
    fun jsonl(route: BattleUnitEditRoute): String {
        ...
        if (route == BattleUnitEditRoute.MUTATION) return RenderEventLog().apply {
            draw("hall-battle-edit-mutation-stable", "HallLayer", "Canvas/Layer/map", "sprite", 0f, 0f, 1488.372f, 800f,
        ...
        decode(blob).lineSequence().filter { it.isNotEmpty() }.forEach { row ->
            ...
            val blend:Any=if(v[9].contains("SRC_ALPHA")) listOf("SRC_ALPHA","ONE_MINUS_SRC_ALPHA") else listOf(770,771)
            log.draw(phase,v[0],v[1],v[2],v[3].toFloat(),v[4].toFloat(),v[5].toFloat(),v[6].toFloat(),v[7].ifEmpty{null},v[8].toFloat(),blend,v[10].toBoolean(),v[11])
        }
        return log.jsonl()
    }

    private fun decode(value:String):String=GZIPInputStream(ByteArrayInputStream(Base64.getDecoder().decode(value))).bufferedReader().readText()

    private fun avatarJsonl(): String {
        val log=RenderEventLog();val phase="hall-battle-edit-avatar-stable";val alpha=listOf("SRC_ALPHA","ONE_MINUS_SRC_ALPHA")
        fun d(path:String,type:String,x:Float,y:Float,w:Float,h:Float,asset:String?=null,text:String="",opacity:Float=1f,visible:Boolean=true,blend:Any=listOf(770,771))=
            log.draw(phase,"HallLayer",path,type,x,y,w,h,asset,opacity,blend,visible,text)
        d("Canvas/Layer/map","sprite",0f,0f,1488.372f,800f,"assets/Game/native/c6/c6b7d3e4-8590-4fb6-85a5-7967e64abc3e.8e84f.jpg#<unnamed-frame>")
        ...
        d("Canvas/Layer/Logo_12-1/bg1/label","label",623.186f,721.3f,231.83f,52.4f,text="전투 외형 수정",blend=alpha)
        d("Canvas/Layer/Logo_12-1/scrollview1","tiled-sprite",162.186f,92f,424f,618f,"Logo_12-1")
        d("Canvas/Layer/Logo_12-1/scrollview1/scrollBar","sliced-sprite",574.186f,92f,12f,618f,"assets/resources/native/61/617323dd-11f4-4dd3-8eec-0caf6b3b45b9.6d707.png#default_scrollbar_vertical_bg",opacity=0f,visible=false)
        d("Canvas/Layer/Logo_12-1/scrollview1/scrollBar/bar","sliced-sprite",575.186f,401f,10f,30f,"assets/resources/native/d6/d6d3ca85-4681-47c1-b5dd-d036a9d39ea2.d55c2.png#default_scrollbar_vertical",opacity=0f,visible=false)
        ...
        d("Canvas/Layer/Logo_12-1/scrollview2/scrollBar","sliced-sprite",1320.686f,92f,12f,618f,"assets/resources/native/61/617323dd-11f4-4dd3-8eec-0caf6b3b45b9.6d707.png#default_scrollbar_vertical_bg",opacity=0f,visible=false)
        d("Canvas/Layer/Logo_12-1/scrollview2/scrollBar/bar","sliced-sprite",1321.686f,401f,10f,30f,"assets/resources/native/d6/d6d3ca85-4681-47c1-b5dd-d036a9d39ea2.d55c2.png#default_scrollbar_vertical",opacity=0f,visible=false)
        ...
        buttons.forEachIndexed{i,(r,text)->d("Canvas/Layer/Logo_12-1/button$i/Background","sliced-sprite",r[0],r[1],r[2],r[3],"box3");d("Canvas/Layer/Logo_12-1/button$i/Background/Label","label",r[4],r[5],r[6],r[7],text=text,blend=alpha)}
        return log.jsonl()
    }
}
...
class BattleUnitEditRouteScreen(private val game:JojoGame,private val route:BattleUnitEditRoute):ScreenAdapter(){
    private val shapes=ShapeRenderer();private val entry=HallBattleUnitEditRoute(true);private val edit=BattleUnitEditLayer();private var installed=false
    override fun render(delta:Float){
        if(!installed){check(entry.selectUnit(true));check(entry.unitInfoButton(10,true));when(route){
            BattleUnitEditRoute.POSTS->edit.openPosts()
            BattleUnitEditRoute.MUTATION->{edit.editAttack(77);check(edit.button(0).contains(BattleUnitEditLayer.Effect.SetAttack(77)));entry.close()}
            BattleUnitEditRoute.AVATAR->check(edit.button(2,true).contains(BattleUnitEditLayer.Effect.OpenAvatarEditor))
            else->{}}
            installed=true
        }
