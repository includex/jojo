...
object HallMoveTimeline {
    ...
    fun segments(path: List<Pair<Int, Int>>): List<Segment> {
        ...
        path.drop(1).forEach { point ->
            ...
            if (nextDirection == direction) {
                ...
                if (direction >= 0) {
                    ...
                    result += Segment(segmentStart.first.toFloat(), segmentStart.second.toFloat(), point.first.toFloat(), point.second.toFloat(), time, duration, direction)
                    time += duration
                    segmentStart = point
                }
        ...
        val duration = .04f * count
        result += Segment(segmentStart.first.toFloat(), segmentStart.second.toFloat(), previous.first.toFloat(), previous.second.toFloat(), time, duration, direction)
        return result
    }
    ...
    fun sample(path: List<Pair<Int, Int>>, elapsed: Float): Sample {
        ...
        if (segments.isEmpty()) {
            ...
            return Sample(point.first.toFloat(), point.second.toFloat(), -1, z(point.first.toFloat(), point.second.toFloat()))
        }
        fun positionAt(time: Float): Triple<Float, Float, Int> {
            ...
            val progress = if (segment.duration <= 0f) 1f else ((time - segment.startsAt) / segment.duration).coerceIn(0f, 1f)
            return Triple(
