...
object FrameworkServiceTraceHarness {
 ...
 @JvmStatic fun main(a:Array<String>){val out=cases(Files.readString(Path.of(a[0]))).joinToString(",","{","}"){c->val id=str(c,"id");if(str(c,"kind")=="service"){var calls=0;val routes=mutableListOf<String>();val p=ServiceFlow({routes+="skm"},{calls++});val z=str(c,"event").split(':');p.touch(z[0].toInt(),z[1].toInt());q(id)+":[{\"kind\":\"service\",\"attached\":${p.attached},\"layers\":${if(routes.isEmpty())"[]" else "[[\"skm\",4]]"},\"calls\":$calls}]"}else{val p=ServiceMenuState(num(c,"flag"));p.touch(num(c,"event"));q(id)+":[{\"kind\":\"skm\",\"attached\":${p.attached},\"visible\":${p.visible.joinToString(",","[","]")}}]"}};Files.createDirectories(Path.of(a[1]).parent);Files.writeString(Path.of(a[1]),out)}
}
