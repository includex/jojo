...
object RaffleGateRenderEvents {
    ...
    fun jsonl(): String = RenderEventLog().apply {
        ...
        fun e(path:String,type:String,x:Float,y:Float,w:Float,h:Float,asset:String?=null,text:String="",opacity:Float=1f,blend:Any=listOf(770,771)) =
            d("SettingLayer",path,type,x,y,w,h,asset,text,opacity,blend)
        fun label(path:String,text:String,x:Float,y:Float,w:Float,h:Float=50.4f)=e(path,"label",x,y,w,h,text=text,blend=alpha)
        ...
        d("HallLayer","Canvas/Layer/Panel_cancel","sprite",0f,0f,1488.372f,800f,"default_sprite_splash",opacity=.118f)
        ...
        label("Canvas/Layer/bg/scrollview/view/content/label","항목을 클릭하여 설정해 주세요. 설정 완료 후 [확인]을 선택해 주세요.",65.851f,650.189f,1078.67f)
        ...
        toggles.forEachIndexed { i,(r,text,checked) ->
            ...
            if(checked)e("$p/checkmark","sprite",if(i==2)218.186f else r[0],r[1],28f,28f,"default_toggle_checkmark")
            label("$p/Label",text,r[2],r[1]-6f,526f,40f)
        }
        fun panel(i:Int,y:Float,bx:Float,by:Float,bw:Float,title:String){val p="Canvas/Layer/bg/scrollview/view/content/panel$i";e(p,"sliced-sprite",793.336f,y,479.7f,100f,"box1");e("$p/bg1","sprite",if(i==3)833.325f else 831.428f,by+.2f,if(i==3)210f else 166f,50f,"bg1");label("$p/bg1/label",title,bx,by,bw)}
        fun radios(i:Int,y:Float,selected:Int,values:List<String>){val xs=floatArrayOf(818.346f,974.286f,1119.419f);val lx=floatArrayOf(855.731f,1024.671f,1169.804f);values.forEachIndexed{j,v->val p="Canvas/Layer/bg/scrollview/view/content/panel$i/toggleContainer/toggle$j";e("$p/Background","sprite",xs[j],y,32f,32f,"default_radio_button_off");if(j==selected)e("$p/checkmark","sprite",xs[j],y,32f,32f,"default_radio_button_on");label("$p/Label",v,lx[j],y-4f,90f,40f)}}
        ...
        e("Canvas/Layer/bg/scrollview/view/content/panel1/slider/Background","sliced-sprite",816.186f,418.346f,434f,20f,"default_scrollbar")
        e("Canvas/Layer/bg/scrollview/view/content/panel1/slider/Handle","sliced-sprite",800.186f,412.346f,32f,32f,"default_radio_button_off")
        ...
        e(p,"sliced-sprite",793.336f,81.389f,479.7f,142f,"box1");e("$p/bg1","sprite",833.325f,198.167f,210f,50f,"bg1");label("$p/bg1/label","대화창 색상",846.27f,197.967f,184.11f)
        floatArrayOf(831.12f,933.12f,1035.12f,1137.12f).forEachIndexed{i,x->val q="$p/item$i";e(q,"sliced-sprite",x,92.703f,100f,100f,"box1");e("$q/Logo_9-1","sprite",x+2f,94.703f,96f,96f,"Logo_${9+i}-1");if(i==0)e("$q/box6","tiled-sprite",x,92.703f,100f,100f,"box6")}
...
class RaffleGateRouteScreen(private val game: JojoGame) : ScreenAdapter() {
    ...
    override fun render(delta:Float){
        if(!entered){route.openHallMenu(true);route.hallMenuButton(3,true);route.settingButton(8,true,0);check(route.view().layer==RaffleGateRoute.Layer.SETTING);entered=true}
        Gdx.gl.glClearColor(0f,0f,0f,1f);Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT)
        shapes.begin(ShapeRenderer.ShapeType.Filled);shapes.color=Color(.25f,.22f,.17f,1f);shapes.rect(0f,0f,1488.372f,800f);shapes.color=Color(.72f,.67f,.55f,1f);shapes.rect(195.686f,41f,1097f,718f);shapes.end()
