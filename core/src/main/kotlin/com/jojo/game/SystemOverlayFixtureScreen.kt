...
class SystemOverlayFixtureScreen(private val game: JojoGame, private val state: String) : ScreenAdapter() {
    ...
    private val font: BitmapFont = KoreanFont.create(34, "저장 완료.게임 저장하시겠습니까?예비원본 공통 알림 UI 비교자원 로딩 중이 완료되면 접속하는 것이 빠를 거예요!")
    ...
    private fun drawSpinner(x: Float, y: Float, size: Float, angle: Float) {
        batch.draw(loadingTexture, x, y, size / 2f, size / 2f, size, size, 1f, 1f, angle,
    ...
    fun renderEventLog(): String {
        val log = RenderEventLog(); val phase = "hall-$state-stable"
        fun draw(layer: String, path: String, type: String, x: Float, y: Float, w: Float, h: Float,
            ...
            log.draw(phase, layer, path, type, x, y, w, h, asset, opacity = opacity,
        ...
        progress?.let { model ->
            ...
            draw("HallLayer", "Canvas/Layer/label_tips", "label", 294.186f, 260.861f, 900f, 51.36f,
                text = ProgressRenderOracle.TIPS)
            return log.jsonl()
        }
        loading?.let { model ->
            draw("HallLayer", "Canvas/Layer/cancel_panel", "sprite", 0f, 0f, 1488.372f, 800f,
            ...
            if (model.imageVisible) draw("LoadingLayer", "Canvas/Layer/bg/img", "sprite", 709.186f, 365f, 70f, 70f, "uiloading")
            return log.jsonl()
        }
        if (toast != null) {
            draw("HallLayer", "Canvas/Layer/sprite_bg", "sprite", 285.686f, 367f, 917f, 66f,
                "assets/main/native/54/54f14842-1332-459f-b7aa-cb75067d180c.7637b.png#img_bg2")
            draw("HallLayer", "Canvas/Layer/sprite_bg/label", "label", 272.226f, 370.8f, 943.92f, 58.4f, text = toast.text)
            return log.jsonl()
        }
        draw("HallLayer", "Canvas/Layer/Panel_cancel", "sprite", 0f, 0f, 1488.372f, 800f,
        ...
        if (model.flag and 2 != 0) {
            draw("MsgBox", "Canvas/Layer/bg0/btns/button1/Background", "sliced-sprite", 554.186f, 271.285f, 180f, 50f, "box3")
            draw("MsgBox", "Canvas/Layer/bg0/btns/button1/Background/Label", "label", 557.336f, 279.085f, 168.1f, 40f, text = "비")
            draw("MsgBox", "Canvas/Layer/bg0/btns/button0/Background", "sliced-sprite", 754.186f, 271.285f, 180f, 50f, "box3")
            draw("MsgBox", "Canvas/Layer/bg0/btns/button0/Background/Label", "label", 757.586f, 279.085f, 169.4f, 40f, text = "예")
        } else {
            draw("MsgBox", "Canvas/Layer/bg0/btns/button0/Background", "sliced-sprite", 654.186f, 271.285f, 180f, 50f, "box3")
            draw("MsgBox", "Canvas/Layer/bg0/btns/button0/Background/Label", "label", 657.586f, 279.085f, 169.4f, 40f, text = "예")
        }
        return log.jsonl()
    }

    override fun resize(width: Int, height: Int) = viewport.update(width, height, true)
    override fun dispose() { batch.dispose(); shapes.dispose(); font.dispose(); background.dispose(); logo9.dispose(); logo3.dispose(); loadingTexture.dispose(); boxTexture.dispose() }
}
...
private class ProgressRenderOracle {
    ...
    fun onProgress(value: Float) { progress = value }
    companion object { const val TIPS = "자원 로딩이 완료되면 게임에 접속하는 것이 빠를 거예요!" }
}
