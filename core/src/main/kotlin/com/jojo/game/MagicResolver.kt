...
internal object MagicResolver {
    ...
    fun castMagic(
        ...
        if (!targetsAny && ((targetsAllies && !env.areAllied(attacker, target)) || (!targetsAllies && env.areAllied(attacker, target)))) {
        ...
        val resultPasses = buildList {
            repeat(repeatCount) { pass ->
                ...
                add(affectedUnits.map { victim ->
                    val (result, entry) = MagicTargetResolver.resolveTarget(pass, attacker, victim, magic, magicCritical, env)
