...
internal object ScenarioConditionEvaluator {
    ...
    fun isInRectangle(args: List<Any?>, env: ScenarioConditionEnvironment): Boolean {
        ...
        val positions = if (target >= 1024) positionsForFilterSelector(target, env) else listOfNotNull(env.battleContext.positions[target])
    ...
    fun positionsForFilterSelector(selector: Int, env: ScenarioConditionEnvironment): List<Pair<Int, Int>> = when (selector) {
        ...
        1027 -> env.battleContext.clickedCharacterId?.let { env.battleContext.positions[it] }?.let(::listOf).orEmpty()
        else -> emptyList()
    }
