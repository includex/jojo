...
/** Exact state implementation of ui/LoadLayer.js. */
class LoadLayer {
    data class View(val labelActive: Boolean, val label: String, val anime: String)
    fun onCreate(text: String?): View = View(text != null, text ?: "", "rotateBy:2:360:repeatForever")
}


/** Actual inherited UILayer.getSystemTime loading lifecycle. */
class ModalLoadProductionRoute {
    var attached = false; private set
    var text: String? = null; private set
    fun getSystemTimeStarted() {
        attached = true
        text = "검증 중……"
    }
...
object ModalLoadRenderEvents {
    ...
    fun jsonl(): String {
        ...
        table.lineSequence().filter { it.isNotEmpty() }.forEach { row ->
            ...
            log.draw("login-modal-load", v[0], v[1], v[2], v[3].toFloat(), v[4].toFloat(),
