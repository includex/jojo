...
class ScenarioDialogueCoordinator(
    ...
    fun advanceDialogue(deferCloseCallbackFrame: Boolean = false, currentState: PlaybackState) {
        check(currentState == PlaybackState.DIALOGUE) { SystemMessage.S_C80F942461 }
    ...
    fun presentExternalBattleDialogue(dialogue: Dialogue, currentState: PlaybackState) {
        check(currentDialogue == null && currentState != PlaybackState.DIALOGUE) { SystemMessage.S_14244CA6EE }
        check(externalDialogueReturnState == null) { SystemMessage.S_ECA779BF1F }
