...
class ScenarioScreen(
    ...
    private fun hallSellCandidates(): List<Map.Entry<Int, Int>> = campaign.inventory.items.entries
        ...
        .filter { (id, _) -> gameDataCatalog.equipmentProfile(id)?.let(gameDataCatalog::equipmentCategory)?.let { category ->
    ...
    override fun render(delta: Float) {
            ...
            when (game.requestedCaptureState()) {
                ...
                "hall-info-fixture", "hall-get-item-equipment-fixture", "hall-get-item-property-fixture", "hall-item-equipment-fixture", "hall-item-property-fixture", "hall-item-discard-confirm-fixture", "hall-choice-fixture", "hall-map-info-fixture", "hall-ambition-fixture", "hall-ask-fixture", "hall-command-fixture", "hall-menu-fixture", "hall-save-fixture", "hall-save-confirm-fixture", "hall-equip-fixture", "hall-unit-list-fixture", "hall-unit-list-select-fixture", "hall-unit-list-close-fixture", "hall-equip-confirm-fixture", "hall-equip-confirm-unload-fixture", "hall-exclusive-fixture", "hall-exclusive-tab1-fixture", "hall-magic-fixture", "hall-feats-fixture", "hall-feats-help-fixture", "hall-buy-fixture", "hall-sell-fixture", "hall-forces-fixture", "hall-property-fixture", "hall-terrain-fixture", "hall-treasure-fixture", "hall-helper-fixture", "hall-skip-open-fixture" -> {
                    ...
                    if (hallOverlayFixture == "magic") {
                        val profile = requireNotNull(gameDataCatalog.allMagicProfiles().firstOrNull { it.name == "회오리" })
        ...
        playback.update(delta, autoCloseUi = settingsPreferences.getInteger(
        ...
        if (branchVerifyMode && elapsed > 0.8f) {
            ...
            Gdx.app.log("JojoGame", "VERIFY_BRANCH_OK: R_00 first choice selected and sel == 1 dialogue branch executed")
            Gdx.app.exit()
        }
        if (alternateBranchVerifyMode && elapsed > 0.8f) {
            ...
            Gdx.app.log("JojoGame", "VERIFY_BRANCH_2_OK: R_00 second choice selected and sel == 2 dialogue branch executed")
            Gdx.app.exit()
        }
    ...
    private fun advance() {
        when (playback.state) {
            ...
            PlaybackState.MODAL -> {
                ...
                if (playback.currentModalKind in setOf(ScenarioInterpreter.ModalKind.INFO, ScenarioInterpreter.ModalKind.MAP_INFO) &&
    ...
    private fun drawStreetDialogueContent(index: Int) {
        ...
        // exact 0.86 Cocos-to-game transform, hence a 373.24 px shift.
    ...
    private fun drawHallManagement(kind: HallManagement) {
        ...
        val profile = unit?.let { gameDataCatalog.battleProfile(it.id, zeroBasedLevel, campaign.unitAttribute(it.id, 17, it.posts)) }
        fun drawUnitSummary(x: Float, y: Float, showEquipment: Boolean = false) {
            ...
            if (showEquipment) {
                ...
                val slots = listOf(
                    Triple("무기:", equipment.firstOrNull { gameDataCatalog.equipmentProfile(it.itemId)?.itemType?.let { t -> t < 20 } == true }, 20.97f),
                    Triple("보구: ", equipment.firstOrNull { gameDataCatalog.equipmentProfile(it.itemId)?.itemType?.let { t -> t in 20..25 } == true }, -114.91f),
                    Triple("보조: ", equipment.firstOrNull { gameDataCatalog.equipmentProfile(it.itemId)?.itemType?.let { t -> t > 25 } == true }, -250.79f),
                )
            ...
            label(gameDataCatalog.postsName(campaign.unitAttribute(unitId, 17, unit?.posts ?: 0)).ifEmpty { "군웅" }, x + 202f, y + 360f)
            ...
            if (showEquipment) {
                campaign.inventory.equippedItems().filter { it.unitId == unitId }.take(1).forEachIndexed { index, equipped ->
                    val item = gameDataCatalog.equipmentProfile(equipped.itemId) ?: return@forEachIndexed
        ...
        when (kind) {
            ...
            HallManagement.BUY -> {
                ...
                if (hallBuyTab == 0) {
                    hallBuyCandidates().take(3).forEachIndexed { index, item ->
                        ...
                        // spacing.  The game applies the same .86 UI scale.
                        ...
                        val total = inventory + campaign.inventory.equippedItems().count { it.itemId == item.id }
                        label(item.name, 283.10f, by + 142f, 188f)
                        label("레벨:", 475.86f, by + 142f, 80f)
                        label("1", 562.72f, by + 142f, 42f)
                        label("속성:", 283.10f, by + 97f, 90f)
                        label(gameDataCatalog.equipmentTypeName(item.itemType), 371.68f, by + 97f, 90f)
                        ...
                        label(if (item.price == 255) "---" else gameDataCatalog.purchasePrice(item).toString(), 562.72f, by + 46f, 76f)
                    }
                } else {
                    hallBuyProperties().take(4).forEachIndexed { index, item ->
                        ...
                        val count = campaign.inventory.items[item.id] ?: 0
                        label(item.name, 282f, by + 73f, 180f)
                        label("인벤토리: $count", 445f, by + 73f, 165f)
                        label("가격: ${gameDataCatalog.purchasePrice(item)}", 338f, by + 30f, 220f)
                    }
                }
            ...
            HallManagement.SELL -> {
                ...
                inventory.forEachIndexed { index, (itemId, count) ->
                    ...
                    if (hallSellTab == 0) {
                        label("Lv: ${campaign.inventory.itemLevels(itemId).firstOrNull() ?: 1}", ix + 94f, iy + 82f, 150f)
                        label("Exp: 0", ix + 220f, iy + 82f, 130f)
                    } else {
                        label("인벤토리: $count", ix + 94f, iy + 82f, 230f)
                    }
                    label("판매가: ${if (item.price == 255) "---" else gameDataCatalog.sellingPrice(item)}", ix + 94f, iy + 37f, 250f)
                }
        ...
        if (kind == HallManagement.EQUIP && hallEquipUnequipConfirmation) {
            ...
            roster.take(6).forEachIndexed { index, id ->
                ...
                label(gameDataCatalog.postsName(posts), 1105.186f * scale, (607f - index * 52f) * scale, 179f * scale, centered = true)
            }
        }
    ...
    private fun drawHallItem(detail: HallItemDetail) {
        ...
        label(if (category == 3) "아이템" else gameDataCatalog.equipmentTypeName(item.itemType), 522.525f, 548.543f, 180f)
        label("가격:", 432.137f, 503.543f, 80.31f)
        label(gameDataCatalog.purchasePrice(item).let { if (it == 255) "---" else it.toString() }, 522.525f, 503.543f, 180f)
        ...
        (0 until 12).forEach { row ->
            ...
            (0..2).forEach { col -> label(gameDataCatalog.postsName(row * 3 + col), 780.186f + col * 143f, sy + 4.84f, 134f, true) }
        }
        ...
        when (kind) {
            HallInfo.FORCES -> {
                ...
                campaign.joinedUnits.take(7).forEachIndexed { row, id ->
                    ...
                    campaign.inventory.ensureDefaultEquipment(id, gameDataCatalog)
                    val level = campaign.unitAttribute(id, 18, unit.level)
                    val profile = gameDataCatalog.battleProfile(id, (level - 1).coerceAtLeast(0), campaign.unitAttribute(id, 17, unit.posts)) ?: return@forEachIndexed
                    val bonus = campaign.inventory.equipment[id]?.let { gameDataCatalog.equipmentBonus(it.asScriptValues(), profile.level) }
                        ?: GameDataCatalog.EquipmentBonus()
                    val displayName = campaign.unitNames[id] ?: GameDataCatalog.sayLayerUnitName(unit.name)
            ...
            HallInfo.PROPERTY -> {
                ...
                val equippedRows = if (hallPropertyTab == HallPropertyTab.PROPERTY) emptyList() else campaign.inventory.equippedItems().filter { accepts(it.itemId) }
                val rows = equippedRows.sortedWith(compareBy<CampaignEquippedItem> { it.itemId }.thenBy { it.unitId }).map { equipped ->
                    val owner = campaign.unitNames[equipped.unitId] ?: gameDataCatalog.unitProfile(equipped.unitId)?.name.orEmpty()
                ...
                } + campaign.inventory.items.entries.sortedBy { it.key }.filter { (id, _) -> accepts(id) }.map { (id, count) ->
                    PropertyRow(id, count, if (hallPropertyTab == HallPropertyTab.AUXILIARY || hallPropertyTab == HallPropertyTab.PROPERTY) "---" else (campaign.inventory.itemLevels(id).firstOrNull() ?: 1).toString(), if (hallPropertyTab == HallPropertyTab.AUXILIARY || hallPropertyTab == HallPropertyTab.PROPERTY) "---" else (campaign.inventory.itemExperiences(id).firstOrNull() ?: 0).toString(), "창고")
                }
                rows.take(7).forEachIndexed { row, entry ->
                    ...
                    val values = listOf("${item.name}${if (entry.count > 1) " ×${entry.count}" else ""}", gameDataCatalog.equipmentTypeName(item.itemType), entry.level, entry.experience, entry.owner)
            ...
            HallInfo.TERRAIN -> {
                ...
                val terrainRows = gameDataCatalog.terrainLayer().select(hallTerrainTab).rows.take(6)
            ...
            HallInfo.TREASURE -> {
                ...
                val treasures = gameDataCatalog.treasureProfiles().take(6)
                treasures.forEachIndexed { index, item ->
                    ...
                    if (item.id in campaign.inventory.discoveredTreasures) {
                ...
                text("지금까지 발견한 보물 ${campaign.inventory.discoveredTreasures.size.toString().padStart(2, '0')} / ${gameDataCatalog.treasureProfiles().size}", x + 8f, y + 37f, 520f)
        ...
        when (kind) {
            ...
            HallInfo.PROPERTY -> if (y in 40f..100f) {
                ...
                row?.let { hallPropertyItemIds().getOrNull(it) }?.let { itemId ->
                    ...
                    val exp = if (hallPropertyTab >= HallPropertyTab.AUXILIARY) 0 else campaign.inventory.itemExperiences(itemId).firstOrNull() ?: 0
                    openHallItem(itemId, level, exp, canDrop = campaign.inventory.items[itemId]?.let { it > 0 } == true && gameDataCatalog.equipmentCategory(requireNotNull(gameDataCatalog.equipmentProfile(itemId))) != 3)
                }
            }
    ...
    private fun hallPropertyItemIds(): List<Int> {
        ...
        val equipped = if (hallPropertyTab == HallPropertyTab.PROPERTY) emptyList() else campaign.inventory.equippedItems()
            .filter { accepts(it.itemId) }.sortedWith(compareBy<CampaignEquippedItem> { it.itemId }.thenBy { it.unitId }).map { it.itemId }
    ...
    private fun openHallItem(itemId: Int, level: String, experience: Int, canDrop: Boolean) {
        ...
        hallItemDetail = HallItemDetail(itemId, level, experience, gameDataCatalog.equipmentExperienceLimit(itemId, level.toIntOrNull() ?: 1))
    ...
    private fun openHallUnitInfo(selectedUnitId: Int) {
        ...
        val rows = ids.mapNotNull { id ->
            ...
            val level = campaign.unitAttribute(id, 18, unit.level)
            val battle = gameDataCatalog.battleProfile(id, (level - 1).coerceAtLeast(0), campaign.unitAttribute(id, 17, unit.posts))
            UnitInfoLayer.Unit(
    ...
    private fun appendHallOverlayFixtureRenderEvents(log: RenderEventLog, fixture: String) {
        ...
        when (fixture) {
            ...
            "item-equipment", "item-property", "item-discard-confirm" -> {
                ...
                val item = requireNotNull(gameDataCatalog.equipmentProfile(if (property) 150 else if (discard) 4 else 0))
                val name = item.name
        ...
        when (kind) {
            HallInfo.FORCES -> {
                ...
                campaign.joinedUnits.take(7).forEachIndexed { row, id ->
                    ...
                    val level = campaign.unitAttribute(id, 18, unit.level)
                    val profile = gameDataCatalog.battleProfile(id, (level - 1).coerceAtLeast(0), campaign.unitAttribute(id, 17, unit.posts)) ?: return@forEachIndexed
                    val bonus = campaign.inventory.equipment[id]?.let { gameDataCatalog.equipmentBonus(it.asScriptValues(), profile.level) } ?: GameDataCatalog.EquipmentBonus()
                    val values = listOf(
                        campaign.unitNames[id] ?: GameDataCatalog.sayLayerUnitName(unit.name), profile.arm.name, profile.level.toString(),
            ...
            HallInfo.PROPERTY -> {
                ...
                val equipped = if (hallPropertyTab == HallPropertyTab.PROPERTY) emptyList() else campaign.inventory.equippedItems().filter { accepts(it.itemId) }
                val rows = equipped.sortedWith(compareBy<CampaignEquippedItem> { it.itemId }.thenBy { it.unitId }).map {
                    ...
                    TraceRow(id, count, if (hallPropertyTab >= HallPropertyTab.AUXILIARY) "---" else (campaign.inventory.itemLevels(id).firstOrNull() ?: 1).toString(),
                        if (hallPropertyTab >= HallPropertyTab.AUXILIARY) "---" else (campaign.inventory.itemExperiences(id).firstOrNull() ?: 0).toString(), "창고")
                }
                rows.take(7).forEachIndexed { row, entry ->
                    val item = gameDataCatalog.equipmentProfile(entry.id) ?: return@forEachIndexed
                    val ry = root[1] + root[3] - 166f - row * 67.08f
                    val values = listOf(item.name + if (entry.count > 1) " ×${entry.count}" else "", gameDataCatalog.equipmentTypeName(item.itemType), entry.level, entry.exp, entry.owner)
                    cx = root[0] + 5f
                    values.forEachIndexed { column, value ->
            ...
            HallInfo.TREASURE -> {
                ...
                label("bg1/footer", "지금까지 발견한 보물 ${campaign.inventory.discoveredTreasures.size.toString().padStart(2, '0')} / ${gameDataCatalog.treasureProfiles().size}", root[0] + 8f, root[1], 520f)
    ...
    private fun appendEquipRenderEvents(
        ...
        val weapon = equipped.firstOrNull { gameDataCatalog.equipmentProfile(it.itemId)?.itemType?.let { type -> type < 20 } == true }
        val armor = equipped.firstOrNull { gameDataCatalog.equipmentProfile(it.itemId)?.itemType?.let { type -> type in 20..25 } == true }
    ...
    private fun appendHallManagementRenderEvents(log: RenderEventLog, kind: HallManagement) {
        ...
        if (kind == HallManagement.BUY || kind == HallManagement.SELL) {
            ...
            campaign.inventory.ensureDefaultEquipment(unitId, gameDataCatalog)
            val bonus = campaign.inventory.equipment[unitId]?.let { gameDataCatalog.equipmentBonus(it.asScriptValues(), profile?.level ?: 1) }
                ?: GameDataCatalog.EquipmentBonus()
            ...
            val postsName = gameDataCatalog.postsName(campaign.unitAttribute(unitId, 17, unit?.posts ?: 0)).ifEmpty { "군웅" }
    ...
    fun compositionTrace(): String {
        ...
        val hallMenu = if (hallMenuOpen || playback.state == PlaybackState.MODAL && playback.currentModalKind == ScenarioInterpreter.ModalKind.AMBITION) {
            val buttonCenters = floatArrayOf(55.107f, 143.365f, 231.846f, 320.74f, 423.317f, 511.575f, 600.056f, 690.441f, 789.44f)
        ...
        val hallInfoTrace = hallInfo?.let { kind ->
            ...
            val rows = when (kind) {
                ...
                HallInfo.PROPERTY -> {
                    ...
                    (equippedCount + campaign.inventory.items.count { (id, _) -> accepts(id) }).coerceAtMost(7).let { count ->
    ...
    private fun dialoguePortrait(unitId: Int): Texture? {
        val face = gameDataCatalog.unitProfile(unitId)?.face ?: return portraitTexture(unitId)
