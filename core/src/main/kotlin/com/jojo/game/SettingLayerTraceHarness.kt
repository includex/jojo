...
object SettingLayerTraceHarness {
    ...
    @JvmStatic fun main(args: Array<String>) {
        ...
        val cases = Regex("""(?s)\{\s*"name"\s*:\s*"([^"]+)"\s*,\s*"initial"\s*:\s*\{([^}]*)}\s*,\s*"events"\s*:\s*\[([^]]*)]""")
            .findAll(raw).map { m ->
                val values = LinkedHashMap<String, Int>()
                Regex(""""([^"]+)"\s*:\s*(-?\d+)""").findAll(m.groupValues[2]).forEach { v -> values[v.groupValues[1]] = v.groupValues[2].toInt() }
                Case(m.groupValues[1], values, Regex("\\\"([^\\\"]+)\\\"").findAll(m.groupValues[3]).map { it.groupValues[1] }.toList())
            }.toList()
        ...
        fun run(spec: Case): String {
            ...
            val store = object : SettingLayer.Store {
                ...
                override fun putInt(key: String, value: Int) { spec.values[key] = value; writes += key to value }
            }
            val sound = object : SettingLayer.Sound {
                override fun music(on: Boolean) { events += "music:$on" }
                override fun effect(on: Boolean) { events += "effect:$on" }
            }
