...
/** Source BattleLayer.unitHide mode used by the production character state. */
...
/**
 * Renderer-independent state missing from the current BattleLayer timeline.
 *
 * In particular this owns the live material/value, the post-hurt hide action,
 * info/status visibility and the source harm-label outline.  BattleLayer can
...
class BattleCharacterPresentation(
    ...
    /** Hurt clip completion calls clsHarmNum then defaultAction. */
    fun finishHit() {
        harm = null
        action = STAND
    }
