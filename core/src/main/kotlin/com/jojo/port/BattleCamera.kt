...
/**
 * Source map geometry after BattleLayer._loadBg has assigned the JSON map
...
/** Source BattleLayer ScrollView follow contract (_contains/centerUnit). */
class BattleCamera(
    ...
    /** Observable equivalent of BattleLayer's synchronous MAP_SCROLLING dispatch. */
     ...
     *
     * `BattleLayer` dispatches MAP_SCROLLING only from its `a != 0` branch.
    ...
    /** Exact source BattleLayer.center(tileX,tileY), including equal-position dispatches. */
...
/** Source Config.js authored global BattleUnit slot layout. */
internal object SourceBattleSlots {
    ...
    /** Source `_unitSet` addresses, preserving the sparse friend range. */
    fun sourceSlot(faction: ScenarioUnitFaction, instanceId: Int, enemyBlockStart: Int = enemyStart): Int = when (faction) {
    ...
    /** Port IDs use a camp-local offset while retaining every source slot. */
    fun battleId(faction: ScenarioUnitFaction, sourceSlot: Int): String = when (faction) {
        ScenarioUnitFaction.MINE -> "mine-$sourceSlot"
        ScenarioUnitFaction.FRIEND -> "friend-${sourceSlot - friendEnd}"
        ScenarioUnitFaction.ENEMY -> "enemy-${sourceSlot - enemyStart}"
    }

    fun stageKey(faction: ScenarioUnitFaction, sourceSlot: Int): String =
        "${faction.name}:${battleId(faction, sourceSlot).substringAfter('-')}"

    /** IDs are the stable port key: mine-N/friend-N/enemy-N. */
    fun indexFor(unit: BattleUnit): Int? {
        unit.sourceBattleSlot?.let { return it }
...
internal fun firstCampCameraUnit(units: Iterable<BattleUnit>, camp: Faction): BattleUnit? {
    val range = SourceBattleSlots.rangeFor(camp)
    return units.asSequence()
        .mapNotNull { unit -> SourceBattleSlots.indexFor(unit)?.let { it to unit } }
