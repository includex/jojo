...
/** Pure port of BattleUnit.showHarmBar(info) target-preview UI. */
object BattleHarmBar {
    ...
    fun show(
        ...
        return if (bounded >= 0) {
            View(bar1 = newProgress, bar2 = oldProgress, amountText = kotlin.math.abs(bounded).toString(), hitRateText = rateText)
        } else {
            View(bar0 = oldProgress, bar2 = newProgress, amountText = kotlin.math.abs(bounded).toString(), hitRateText = rateText)
        }
    }
}
