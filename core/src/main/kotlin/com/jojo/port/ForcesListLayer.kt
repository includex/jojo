...
/** Stateful port of `ui/ForcesListLayer.js` (onCreate/_changeSel/_onClick). */
class ForcesListLayer {
    data class Unit(
        ...
        val battleIndex: Int = id, val poisoned: Boolean = false, val fengZhou: Boolean = false,
    )
    data class Row(val unit: Unit, val colors: List<RowColor>, val labels: List<String>)
    enum class RowColor { BLACK, RED, BLUE }
    data class View(val selectedTab: Int, val rows: List<Row>, val tabsVisible: Boolean, val attached: Boolean, val selectedIndex: Int? = null)
    private var mine = emptyList<Unit>(); private var enemy = emptyList<Unit>(); private var flag=0; private var view: View?=null
    ...
    fun changeSel(tab: Int): View {
        ...
        val ordered = if (flag and 1 != 0) source.sortedWith(compareBy<Unit> { !it.famous }.thenBy { it.battleIndex }) else source
        val rows=ordered.map { unit ->
            val values=listOf(unit.name,unit.post,unit.level.toString(),"${unit.hp}/${unit.maxHp}","${unit.mp}/${unit.maxMp}",unit.attack.toString(),unit.defense.toString(),unit.spirit.toString(),unit.critical.toString(),unit.morale.toString())
    ...
    fun onClose(event:Int):Boolean { if(event!=TOUCH_END) return false; view=view().copy(attached=false); return true }
