...
/**
 * Renderer-independent port of the await/callback order in
 * `BattleLayer._attack2`, `_attack3`, `_attack6`, and the caller's final
 * `_jiesuan(g_charinfo)`.
 *
...
object BattlePhysicalCallbackPlan {
    ...
    sealed interface Step {
        ...
        data class AttackUntilHit(
            val kind: InvocationKind,
            val attackerId: String,
        ) : Step { override val awaitsCallback = true }
        ...
        ) : Step { override val awaitsCallback = false }
        ...
        fun appendInvocation(invocation: Invocation) {
            ...
            invocation.targets.forEach { target ->
                ...
                if (target.harm == 0 && target.mpShieldDamage == 0) {
                    ...
                    if (target.mpShieldDamage == 0) {
                        if (target.lifeStealHealing > 0) add(Step.LifeStealCommitted(invocation.attackerId, target.lifeStealHealing))
                        ...
                        if (target.playerMoneyDelta != 0 || target.enemyMoneyDelta != 0) {
                            add(Step.MoneyAbsorbCommitted(invocation.attackerId, target.playerMoneyDelta, target.enemyMoneyDelta))
                        }
                    }
                    add(Step.HurtUntilComplete(
