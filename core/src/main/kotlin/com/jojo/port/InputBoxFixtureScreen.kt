...
private class InputBoxRenderOracle(
    ...
    fun touchButton(button: Int, event: Int) {
        ...
        attached = false
        if (button == 0) { persist(value); callback(value) } else callback(null)
    }
...
object InputBoxRenderEvents {
    fun jsonl(state: String, value: String, attached: Boolean): String {
        ...
        draw("Canvas/Layer/bg0/box1/editbox/${if (value.isEmpty()) "PLACEHOLDER_LABEL" else "TEXT_LABEL"}", "label",
