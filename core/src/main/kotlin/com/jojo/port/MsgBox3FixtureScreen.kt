...
/** Strict-log diagnostic entered through the real port Buy/Sell adapters. */
class MsgBox3FixtureScreen(private val game: JojoGame, private val state: String) : ScreenAdapter() {
    ...
    init {
        val buy = BuyLayerPort(listOf(item), money = 70, owned = 0, capacity = 99)
        model = if (state == "quantity-buy-initial") {
            ...
            SellLayerPort(listOf(item), money = buy.money, owned = buy.owned)
