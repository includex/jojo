 ...
 *
 * The real layer is persistent for the lifetime of BattleLayer. NOTICE_MSG is
...
/**
 * Stable-endpoint NoticeInfo composition shared by BattleLayer drawing and
...
object NoticeInfoRenderer {
    ...
    fun commands(view: NoticeInfoLayer.View): List<NoticeDrawCommand> = buildList {
        if (view.shown) {
            ...
            view.messages.takeLast(VISIBLE_ROWS).forEachIndexed { index, text ->
                ...
                add(NoticeDrawCommand("$PATH/scrollview/view/content/item/lab", "label", 2f, y, 487f, 32.76f, text = text))
                if (index < VISIBLE_ROWS - 1 && index < view.messages.takeLast(VISIBLE_ROWS).lastIndex) {
                    add(NoticeDrawCommand("$PATH/scrollview/view/content/item/vline2", "sprite", 2f, y + 38.76f, 487f, 2f, "vline2"))
                }
            }
        }
    ...
    fun jsonl(route: String, view: NoticeInfoLayer.View): String = RenderEventLog().also { append(it, route, view) }.jsonl()
