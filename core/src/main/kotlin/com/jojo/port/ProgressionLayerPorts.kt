...
/** Direct, renderer-free ports of the persistence-facing state machines used by the progression overlays. */
class AchievementsLayerPort(private val rewards: Map<Int, List<Any>>) {
    var removed = 0
...
class SignInLayerPort(var count:Int, val signins:MutableList<Int>, private val now:Int) {
...
class RaffleLayerPort(var count:Int,var coins:Int) {
...
class RegisterLayerPort { var removed=0;var changed=false;var display="";val toasts=mutableListOf<String>();val files=mutableListOf<List<String>>();fun textChanged(){changed=true};fun editingEnded(s:String){if(changed)display=s};fun touch(button:Int,event:Int){if(event==2){toasts+="클릭했습니다$button";if(button==1)removed++}}
