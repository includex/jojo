...
internal class ActionStatusFrameBarrier {
     ...
     * are sampled in one port frame and the state edge is attributed to the
     * wrong episode.
     */
 ...
 * Cocos exposes the resulting idle row between those callbacks.  The port's
...
/**
 * Callback-driven port of BattleLayer.ctrl_mine's lifecycle.
 *
...
class BattleTurnController(
    ...
    /** Final callback after BattleLayer has shown every `_ai2` actor pass. */
    ...
    private fun enterNextCamp() {
        ...
        // BattleLayer._setOper only creates RoundLayer when crossing between
