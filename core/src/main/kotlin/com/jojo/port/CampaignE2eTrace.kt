...
internal fun s57GuidedOffensiveMagicPlan(
    ...
    // The recovered S57 roster has exactly one offensive player strategy:
    // Cao Cao (source 0), Whirlwind (10), cost 6/power 50, enemy-targeted.
    ...
    // source scene, so this is a hard route gate rather than a target score.
    if (scenario != "S_57" || !guidedAuthoredRoute || holdFire || firstRoomLeaderVisible || casterSourceId != 0) return null
...
/**
 * The R_01 StartBattleLayer permits four through seven units, but its source
 ...
 * slots before pressing the normal StartBattleLayer confirmation.  Other
...
internal class CampaignE2eDriver(private val config: CampaignE2eTraceConfig) {
    ...
    fun update(delta: Float, current: Screen?) {
        ...
        if (current !== lastScreen && lastScreen is ScenarioPreviewScreen) drainScenarioStarts()
        if (current !== lastScreen) {
            lastScreen = current
            when (current) {
                is TitleScreen -> route += "TitleScreen"
                is ScenarioPreviewScreen -> route += "ScenarioPreviewScreen:${current.campaignE2eState().module}"
                ...
                is BattleLayer -> route += "BattleLayer:${current.campaignE2eState().scenario}"
                null -> route += "null"
                else -> route += current.javaClass.simpleName
            }
            nextInputAt = elapsed + .2f
            Gdx.app.log("JojoPort", "CAMPAIGN_E2E_SCREEN: ${route.last()}")
        }
        if (current is ScenarioPreviewScreen) drainScenarioStarts()
        ...
        when (current) {
            ...
            is ScenarioPreviewScreen -> driveScenario(current.campaignE2eState())
            is BattlePreparationScreen -> driveBattlePreparation(current.campaignE2eState())
            is BattleLayer -> driveBattle(delta, current)
        }
    }
    ...
    private fun drainScenarioStarts() {
        pendingScenarioStarts.forEach { (module, index) ->
            ...
            if (observedScenarioScenes.add(started)) {
                route += "ScenarioPreviewScreen:$started"
                Gdx.app.log("JojoPort", "CAMPAIGN_E2E_SCENE: $started")
            }
        }
    ...
    private fun driveScenario(state: CampaignE2eScenarioState) {
        ...
        if (stateLabel != lastScenarioState) {
            ...
            Gdx.app.log("JojoPort", "CAMPAIGN_E2E_STATE: $stateLabel")
        }
        ...
        when (state.playback) {
            ...
            PlaybackState.COMPLETE -> if (state.menuVisible && hallBattleCommands.add(state.module)) {
                ...
                route += "ScenarioPreviewScreen:${state.module}:hall-battle-button"
                pointerOnce(
    ...
    private fun driveBattle(delta: Float, screen: BattleLayer) {
        ...
        if (stateLabel != lastBattleState) {
            ...
            Gdx.app.log("JojoPort", "CAMPAIGN_E2E_BATTLE: $stateLabel")
        }
        if (state.initialScene1Started && observedInitialBattleScenes.add(state.scenario)) {
            ...
            route += "BattleLayer:${state.scenario}:scene1"
        }
        ...
        if (state.resultScene1Started && observedResultBattleScenes.add(state.scenario)) {
            ...
            route += "BattleLayer:${state.scenario}:result-scene1"
        }
        if (state.scene2Started && observedBattleScene2.add(state.scenario)) {
            sawScene2 = true
            route += "BattleLayer:${state.scenario}:scene2"
        }
        if (state.savePromptOpen && observedSavePrompts.add(state.scenario)) {
            sawSavePrompt = true
            route += "BattleLayer:${state.scenario}:save-prompt"
        }
    ...
    private fun screenObservation(): String = when (val screen = lastScreen) {
        is ScenarioPreviewScreen -> screen.campaignE2eState().let {
            "screen=ScenarioPreviewScreen;module=${it.module};scene=${it.sceneIndex};" +
    ...
    private fun finish(actualModule: String, actualSceneIndex: Int, forwardOvershoot: Boolean) {
        val expected = listOf(
            ...
            "ScenarioPreviewScreen:R_00:scene0", "ScenarioPreviewScreen:R_00:scene1",
            "ScenarioPreviewScreen:R_00:scene2", "ScenarioPreviewScreen:R_00:scene3",
            "BattleLayer:S_00", "BattleLayer:S_00:scene1",
            "BattleLayer:S_00:result-scene1", "BattleLayer:S_00:scene2",
            "BattleLayer:S_00:save-prompt", "ScenarioPreviewScreen:R_01",
            "ScenarioPreviewScreen:R_01:scene0", "ScenarioPreviewScreen:R_01:scene1",
        )
...
internal class ProductionBattleInputDriver(
    ...
    fun update(
        ...
        when {
            ...
            tacticalInputReady && !state.authoredRouteHoldFire &&
                ...
                // Re-observe before map input. BattleLayer only emits this
    ...
    private fun battleObservation(): String {
        ...
        return "screen=BattleLayer;scenario=${state.scenario};playback=${state.playback};" +
