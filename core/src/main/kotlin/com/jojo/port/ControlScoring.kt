...
/**
 * Injectable, side-effect-free port of Control.js lines 325-666 and the
...
object ControlScoring {
    object Arm { const val QUAN_NENG = 0; const val WEN_GUAN = 1; const val WU_JIANG = 2 }
    /** BattleConfg.UNIT_STATUS_LIFT: DOWN=0, NORMAL=1, UP=2. */
    object Lift { const val DOWN = 0; const val NORMAL = 1; const val UP = 2 }
    object Status {
        const val ATT = 0; const val DEF = 1; const val SPR = 2; const val CRI = 3
        const val MOR = 4; const val MOV = 5; const val MB = 7; const val JZ = 8
        const val HL = 9; const val ZD = 10; const val XD = 14; const val MAX = 15
    }
    object Category {
        const val JDMJ = 4; const val JDSQ = 5; const val JDNL = 6; const val JDFY = 7
        const val HL = 8; const val ZD = 9; const val MB = 10; const val FZ = 11
        const val HFZT = 15; const val ZJYDL = 16; const val ZJMJ = 17; const val ZJSQ = 18
        const val ZJNL = 19; const val ZJFY = 20; const val ZCXD = 21; const val MX = 24
        const val SQ = 30
    }
    object Type { const val XISHOU_HP = 5; const val XISHOU_MP = 6; const val HUIFU_HP = 19; const val HUIFU_MP = 20 }
    ...
    interface Unit {
        val index: Int; val hp: Int; val hpCur: Int; val mp: Int; val mpCur: Int
        val armType: Int; val isRemote: Boolean; val famous: Boolean; val mine: Boolean
        val ai: Int; val aiValue: Int
    ...
    interface Magic { val id: Int; val category: Int; val type: Int; val harmType: Int; val expendMp: Int }
    /**
    ...
    data class Move(val x: Int, val y: Int, val value: Int, val attacks: List<Action> = emptyList(), val magics: List<Action> = emptyList())
    ...
    fun attackValue(attacker: Unit, target: Unit, counter: Boolean, values: Values = Values(), skills: Skills = Skills()): Int {
        var score = 0
    ...
    fun magicValue(magic: Magic, caster: Unit, target: Unit, cache: MutableMap<String, Int>,
                   hitRate: (Unit, Unit, Magic) -> Int = { _, _, _ -> 100 }, values: Values = Values(), skills: Skills = Skills()): Int {
        val key = "magic_${caster.index}_${target.index}_${magic.id}"
        cache[key]?.let { return it }
        var score = 0; var hpSteal = false; var lift = 0; var abnormal = 0
        ...
        when (magic.type) {
            ...
            Type.HUIFU_HP -> if (target.isCanXue() && target.mine == caster.mine) {
                ...
                if (harm >= 1) score += floorRate(harm, values.hpMpRate, target.hp) + if (famousMask and 2 != 0) values.healFamous else values.healNormal
            }
            else -> {
                ...
                run category@ {
                    when (magic.category) {
                        Category.HFZT -> for (s in Status.MB..Status.ZD) if (target.status(s) != Lift.NORMAL) abnormal = abnormal or (1 shl s)
                        Category.JDNL -> { val k = armMask(target.armType); if (k and 1 != 0) { if (target.status(Status.ATT) == Lift.DOWN) return@category; lift = lift or (1 shl Status.ATT) }; if (k and 2 != 0) { if (target.status(Status.SPR) == Lift.DOWN) return@category; lift = lift or (1 shl Status.SPR) } }
                        Category.JDFY -> { if (target.status(Status.DEF) == Lift.DOWN) return@category; lift = lift or (1 shl Status.DEF) }
                        Category.JDMJ -> { if (target.status(Status.CRI) == Lift.DOWN) return@category; lift = lift or (1 shl Status.CRI) }
                        Category.JDSQ -> { if (target.status(Status.MOR) == Lift.DOWN) return@category; lift = lift or (1 shl Status.MOR) }
                        Category.SQ -> for (s in Status.ATT..Status.MOV) if (target.status(s) != Lift.DOWN) lift = lift or (1 shl s)
                        Category.MB -> { if (target.status(Status.MB) != Lift.NORMAL) return@category; lift = lift or (1 shl Status.MB) }
                        Category.FZ -> { if (target.status(Status.JZ) != Lift.NORMAL) return@category; lift = lift or (1 shl Status.JZ) }
                        Category.HL -> { if (target.status(Status.HL) != Lift.NORMAL) return@category; lift = lift or (1 shl Status.HL) }
                        Category.ZD -> { if (target.status(Status.ZD) != Lift.NORMAL) return@category; lift = lift or (1 shl Status.ZD) }
                        ...
                        Category.ZCXD -> { if (target.ai < 3 || target.status(Status.XD) == Lift.NORMAL) return@category; abnormal = abnormal or (1 shl Status.MOV) }
                        Category.ZJYDL -> { if (target.status(Status.MOV) != Lift.DOWN) return@category; abnormal = abnormal or (1 shl Status.MOV) }
                        Category.ZJMJ -> { if (target.status(Status.CRI) != Lift.DOWN) return@category; abnormal = abnormal or (1 shl Status.CRI) }
                        Category.ZJSQ -> { if (target.status(Status.MOR) != Lift.DOWN) return@category; abnormal = abnormal or (1 shl Status.MOR) }
                        Category.ZJNL -> { val k = armMask(target.armType); if (k and 1 != 0) { if (target.status(Status.ATT) != Lift.DOWN) return@category; abnormal = abnormal or (1 shl Status.ATT) }; if (k and 2 != 0) { if (target.status(Status.SPR) != Lift.DOWN) return@category; abnormal = abnormal or (1 shl Status.SPR) } }
                        Category.ZJFY -> { if (target.status(Status.DEF) != Lift.DOWN) return@category; abnormal = abnormal or (1 shl Status.DEF) }
                    }
                }
        ...
        if (magic.harmType != 4) {
            ...
            score += if (caster.mine == target.mine) -floorRate(harm, values.hpMpRate, target.hp) else floorRate(harm, values.hpMpRate, target.hp)
            if (hpSteal) { harm = minOf(caster.hp - caster.hpCur, harm); score += floorRate(harm, values.hpMpRate, caster.hp) }
        }
        if (score < 1) return 0
    ...
    private fun armMask(arm: Int) = when (arm) { Arm.QUAN_NENG -> 3; Arm.WEN_GUAN -> 2; else -> 1 }
    ...
    private fun statusValue(mask: Int, unit: Unit, v: Values): Int {
        ...
        if (has(Status.DEF)) n += when (unit.armType) { Arm.QUAN_NENG -> v.attackQn; Arm.WEN_GUAN -> v.defWg; else -> v.defWj }
        ...
        if (has(Status.JZ)) n += when (unit.armType) { Arm.QUAN_NENG -> v.jzQn; Arm.WEN_GUAN -> v.jzWg; else -> v.jzWj }
        if (has(Status.HL)) n += when (unit.armType) { Arm.QUAN_NENG -> v.hlQn; Arm.WEN_GUAN -> v.hlWg; else -> v.hlWj }
        if (has(Status.ZD)) n += when (unit.armType) { Arm.QUAN_NENG -> v.zdQn; Arm.WEN_GUAN -> v.zdWg; else -> v.zdWj }
