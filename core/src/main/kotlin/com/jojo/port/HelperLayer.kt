...
/**
 * Injectable port of `ui/HelperLayer.js` and its `Global/scene/HelperLayer`
...
class HelperLayer(private val model: Model, private val removeFromParent: () -> Unit = {}) {
    ...
    data class View(val prefab: Prefab, val richText: String, val attached: Boolean, val blocksUnderlyingInput: Boolean = true)
    ...
    fun onButtonTouch(eventType: Int): View {
        if (eventType == TOUCH_END) { removeFromParent(); attached = false }
        return view()
    }
...
object SourceInfoText {
    fun replace(
        ...
        if (flags and 1 != 0) {
            Regex("&\\*[.+](\\d+)\\n").findAll(text).toList().forEach { text = text.replace(it.value, "&${it.groupValues[1]}\n") }
            Regex("\\*[.+](\\d+)").findAll(text).toList().forEach { text = text.replace(it.value, unitName(it.groupValues[1].toInt())) }
        }
        if (flags and 2 != 0) {
            Regex("\\*/(\\d+)").findAll(text).toList().forEach { text = text.replace(it.value, global(it.groupValues[1].toInt()).toString()) }
            Regex("\\*(\\d+)").findAll(text).toList().forEach { text = text.replace(it.value, global(it.groupValues[1].toInt()).toString()) }
        }
