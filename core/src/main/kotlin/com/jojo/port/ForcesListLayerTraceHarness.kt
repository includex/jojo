...
object ForcesListLayerTraceHarness {
    private data class Case(val name:String,val flag:Int,val mine:List<ForcesListLayer.Unit>,val enemy:List<ForcesListLayer.Unit>,val events:List<String>)
    private fun cases(input:String):List<Case> {
        val compact=input.replace(Regex("\\s+"),"")
        return Regex("""\{"name":"([^"]+)","flag":(\d+),"mine":\[(.*?)]\,"enemy":\[(.*?)]\,"events":\[(.*?)]}""").findAll(compact).map { c ->
            fun units(raw:String):List<ForcesListLayer.Unit> = Regex("""\{([^{}]*)}""").findAll(raw).map { m ->
                ...
                ForcesListLayer.Unit(n("id"),n("id").toString(),"",1,10,10,5,5,0,0,0,0,0,b("famous"),status?.let { (0..4).associateWith { _ -> it } } ?: emptyMap(),n("index"),b("poison"),b("fengZhou"))
            }.toList()
            Case(c.groupValues[1],c.groupValues[2].toInt(),units(c.groupValues[3]),units(c.groupValues[4]),Regex("\"([^\"]+)\"").findAll(c.groupValues[5]).map { it.groupValues[1] }.toList())
        }.toList()
    }
    private fun e(x:String)=x.replace("\\","\\\\").replace("\"","\\\"")
    @JvmStatic fun main(args:Array<String>) {
        val all=cases(Files.readString(Path.of(args[0]))).joinToString(",","{","}") { c ->
            val layer=ForcesListLayer(); var destroyed=false; var n1=1; var n2=1; var rendered=0; val routes=mutableListOf<String>(); val trace=mutableListOf<String>()
            fun state(step:String):String {
                val view=layer.view()
                val rows=view.rows.mapIndexed { index,row ->
                    val colors=row.colors.mapIndexed { i,_ -> when {
                ...
                return "{\"step\":\"${e(step)}\",\"sel\":${view.selectedTab},\"rows\":$rows,\"tabsVisible\":${view.tabsVisible},\"dead\":${!view.attached},\"baseDestroyed\":$destroyed,\"routes\":[${routes.joinToString(",")}],\"n1\":${if(destroyed) 0 else n1},\"n2\":${if(destroyed) 0 else n2}}"
            }
            ...
            c.events.forEach { event ->
                ...
                when(p[0]) {
                    "tab" -> if(c.flag and 1 != 0 && p[2].toInt()==2 && p[1].toInt()!=layer.view().selectedTab) {
                        ...
                        repeat(rendered) { if(it%2==0) { n1--; if(n1<1)n1++ } else { n2--; if(n2<1)n2++ } }
                    }
                    "row" -> layer.onRowTouch(p[1].toInt(),p[2].toInt())?.let {
                        routes += "{\"layer\":\"UnitInfoLayer\",\"index\":${p[1]},\"unitIds\":[${layer.view().rows.map { r->r.unit.battleIndex }.joinToString(",")}],\"flag\":${c.flag}}"
                    }
