...
/**
 * Injectable port of recovered `battle/Battle.js`.
 *
...
class BattleScenePort(
    ...
    interface BattleLayer {
        /** BattleLayer.save(out). */
        fun save(out: MutableMap<String, Any?>)
        /** BattleLayer.filterUnits(1187/1196). */
    ...
    interface Factory {
        fun addBattleLayer(data: Any?): BattleLayer
    ...
    private var battleLayer: BattleLayer? = null

    /** Battle.onCreate: only BattleLayer is eagerly constructed. */
    fun onCreate(data: Any?) { battleLayer = factory.addBattleLayer(data) }
