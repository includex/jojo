...
data class ScenarioBattleUnit(
    ...
    /** BattleLayer.createFriend/createEnemy default direction is down (2). */
    val direction: Int = 2,
    val level: Int = 0,
    ...
    /** Exact Config.js `_unitSet` index, not the authored `i` field. */
    val sourceBattleSlot: Int = SourceBattleSlots.sourceSlot(faction, instanceId),
)

val ScenarioBattleUnit.battleId: String
    get() = SourceBattleSlots.battleId(faction, sourceBattleSlot)

val ScenarioBattleUnit.stageKey: String
    get() = SourceBattleSlots.stageKey(faction, sourceBattleSlot)

/** BattleLayer gate/object type plus the terrain ID it overlays. */
 ...
 * captures the extra rules applied immediately before StartBattleLayer opens.
 */
data class ScenarioBattleEntryPlan(
    val selectionLimit: ScenarioJoinBattleLimit,
    /** Non-null when HallLayer bypasses StartBattleLayer altogether. */
...
/** Arguments retained from BattleLayer.reward(t, items, end). */
...
/** Camera/barrier side effect of BattleLayer.setObject2/playMagicMeff. */
...
/** Synchronous BattleLayer.center request; repeated equal coordinates are distinct dispatches. */
data class ScenarioCameraCenterRequest(val x: Int, val y: Int)

/**
 * Source BattleLayer calls whose model mutation is not their complete contract.
 *
...
sealed class ScenarioScriptPresentationRequest {
    ...
    data class GetItem(
        ...
        /** BattleLayer._filterUnit selector (1025 mine, 1027 main actor, or character id). */
...
class ScenarioStage(private val campaign: CampaignState = CampaignState()) {
    ...
    /** BattleLayer._winConditions fields, assembled by winConProcess. */
    ...
    /** BattleLayer.setGlobalData's first argument, the authoritative turn cap. */
    ...
    var battleLevelOffset: Int = 0
        private set
    /**
     * Historical port name retained for callers.  BattleLayer stores a
    ...
    var fightInitialized: Boolean = false
        private set
    /** BattleLayer START_OPER flag. It is distinct from initFight(). */
    var battleOperationStarted: Boolean = false
        private set
    /**
     * BattleLayer.draw() is the source boundary that removes the opaque
    ...
    /** BattleLayer._unitIds: a character ID always resolves to its first actor. */
     ...
     * and its default must not overwrite BattleLayer's authored spawn facing.
     */
     ...
     */
    fun setBattleUnitPosts(unitId: Int, posts: Int, flags: Int = 19, data: OriginalGameData = OriginalGameData.load(), enabledFeatures: Int = 0): CampaignUnitPostsChange? {
    ...
    fun setModelUnitPosts(
        ...
        posts: Int,
        flags: Int = 3,
        data: OriginalGameData = OriginalGameData.load(),
    ...
    /**
     * BattleUnit.move uses BattleLayer.findEmptyPos + its terrain-weighted AStar.
    ...
    /** BattleLayer.setMaxRound: ZJHH adds four, and setProperty skips an equal write. */
    ...
    fun addUnitLevels(unitId: Int, delta: Int, registeredFeatures: Int = 0): CampaignUnitLevelChange? =
        campaign.addUnitLevels(unitId, delta, OriginalGameData.load(), registeredFeatures)?.also(scriptedUnitLevelChanges::addLast)
    ...
    private fun battleAvatarId(unit: ScenarioBattleUnit, posts: Int, data: OriginalGameData): Int? {
    ...
    fun startFight(firstUnitId: Int, secondUnitId: Int, backgroundIndex: Int): Long {
        ...
        // BattleLayer.startFight switches to ENTER_DANTIAO until fight.end().
        backgroundSound = 8
        return fightId
    }
    ...
    fun joinUnit(unitId: Int) {
        ...
        campaign.ensureDefaultEquipment(unitId, OriginalGameData.load())
    }
    fun setJoinBattle(minimum: Int, maximum: Int, required: List<Any?>, excluded: List<Any?>) {
        ...
        ).also { campaign.configureBattleRoster(it) }
    }
    ...
    fun getItem(itemId: Int, suppliedCountOrLevel: Int = 0, addToInventory: Boolean = true): String {
        ...
        if (addToInventory) {
            ...
            campaign.discoverTreasure(itemId, data)
            acquiredItems += itemId
        }
        return "얻었다 ${item?.name ?: "아이템 $itemId"} ${if (property) "X" else "Lv"}$displayValue"
    }
    /** BattleLayer.getItem's InfoLayer text differs from HallLayer.getItem. */
    fun battleItemCompletionMessage(itemId: Int): String =
        "얻었다${OriginalGameData.load().equipmentProfile(itemId)?.name ?: "아이템 $itemId"}!"
    fun setJoinEquip(unitId: Int, weapon: Int, weaponLevel: Int, armor: Int, armorLevel: Int, auxiliary: Int) {
        ...
        campaign.setEquipment(unitId, weapon, weaponLevel, armor, armorLevel, auxiliary)
    }
    ...
    /** setUnitStatus is an immediate BattleLayer operation, so consume each command once. */
    ...
    /** BattleLayer.nearEvent(values, flag): bit 1 is talk; otherwise vs. */
    ...
    fun createBattleUnits(faction: ScenarioUnitFaction, entries: List<Any?>) {
        // BattleLayer.createEnemy selects one block per invocation by probing
        // only its anchor.  A sparse first call with no i=0 therefore really
        // does reuse 60 on the next call, exactly as the source does.
        val enemyBlockStart = if (faction == ScenarioUnitFaction.ENEMY) {
            generateSequence(SourceBattleSlots.enemyStart) { it + SourceBattleSlots.enemyBlockLength }
                .first { candidate -> battleUnits.values.none { it.sourceBattleSlot == candidate } }
        } else null
        entries.forEachIndexed { fallbackIndex, raw ->
            ...
            val characterId = if (faction == ScenarioUnitFaction.MINE && "id" !in entry) {
                // BattleLayer.createMine resolves idx exclusively through the
                // already-selected BattleHall roster.  It stops when that
                // roster has no matching slot; inventing an ID here produces
                // a playable unit the original battle never creates.
                campaign.battleRoster.getOrNull(rosterIndex) ?: return@forEachIndexed
            ...
            val sourceBattleSlot = SourceBattleSlots.sourceSlot(
                faction,
                if (faction == ScenarioUnitFaction.MINE) rosterIndex else instanceId,
                enemyBlockStart ?: SourceBattleSlots.enemyStart,
            )
            val battleUnit = ScenarioBattleUnit(
                ...
                // BattleLayer._truncUnitData: controllable units are active;
                ...
                sourceBattleSlot = sourceBattleSlot,
            )
            ...
            if (firstBattleUnitKeyByCharacterId.putIfAbsent(characterId, battleUnit.stageKey) == null) unit(characterId).apply {
                ...
                // placed HallUnit. Omitted battle coordinates use the source
                ...
                // BattleLayer's later live-state synchronization reads this
    ...
    /** Source `BattleLayer.unit(characterId)`: first pushed actor only. */
    fun battleUnitForCharacterId(characterId: Int): ScenarioBattleUnit? =
        firstBattleUnitKeyByCharacterId[characterId]?.let(battleUnits::get)
            ...
            // directly; insertion order is the source push order there too.
            ?: battleUnits.values.firstOrNull { it.characterId == characterId }

    /** Source `BattleLayer.unit(index, 1)`: direct `_unitSet` slot lookup. */
    fun battleUnitForSourceSlot(sourceBattleSlot: Int): ScenarioBattleUnit? =
        battleUnits.values.firstOrNull { it.sourceBattleSlot == sourceBattleSlot }

    /**
     * Mirrors BattleLayer.setAI's rectangle/camp selection, including its
    ...
    private fun ScenarioBattleUnit.matchesAiCamp(camp: Int): Boolean = when (camp) {
        ...
        // BattleLayer._testUnitType uses 4 for Mine+Friend and 5 for
    ...
    private fun beginMove(unit: TacticalUnit, path: List<Pair<Int, Int>>, x: Int, y: Int, direction: Int) {
        ...
        // BattleLayer.findEmptyPos may move an occupied authored destination
