...
render oracles. A reference/game pixel or pairwise match in the latter category is not
...
| Hall/street dialogue | `ScenarioScreen` + `ScenarioInterpreter` + `ScenarioStage` | The duplicate `SayDialogueLayer` and its dedicated harness were deleted. |
| Battle dialogue | `BattleScreen` + `ScenarioInterpreter` | Same separation as Hall dialogue. |
| Battle unit/terrain information | `BattleScreen` -> `MineUnitInfoLayer`/`OtherUnitInfoLayer` and terrain overlay | The duplicate `BattleInfoPopup` and its dedicated harness were deleted. |
| Stage script overlays | `ScenarioStage` + `ScenarioScreen` | The duplicate stage-layer compatibility type and its dedicated harness were deleted. |
| Victory conditions | `BattleScreen` -> compact/full victory-condition layers | The unused parallel `WinConditionRouteFlow` was deleted. |
...
feature from Title, an authored R script, battle preparation, or `BattleScreen`.
...
|---|---|---|
| InputBox prefab | source inventory + capture oracle | Registered in `Instance.LAYER`, but no recovered production caller was found. The production-looking game class/test were deleted; only private capture state remains. |
| Progress prefab | source inventory + capture oracle | Registered prefab only; no recovered production caller was found. The production-looking game class/test were deleted; only private capture state remains. |
...
Button 7 opens the achievements layer only when saved rewards exist. Buttons 8 and
...
the Login context and `BattleScreen` supplies the Battle context. The desktop helper
...
The unused reset-layer reconstruction was deleted because recovered source contains only prefab
