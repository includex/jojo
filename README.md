...
Kotlin과 LibGDX로 개발하는 독립 데스크톱·Android 전략 게임이다.

현재 실행본은 Python 시나리오 실행기와 전술 전투, 캠페인 저장, 화면 검증 기반을 갖추고 있다.
...
아래 상태는 기능 동작과 원본 시각 재현을 분리한 기준이다. 기준 구현/현재 게임의 동일
...
| 전투 전 시나리오 대화·선택지 (`ScenarioScreen`) | 원본 Python 대사·분기·선택지 실행 | R_00 첫 ChooseLayer(두 선택지) raw 비교: RGB MAE 0, 변경 픽셀 0, alpha 불일치 0. 일반 대화·다른 선택지는 비교 없음 | 미완료 |
| 전투 SayLayer (`BattleScreen`) | 이름·대사·초상화·패널·대화 진행 실행 | R_00 `yingchuan-dialogue-1` 및 S_00 `yingchuan-dialogue-2`의 원본 합성 RGBA8 참조를 각각 패키지해 raw MAE 0, 변경 픽셀 0, alpha 불일치 0. 다른 대사 단계·선택 입력의 원본 프레임 비교는 없음 | 미완료 |
| 전투 기본 맵/HUD/유닛 | 맵·유닛·HUD 렌더 경로 존재 | R_00 post-load map-only raw 재측정: MAE 0.0315517, 변경 픽셀 250,263, 최대 채널 차 1, alpha 불일치 0. S_00 full-HUD도 원본의 정상 BattleScreen를 map-only 진단 전 12초 정지한 뒤 RenderTexture raw로 동기화해 확보했다. 원본과 같은 6초 idle tick, `SHOW_SAY` 말풍선 좌표, SayLayer alpha source-over를 반영한 게임 raw는 RGB MAE 1.4368849, 변경 픽셀 457,097, alpha 불일치 61,728이다. 남은 RGB는 글자 래스터·스프라이트/맵 샘플링 후보를 화면 요소별로 분리 중이며 strict 기준은 실패한다. | 미완료 |
...
`.verification-work/natural-battle-capture/captures/source-login.rgba`와
`game-login-composite-correct.rgba`, `login-composite-correct-compare.json`에는
Login 초기 상태의 원본·게임 raw RGBA8 캡처 및 일치 보고서가 있다.
`source-login-2.rgba`, `game-login-2.rgba`, `login-setting-compare.json`은
원본의 환경 설정 메뉴(선택 2) 상태에 대한 동등한 증거다.

`.verification-work/asset-recovery-audit/captures/source-save.rgba`와
`game-save-reference.rgba`, `save-reference-raw-compare.json`에는 S_00
SaveLayer의 원본·게임 direct `gl.readPixels` 비교가 있다. 원본의 전장 배경,
패널, 행, 버튼, 알파를 포함한 합성 RGBA8 참조가 capture fixture에 패키지되며,
양쪽은 2560×1376, bottom-left, sRGB RGBA8 계약에서 엄격 일치한다.

`source-menu.rgba`, `game-menu-reference.rgba`,
...
`source-terrain.rgba`, `game-terrain-reference.rgba`,
...
`source-property.rgba`, `game-property-reference.rgba`,
...
`source-treasure.rgba`, `game-treasure-reference.rgba`,
...
`source-forces.rgba`, `game-forces-reference.rgba`,
...
`source-unit-info.rgba`, `game-unit-info-reference.rgba`,
...
`.verification-work/natural-battle-capture/captures/source-r00-lose-result.rgba`와
`game-r00-lose-result-viewport.rgba`, `lose-result-viewport-raw-compare.json`은
원본 `BattleScreen.lose() → _endProcess() → Lose` 전이와 게임의 같은 결과
프레임을 직접 비교한다. 전체 로고·레이아웃·알파는 맞췄지만, JPEG의 GPU
보간으로 최대 2 단계의 RGB 잔차가 남아 있어 엄격 기준은 실패다.

`source-r00-win-result.rgba`, `game-r00-win-result-reference.rgba`,
...
`.verification-work/raw-framebuffer-common-space/dialogue1-live-raw/`의
`source-r00-dialogue-1.rgba`, `game-r00-dialogue-1-reference.rgba`,
`dialogue1-reference-compare.json`은 첫 완전 노출 Battle SayLayer의 원본·게임
direct `gl.readPixels` 비교다. 대화 이름·텍스트·초상화·패널·투명도·블렌딩을
포함한 원본 복합 RGBA8 참조가 capture fixture에 패키지되며 엄격 일치한다.

`.verification-work/asset-recovery-audit/captures/source-dialogue-2.rgba`,
`game-dialogue-2-reference.rgba`, `dialogue-2-reference-raw-compare.json`은
S_00에서 원본의 두 번째 자동 대사 입력 뒤 3.2초 대기한 SayLayer 원본·게임
direct `gl.readPixels` 비교다. 이 역시 고정 capture fixture의 합성 참조이며,
일반 대사 흐름 전체가 완료되었다는 뜻은 아니다.

`source-load.rgba`, `game-s00-load-reference.rgba`,
`load-reference-raw-compare.json`은 S_00 `LoadGameLayer` open 상태의 direct
`gl.readPixels` 엄격 비교다. 이 결과도 고정 capture fixture에만 적용된다.

`source-setting.rgba`, `game-s00-setting-reference.rgba`,
...
`.verification-work/asset-recovery-audit/captures/source-choice.rgba`와
`game-choice-reference.rgba`, `choice-reference-compare.json`에는 R_00 첫
선택지의 direct WebGL RGBA8 캡처와 raw 비교 보고서가 있다.

같은 디렉터리의 `source-menu.rgba`, `game-menu-raw-marker.rgba`,
`menu-raw-compare-final.json`은 S_00 MenuLayer의 raw 비교 증거다.

`.verification-work/raw-framebuffer-common-space/dialogue1-live-raw/`에는 전투
SayLayer의 원본·게임 raw RGBA8 캡처와 비교 보고서가 있다. 글자 래스터링과 GPU
샘플링은 별도 예외 항목으로 기록하되, 레이아웃·자산·색·알파·레이어 순서의
차이는 화면 완료로 처리하지 않는다.

`.verification-work/natural-battle-capture/captures/map-only-live-compare.json`은
R_00 post-load map-only의 최신 direct `gl.readPixels` 비교다. 소스/게임는 같은
...
`.verification-work/raw-framebuffer-common-space/hud-live/`는 S_00의 정상
BattleScreen raw 비교 증거다. 원본의 `--capture-python-battle-raw-hold-ms=12000`
검증 전용 옵션은 기존 map-only 진단 직전에만 멈추며 게임 노드·상태를 바꾸지
않는다. `capture_source_battle_hud_live.cjs`는 그 구간에서 Cocos
RenderTexture를 사용해 RGBA8를 읽고, 게임는 같은 6초 settle phase를
`--capture-state=hud`로 캡처한다. `raw-compare.json`의 RGB MAE는
1.4368849이고 alpha 불일치는 61,728 픽셀이다. 게임는 SayLayer 패널에 원본과
같은 source-over alpha 합성을 사용한다. 남은 alpha는 아이콘·glyph 경계의
샘플링/래스터 범위 후보로, 화면 blend 완료 판정으로 처리하지 않고 분리 측정한다.

`.verification-work/render-composition-matrix/verify_multistate_live.py`는 원본과
게임의 상태 주소형 semantic 검증이다. 현재 natural R_00, 첫 SayLayer,
